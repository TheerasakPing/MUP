...
(0, bun_test_1.describe)("createOrpcServer", () => {
    (0, bun_test_1.test)("serveStatic fallback does not swallow /api routes", async () => {
        ...
        try {
            ...
            (0, bun_test_1.expect)(uiText).toContain("mux");
    ...
    (0, bun_test_1.test)("OAuth callback routes accept POST redirects (query + form_post)", async () => {
        const stubContext = {
        ...
        try {
            ...
            (0, bun_test_1.expect)(queryRes.status).toBe(200);
            const queryText = await queryRes.text();
            (0, bun_test_1.expect)(queryText).toContain("Enrollment complete");
            ...
            (0, bun_test_1.expect)(formRes.status).toBe(200);
            const formText = await formRes.text();
            (0, bun_test_1.expect)(formText).toContain("Enrollment complete");
        }
        finally {
            await server?.close();
        }
