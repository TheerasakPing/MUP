...
const replayBufferedStreamMessageRelay_1 = require("./replayBufferedStreamMessageRelay");
...
function normalizeMuxMessageFromDisk(value) {
    ...
    return (0, legacy_1.normalizeLegacyMuxMetadata)(value);
}
    ...
    try {
        ...
        for (let i = 0; i < lines.length; i++) {
            try {
                ...
                log_1.log.warn(`Skipping malformed JSON at line ${i + 1} in ${params.logLabel}:`, parseError instanceof Error ? parseError.message : String(parseError), "\nLine content:", lines[i].substring(0, 100) + (lines[i].length > 100 ? "..." : ""));
            }
        }
        return messages;
    }
...
async function readPartialJsonBestEffort(partialPath) {
    try {
        ...
        log_1.log.warn("Failed to read partial.json for transcript", {
            partialPath,
...
async function findSubagentTranscriptEntryByScanningSessions(params) {
    ...
    for (const dirent of dirents) {
        ...
        const artifacts = await (0, subagentTranscriptArtifacts_1.readSubagentTranscriptArtifactsFile)(sessionDir);
    ...
    return t.router({
        ...
        config: {
            ...
            updateModelPreferences: t
                ...
                .handler(async ({ context, input }) => {
                const normalizeModelString = (value) => {
                    ...
                    const normalized = (0, models_1.normalizeGatewayModel)(trimmed);
                    if (!(0, models_1.isValidModelFormat)(normalized)) {
                        return undefined;
                    }
                    return normalized;
                };
        ...
        agents: {
            list: t
                ...
                const { runtime, discoveryPath, metadata } = await resolveAgentDiscoveryContext(context, input);
                ...
                if (input.workspaceId && metadata) {
                    try {
                        planReady = await (0, helpers_1.hasNonEmptyPlanFile)(runtime, metadata.name, metadata.projectName, input.workspaceId);
                    }
                    catch {
                        planReady = false;
                    }
                ...
                const resolved = await Promise.all(descriptors.map(async (descriptor) => {
                    try {
                        const resolvedFrontmatter = await (0, agentDefinitionsService_1.resolveAgentFrontmatter)(runtime, discoveryPath, descriptor.id);
                        ...
                        const uiSelectableBase = typeof resolvedFrontmatter.ui?.hidden === "boolean"
        ...
        agentSkills: {
            ...
            listDiagnostics: t
                ...
                const { runtime, discoveryPath } = await resolveAgentDiscoveryContext(context, input);
                return (0, agentSkillsService_1.discoverAgentSkillsDiagnostics)(runtime, discoveryPath);
            }),
            get: t
        ...
        providers: {
            ...
            setModelMetadata: t
                ...
                .handler(({ context, input }) => context.providerService.setModelMetadata(input.provider, input.modelId, input.metadata)),
            onConfigChanged: t
        ...
        policy: {
            ...
            refreshNow: t
                ...
                return (0, result_1.Ok)(context.policyService.getPolicyGetResponse());
            }),
        },
        muxGateway: {
            getAccountStatus: t
                ...
                const creds = (0, providerRequirements_1.resolveProviderCredentials)("mux-gateway", {
                ...
                if (!creds.isConfigured || !creds.couponCode) {
                    return (0, result_1.Err)("Mux Gateway is not logged in");
                }
                let response;
                try {
                    response = await fetch(`${muxGatewayOAuth_1.MUX_GATEWAY_ORIGIN}/api/v1/balance`, {
                        headers: {
                    ...
                    return (0, result_1.Err)(`Mux Gateway balance request failed: ${message}`);
                }
                if (response.status === 401) {
                    ...
                    return (0, result_1.Err)(muxGatewayOAuth_1.MUX_GATEWAY_SESSION_EXPIRED_MESSAGE);
                }
                if (!response.ok) {
                    let body = "";
                    try {
                    ...
                    return (0, result_1.Err)(`Mux Gateway balance request failed (HTTP ${response.status}): ${prefix || response.statusText}`);
                }
                let json;
                try {
                    ...
                    const message = error instanceof Error ? error.message : String(error);
                    return (0, result_1.Err)(`Mux Gateway balance response was not valid JSON: ${message}`);
                }
                const payload = json;
                ...
                if (typeof remaining !== "number" ||
                    ...
                    concurrency < 0) {
                    return (0, result_1.Err)("Mux Gateway returned an invalid balance payload");
                }
        ...
        secrets: {
            get: t
                ...
                .handler(({ context, input }) => {
                const projectPath = typeof input.projectPath === "string" && input.projectPath.trim().length > 0
                    ? input.projectPath
                    : undefined;
                return projectPath
        ...
        mcp: {
            ...
            test: t
                ...
                const secrets = (0, secrets_1.secretsToRecord)(projectPathProvided
                    ? context.config.getEffectiveSecrets(resolvedProjectPath)
                    : context.config.getGlobalSecrets());
                const configuredTransport = input.name
                    ? (await context.mcpConfigService.listServers(projectPathProvided ? resolvedProjectPath : undefined))[input.name]?.transport
                    : undefined;
            ...
            setToolAllowlist: t
                ...
                const result = await context.mcpConfigService.setToolAllowlist(input.name, input.toolAllowlist);
                if (result.success && server) {
                    const hasHeaders = server.transport !== "stdio" &&
                        Boolean(server.headers && Object.keys(server.headers).length > 0);
                    const usesSecretHeaders = server.transport !== "stdio" &&
        ...
        mcpOauth: {
            ...
            startServerFlow: t
                ...
                if (!host) {
                    return (0, result_1.Err)("Missing Host header");
                }
        ...
        projects: {
            ...
            mcp: {
                ...
                test: t
                    ...
                    .handler(async ({ context, input }) => {
                    const start = Date.now();
                    const secrets = (0, secrets_1.secretsToRecord)(context.config.getEffectiveSecrets(input.projectPath));
                    const configuredTransport = input.name
        ...
        nameGeneration: {
            generate: t
                ...
                const result = await (0, workspaceTitleGenerator_1.generateWorkspaceIdentity)(input.message, input.candidates, context.aiService);
                if (!result.success) {
                    return result;
                }
                return {
        ...
        workspace: {
            ...
            replaceChatHistory: t
                ...
                .handler(async ({ context, input }) => {
                const result = await context.workspaceService.replaceHistory(input.workspaceId, input.summaryMessage, { mode: input.mode, deletePlanFile: input.deletePlanFile });
                if (!result.success) {
                    return { success: false, error: result.error };
                }
                return { success: true, data: undefined };
            ...
            getSubagentTranscript: t
                ...
                (0, strict_1.default)(taskId.length > 0, "workspace.getSubagentTranscript: taskId must be non-empty");
                const requestingWorkspaceIdTrimmed = input.workspaceId?.trim();
                const requestingWorkspaceId = requestingWorkspaceIdTrimmed && requestingWorkspaceIdTrimmed.length > 0
                ...
                if (requestingWorkspaceId) {
                    try {
                        isDescendant = await context.taskService.isDescendantAgentTask(requestingWorkspaceId, taskId);
                    }
                    catch (error) {
                        log_1.log.warn("workspace.getSubagentTranscript: descendant check failed", {
                ...
                if (!resolved) {
                    if (requestingWorkspaceId && isDescendant) {
                        ...
                        const model = metaResult.success &&
                        ...
                        const thinkingLevel = metaResult.success
                            ? (0, thinking_1.coerceThinkingLevel)(metaResult.data.taskThinkingLevel)
                            : undefined;
                        return { messages, model, thinkingLevel };
                    }
                    // Helpful error message for UI.
                    throw new Error(requestingWorkspaceId
                ...
                const model = typeof resolved.entry.model === "string" && resolved.entry.model.trim().length > 0
                    ? resolved.entry.model.trim()
                    : undefined;
                const thinkingLevel = (0, thinking_1.coerceThinkingLevel)(resolved.entry.thinkingLevel);
                return { messages, model, thinkingLevel };
            }),
            executeBash: t
            ...
            onChat: t
                ...
                // by replay.
                const replayRelay = (0, replayBufferedStreamMessageRelay_1.createReplayBufferedStreamMessageRelay)(push);
            ...
            mcp: {
                get: t
                    ...
                    const mcpDisabledByPolicy = context.policyService.isEnforced() &&
        ...
        modelPresets: {
            ...
            save: t
                ...
                .handler(({ context, input }) => context.modelPresetsService.savePreset(input.name, input.models, input.description)),
            get: t
            ...
            update: t
                ...
                .handler(({ context, input }) => context.modelPresetsService.updatePreset(input.id, {
                name: input.name,
        ...
        modelHealth: {
            checkModel: t
                ...
                .handler(({ context, input }) => context.modelHealthService.checkModel(input.provider, input.modelId, input.metadata)),
            checkAll: t
