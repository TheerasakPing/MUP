...
function hasDurableCompactionBoundary(metadata) {
...
function getCompactionMetadataToPreserve(workspaceId, existingMessage, incomingMessage) {
    ...
    if (existingMessage.role !== "assistant") {
        ...
        log_1.log.warn("Skipping malformed persisted compaction boundary during history update", {
            workspaceId,
    ...
    if ((0, message_1.isCompactionSummaryMetadata)(existingMetadata.muxMetadata) &&
        !(0, message_1.isCompactionSummaryMetadata)(incomingMessage.metadata?.muxMetadata)) {
...
class HistoryService {
    ...
    static REVERSE_READ_CHUNK_SIZE = 256 * 1024;
    /** String-search needle for compaction boundary lines. */
    static BOUNDARY_NEEDLE = '"compactionBoundary":true';
    /**
     ...
     * the returned offset.
     */
    async findLastBoundaryByteOffset(workspaceId, skip = 0) {
        ...
        try {
            ...
            while (readEnd > 0) {
                ...
                const buffer = carryoverBytes.length > 0 ? Buffer.concat([rawChunk, carryoverBytes]) : rawChunk;
                ...
                for (let nl = newlinePositions.length - 1; nl >= 0; nl--) {
                    ...
                    const lineEnd = nl < newlinePositions.length - 1 ? newlinePositions[nl + 1] : buffer.length;
                    ...
                    if (line.includes(HistoryService.BOUNDARY_NEEDLE)) {
                        try {
                            ...
                            if ((0, compactionBoundary_1.isDurableCompactionBoundaryMarker)(msg)) {
                                if (skipped < skip) {
                                    skipped++;
                                }
                                else {
     ...
     */
    async readHistoryFromOffset(workspaceId, byteOffset) {
        const filePath = this.getChatHistoryPath(workspaceId);
        const stat = await fs.stat(filePath);
        ...
        try {
            ...
            for (const line of lines) {
                try {
                    messages.push((0, legacy_1.normalizeLegacyMuxMetadata)(JSON.parse(line)));
                }
                catch {
     ...
     */
    async readLastMessages(workspaceId, n) {
        const filePath = this.getChatHistoryPath(workspaceId);
        let fileSize;
        try {
            const stat = await fs.stat(filePath);
            ...
            while (readEnd > 0 && collected.length < n) {
                ...
                for (let nl = newlinePositions.length - 1; nl >= 0 && collected.length < n; nl--) {
                    ...
                    if (line.length === 0)
                        continue;
                    try {
                        collected.push((0, legacy_1.normalizeLegacyMuxMetadata)(JSON.parse(line)));
                    }
                    catch {
    ...
    async readChatHistory(workspaceId) {
        try {
            ...
            if (data.length > 5 * 1024 * 1024) {
                log_1.log.warn("chat.jsonl exceeds 5MB — full read may be slow, consider compaction", {
                    workspaceId,
            ...
            for (let i = 0; i < lines.length; i++) {
                try {
                    ...
                    messages.push((0, legacy_1.normalizeLegacyMuxMetadata)(message));
                }
                catch (parseError) {
     ...
     */
    async iterateForward(workspaceId, visitor) {
        const filePath = this.getChatHistoryPath(workspaceId);
        let fileSize;
        try {
            const stat = await fs.stat(filePath);
        ...
        try {
            ...
            while (readPos < fileSize) {
                ...
                readPos += toRead;
                const buffer = carryoverBytes.length > 0 ? Buffer.concat([carryoverBytes, rawChunk]) : rawChunk;
                ...
                for (const line of completeText.split("\n")) {
                    ...
                    if (trimmed.length === 0)
                        continue;
                    try {
                        messages.push((0, legacy_1.normalizeLegacyMuxMetadata)(JSON.parse(trimmed)));
                    }
                    catch {
                        // Skip malformed lines — same self-healing behavior as readChatHistory
                    }
            ...
            if (carryoverBytes.length > 0) {
                ...
                if (line.length > 0) {
                    try {
                        const msg = (0, legacy_1.normalizeLegacyMuxMetadata)(JSON.parse(line));
                        await visitor([msg]);
                    }
                    catch {
                        // Skip malformed line
     ...
     */
    async iterateBackward(workspaceId, visitor) {
        const filePath = this.getChatHistoryPath(workspaceId);
        let fileSize;
        try {
            const stat = await fs.stat(filePath);
     ...
     */
    async iterateFullHistory(workspaceId, direction, visitor) {
        try {
            ...
            return (0, result_1.Err)(`Failed to iterate history: ${message}`);
        }
    }
    /**
     ...
     */
    async getHistoryFromLatestBoundary(workspaceId, skip = 0) {
        try {
            ...
            const message = error instanceof Error ? error.message : String(error);
            return (0, result_1.Err)(`Failed to read history from boundary: ${message}`);
        }
    }
    /**
     ...
     */
    async getLastMessages(workspaceId, n) {
        try {
            ...
            const message = error instanceof Error ? error.message : String(error);
            return (0, result_1.Err)(`Failed to read last ${n} messages: ${message}`);
        }
    }
    /**
    ...
    async getNextHistorySequence(workspaceId) {
        ...
        if (lastResult.success && lastResult.data.length > 0) {
            ...
            if (scanResult.success) {
                ...
                (0, node_assert_1.default)(isNonNegativeInteger(nextSeqNum), "next history sequence counter must be a non-negative integer");
                this.sequenceCounters.set(workspaceId, nextSeqNum);
                return nextSeqNum;
            }
        }
    ...
    async _appendToHistoryUnlocked(workspaceId, message) {
        try {
            ...
            if (!message.metadata) {
                ...
                (0, node_assert_1.default)(isNonNegativeInteger(nextSeqNum), "getNextHistorySequence must return a non-negative integer");
                message.metadata = {
                ...
                if (existingSeqNum !== undefined) {
                    (0, node_assert_1.default)(isNonNegativeInteger(existingSeqNum), "appendToHistory requires historySequence to be a non-negative integer when provided");
                    // Already has history sequence, update counter if needed
                    const currentCounter = this.sequenceCounters.get(workspaceId) ?? 0;
                    (0, node_assert_1.default)(isNonNegativeInteger(currentCounter), "history sequence counter must remain a non-negative integer");
    ...
    async updateHistory(workspaceId, message) {
        return this.fileLocks.withLock(workspaceId, async () => {
            try {
                ...
                (0, node_assert_1.default)(isNonNegativeInteger(targetSequence), "updateHistory requires historySequence to be a non-negative integer");
                ...
                for (let i = 0; i < messages.length; i++) {
                    if (messages[i].metadata?.historySequence === targetSequence) {
                        ...
                        (0, node_assert_1.default)(existingMessage, "updateHistory matched message must exist");
                        ...
                        const preservedCompactionMetadata = getCompactionMetadataToPreserve(workspaceId, existingMessage, message);
        ...
        return this.fileLocks.withLock(workspaceId, async () => {
            try {
                ...
                const maxSeq = filteredMessages.reduce((max, msg) => {
                    ...
                    if (!isNonNegativeInteger(seq)) {
                        log_1.log.warn("Ignoring malformed persisted historySequence while updating sequence counter after delete", {
                            workspaceId,
                            messageId: msg.id,
                            historySequence: seq,
                        });
                ...
                (0, node_assert_1.default)(isNonNegativeInteger(nextSeq), "next history sequence counter after delete must be a non-negative integer");
        ...
        return this.fileLocks.withLock(workspaceId, async () => {
            try {
                ...
                const maxTruncatedSeq = truncatedMessages.reduce((max, msg) => {
                    ...
                    if (!isNonNegativeInteger(seq)) {
                        log_1.log.warn("Ignoring malformed persisted historySequence while updating sequence counter after truncation", {
                            workspaceId,
                            messageId: msg.id,
                            historySequence: seq,
                        });
                ...
                (0, node_assert_1.default)(isNonNegativeInteger(nextSeq), "next history sequence counter after truncation must be a non-negative integer");
                this.sequenceCounters.set(workspaceId, nextSeq);
                return (0, result_1.Ok)(undefined);
            }
            catch (error) {
            ...
            try {
                ...
                if (percentage >= 1.0) {
                    ...
                    const deletedSequences = messages
                        ...
                        .filter((s) => isNonNegativeInteger(s));
                    try {
                ...
                const maxRemainingSeq = remainingMessages.reduce((max, msg) => {
                    ...
                    if (!isNonNegativeInteger(seq)) {
                        log_1.log.warn("Ignoring malformed persisted historySequence while updating sequence counter after truncateHistory", {
                            workspaceId,
                            messageId: msg.id,
                            historySequence: seq,
                        });
                ...
                (0, node_assert_1.default)(isNonNegativeInteger(nextSeq), "next history sequence counter after truncateHistory must be a non-negative integer");
                this.sequenceCounters.set(workspaceId, nextSeq);
                return (0, result_1.Ok)(deletedSequences);
            }
            catch (error) {
