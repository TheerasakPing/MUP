...
class CoderService {
    ...
    async resolveCoderBinaryPath() {
        ...
        try {
            ...
            try {
                const proc = __addDisposableResource(env_1, (0, disposableExec_1.execAsync)("command -v coder", shell ? { shell } : undefined), false);
    ...
    async getCoderInfo() {
        ...
        try {
            ...
            try {
                const proc = __addDisposableResource(env_2, (0, disposableExec_1.execAsync)("coder version --output=json"), false);
                const { stdout } = await proc.result;
                // Parse JSON output
                ...
                try {
                    ...
                    const notLoggedInMessage = binaryPath
                        ...
                        : sanitizedLine;
                    log_1.log.debug("Failed to fetch Coder whoami data", { error });
    ...
    async createApiSession(tokenName) {
        ...
        try {
            const tokenProc = __addDisposableResource(env_3, (0, disposableExec_1.execAsync)(`coder tokens create --lifetime 5m --name ${streamUtils_1.shescape.quote(tokenName)}`), false);
            ...
            return {
                ...
                dispose: async () => {
                    try {
                        ...
                        try {
                            const deleteProc = __addDisposableResource(env_4, (0, disposableExec_1.execAsync)(`coder tokens delete ${streamUtils_1.shescape.quote(tokenName)}`), false);
                        ...
                        log_1.log.debug("Failed to delete temporary Coder API token", { tokenName });
                    }
                },
            };
        }
    ...
    async getWhoamiData(options) {
        ...
        try {
            ...
            const proc = __addDisposableResource(env_5, (0, disposableExec_1.execAsync)("coder whoami --output=json"), false);
            const { stdout } = await proc.result;
            const data = JSON.parse(stdout);
            if (!data[0]?.url) {
    ...
    async getActiveTemplateVersionId(templateName, org) {
        ...
        try {
            ...
            const proc = __addDisposableResource(env_6, (0, disposableExec_1.execAsync)("coder templates list --output=json"), false);
            const { stdout } = await proc.result;
            if (!stdout.trim()) {
    ...
    async getPresetParamNames(templateName, presetName, org) {
        try {
            ...
            try {
                ...
                const proc = __addDisposableResource(env_7, (0, disposableExec_1.execAsync)(`coder templates presets list ${streamUtils_1.shescape.quote(templateName)}${orgFlag} --output=json`), false);
                const { stdout } = await proc.result;
    ...
    async listTemplates() {
        try {
            ...
            try {
                const proc = __addDisposableResource(env_8, (0, disposableExec_1.execAsync)("coder templates list --output=json"), false);
                const { stdout } = await proc.result;
            ...
            log_1.log.warn("Failed to list Coder templates", { error });
    ...
    async listPresets(templateName, org) {
        try {
            ...
            try {
                const orgFlag = org ? ` --org ${streamUtils_1.shescape.quote(org)}` : "";
                const proc = __addDisposableResource(env_9, (0, disposableExec_1.execAsync)(`coder templates presets list ${streamUtils_1.shescape.quote(templateName)}${orgFlag} --output=json`), false);
                const { stdout } = await proc.result;
            ...
            log_1.log.warn("Failed to list Coder presets", { templateName, error });
            return { ok: false, error: message || "Unknown error" };
        }
    }
    /**
    ...
    async workspaceExists(workspaceName) {
        try {
            ...
            try {
                const proc = __addDisposableResource(env_10, (0, disposableExec_1.execAsync)(`coder list --search ${streamUtils_1.shescape.quote(`name:${workspaceName}`)} --output=json`), false);
                const { stdout } = await proc.result;
                if (!stdout.trim()) {
                    return false;
                }
    ...
    async listWorkspaces() {
        ...
        try {
            ...
            try {
                const proc = __addDisposableResource(env_11, (0, disposableExec_1.execAsync)("coder list --output=json"), false);
                const { stdout } = await proc.result;
            ...
            log_1.log.warn("Failed to list Coder workspaces", { error });
            return { ok: false, error: message || "Unknown error" };
        }
    }
    /**
    ...
    async startWorkspace(workspaceName, options) {
        ...
        try {
            ...
            const interpreted = interpretCoderResult(result);
            if (!interpreted.ok) {
                return (0, result_1.Err)(interpreted.error);
            }
    ...
    async deleteWorkspaceEventually(name, options) {
        ...
        if (!name.startsWith("mux-")) {
            log_1.log.warn("Refusing to delete Coder workspace without mux- prefix", { name });
            return (0, result_1.Ok)(undefined);
        }
        ...
        while (!isTimedOut()) {
            if (options?.signal?.aborted) {
                return (0, result_1.Err)("Delete operation aborted");
            }
            ...
            if (statusResult.kind === "ok") {
                ...
                if (unstableStates.has(statusResult.status)) {
                    log_1.log.debug("Coder workspace in transitional state; will retry delete", {
                        name,
        ...
        return (0, result_1.Err)(lastError ?? "Timed out deleting Coder workspace");
    }
    /**
