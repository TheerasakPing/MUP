...
async function isSshAgentModuleAvailable() {
    ...
    try {
        ...
        await Promise.resolve().then(() => __importStar(require("@coder/mux-md-client/ssh-agent")));
        sshAgentModuleAvailable = true;
    }
    catch {
        log_1.log.warn("[SigningService] @coder/mux-md-client/ssh-agent is not available — ssh-agent signing disabled. " +
...
function getDefaultKeyPaths() {
    return [
        (0, path_1.join)((0, paths_1.getMuxHome)(), "message_signing_key"), // Explicit mux key (any supported type, symlink-friendly)
...
class SigningService {
    ...
    async signMessage(content) {
        ...
        const sshAgentModule = await Promise.resolve().then(() => __importStar(require("@coder/mux-md-client/ssh-agent"))).catch((err) => {
            const message = err instanceof Error ? err.message : String(err);
            log_1.log.error("[SigningService] Failed to load ssh-agent signing module:", message);
