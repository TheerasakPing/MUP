...
const toolHookEnv_1 = require("../../common/utils/tools/toolHookEnv");
...
function getToolInputValueForEnv(context) {
    ...
    try {
        return JSON.parse(context.toolInput);
    }
    catch {
        return undefined;
    }
...
async function runWithHook(runtime, hookPath, context, executeTool, timingOptions) {
    ...
    if (context.toolInput.length > TOOL_INPUT_ENV_LIMIT) {
        ...
        try {
            ...
            toolInputPath = joinPathLike(tempDir, `mux-tool-input-${Date.now()}-${crypto.randomUUID()}.json`);
    ...
    const hookEnv = {
        ...
        ...(0, toolHookEnv_1.flattenToolHookValueToEnv)(toolInputValueForEnv, "MUX_TOOL_INPUT", {
...
async function runPostHook(runtime, hookPath, context, toolResult, options) {
    ...
    const resultPath = joinPathLike(context.runtimeTempDir ?? "/tmp", `mux-tool-result-${Date.now()}-${crypto.randomUUID()}.json`);
    let resultPathForEnv;
    ...
    const hookEnv = {
        ...
        ...(0, toolHookEnv_1.flattenToolHookValueToEnv)(toolResult, "MUX_TOOL_RESULT", {
            maxValueLength: TOOL_INPUT_ENV_LIMIT,
            maxVars: FLATTENED_TOOL_ENV_MAX_VARS,
            maxArrayLength: FLATTENED_TOOL_ENV_MAX_ARRAY_LENGTH,
        }),
    ...
    const cleanup = async () => {
        ...
        if (!resultPathForEnv)
            return;
        try {
            await (0, helpers_1.execBuffered)(runtime, `rm -f ${shellEscape(resultPathForEnv)}`, {
                cwd: context.projectDir,
                timeout: 5,
            });
        }
