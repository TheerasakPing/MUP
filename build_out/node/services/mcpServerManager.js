...
function extractWwwAuthenticateHeader(error) {
    if (!isPlainObject(error)) {
        return null;
    }
    const direct = extractHeaderValue(error.responseHeaders, "www-authenticate") ??
    ...
    if (isPlainObject(data)) {
        const fromData = extractHeaderValue(data.responseHeaders, "www-authenticate") ??
...
async function probeWwwAuthenticateHeader(url) {
    ...
    const testPromise = (async () => {
        ...
        try {
            if (server.transport === "stdio") {
                ...
                client = await (0, mcp_1.createMCPClient)({ transport: stdioTransport });
            }
            else {
                ...
                const tryHttp = async () => (0, mcp_1.createMCPClient)({
                    transport: {
                        type: "http",
                        ...transportBase,
                    },
                });
                const trySse = async () => (0, mcp_1.createMCPClient)({
                    transport: {
                        type: "sse",
                        ...transportBase,
                    },
...
class MCPServerManager {
    ...
    setMcpOauthService(service) {
        this.mcpOauthService = service;
    }
    constructor(configService, options, policyService) {
    ...
    async getToolsForWorkspace(options) {
        ...
        const enabledServers = this.filterServersByPolicy(this.applyServerOverrides(fullServerInfo, overrides));
        ...
        for (const [name, info] of enabledEntries) {
            ...
            if (this.mcpOauthService) {
                try {
                    ...
                    log_1.log.debug("[MCP] Failed to resolve MCP OAuth status", { name, error });
                }
            }
            try {
        ...
        if (existing && leaseCount > 0) {
            ...
            if (hasClosedInstance) {
                ...
                const restartedInstances = await this.startServers(serversToRestart, runtime, projectPath, workspacePath, projectSecrets, () => this.markActivity(workspaceId));
        ...
        const instances = await this.startServers(enabledServers, runtime, projectPath, workspacePath, projectSecrets, () => this.markActivity(workspaceId));
    ...
    async test(options) {
        ...
        if (trimmedName && !command?.trim() && !url?.trim()) {
            ...
            if (server.transport === "stdio") {
                return runServerTest({ transport: "stdio", command: server.command }, projectPath, `server "${trimmedName}"`);
            }
            try {
                ...
                }, projectPath, `server "${trimmedName}"`);
            }
            catch (error) {
                const message = error instanceof Error ? error.message : String(error);
        ...
        if (url?.trim()) {
            ...
            try {
                ...
                return runServerTest({
                    ...
                    headers: resolved.headers,
                    ...(authProvider ? { authProvider } : {}),
                }, projectPath, trimmedName ? `server "${trimmedName}" (url)` : "url");
            }
            catch (error) {
                const message = error instanceof Error ? error.message : String(error);
                return { success: false, error: message };
    ...
    async startServers(servers, runtime, projectPath, workspacePath, projectSecrets, onActivity) {
        const result = new Map();
        const entries = Object.entries(servers);
        for (const [name, info] of entries) {
            try {
                const instance = await this.startSingleServer(name, info, runtime, projectPath, workspacePath, projectSecrets, onActivity);
                if (instance) {
    ...
    async startSingleServer(name, info, runtime, _projectPath, workspacePath, projectSecrets, onActivity) {
        if (info.transport === "stdio") {
            ...
            const client = await (0, mcp_1.createMCPClient)({ transport });
