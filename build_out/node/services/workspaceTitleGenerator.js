...
exports.extractIdentityFromText = extractIdentityFromText;
...
async function generateWorkspaceIdentity(message, candidates, aiService) {
    if (candidates.length === 0) {
        return (0, result_1.Err)({ type: "unknown", raw: "No model candidates provided for name generation" });
    }
    ...
    for (let i = 0; i < maxAttempts; i++) {
        ...
        if (!modelResult.success) {
            ...
            log_1.log.debug(`Name generation: skipping ${modelString} (${modelResult.error.type})`);
            continue;
        }
        try {
            ...
            const stream = (0, ai_1.streamText)({
                ...
                output: ai_1.Output.object({ schema: workspaceIdentitySchema }),
...
- title: A 2-5 word description in verb-noun format. Examples: "Fix plan mode", "Add user authentication", "Refactor sidebar layout"`,
            ...
            if (ai_1.NoObjectGeneratedError.isInstance(error) && error.text) {
                ...
                if (textFallback) {
                    log_1.log.info(`Name generation: structured output failed for ${modelString}, recovered from text fallback`);
                    const suffix = generateNameSuffix();
            ...
            log_1.log.warn(`Name generation failed with ${modelString}, trying next candidate`, {
                error: lastApiError,
            });
            continue;
        }
