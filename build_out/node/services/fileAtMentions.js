...
function renderMuxFileBlock(options) {
    ...
    return (`<mux-file path="${options.filePath}" range="${options.rangeLabel}"${truncatedAttr}>\n` +
        `${fence}\n` +
        `${options.content}\n` +
        `\`\`\`\n` +
        `</mux-file>`);
...
async function injectFileAtMentions(messages, options) {
    ...
    for (let i = 0; i < messages.length; i++) {
        ...
        if (blocks && blocks.length > 0) {
            result.push((0, message_1.createMuxMessage)((0, messageIds_1.createFileAtMentionMessageId)(createdAt, i), "user", blocks.join("\n\n"), {
                timestamp: createdAt,
                synthetic: true,
            }));
        }
