...
`, "utf-8");
    ...
    (0, bun_test_1.test)("resolveAgentFrontmatter inherits omitted fields from base chain (same-name override)", async () => {
        const env_6 = { stack: [], error: void 0, hasError: false };
        try {
            const project = __addDisposableResource(env_6, new tempDir_1.DisposableTempDir("agent-frontmatter-project"), false);
            const global = __addDisposableResource(env_6, new tempDir_1.DisposableTempDir("agent-frontmatter-global"), false);
            const projectAgentsRoot = path.join(project.path, ".mux", "agents");
            const globalAgentsRoot = global.path;
            await fs.mkdir(projectAgentsRoot, { recursive: true });
            await fs.mkdir(globalAgentsRoot, { recursive: true });
...
`, "utf-8");
            const roots = { projectRoot: projectAgentsRoot, globalRoot: globalAgentsRoot };
            const runtime = new LocalRuntime_1.LocalRuntime(project.path);
            const frontmatter = await (0, agentDefinitionsService_1.resolveAgentFrontmatter)(runtime, project.path, "foo", { roots });
            (0, bun_test_1.expect)(frontmatter.description).toBe("Base description");
            (0, bun_test_1.expect)(frontmatter.ui?.hidden).toBe(true);
            (0, bun_test_1.expect)(frontmatter.ui?.color).toBe("blue");
            (0, bun_test_1.expect)(frontmatter.ui?.requires).toEqual(["plan"]);
            (0, bun_test_1.expect)(frontmatter.subagent?.runnable).toBe(true);
            (0, bun_test_1.expect)(frontmatter.subagent?.append_prompt).toBe("Base subagent prompt");
            (0, bun_test_1.expect)(frontmatter.subagent?.skip_init_hook).toBe(true);
            (0, bun_test_1.expect)(frontmatter.ai?.model).toBe("base-model");
            (0, bun_test_1.expect)(frontmatter.ai?.thinkingLevel).toBe("high");
            (0, bun_test_1.expect)(frontmatter.tools?.add).toEqual(["baseAdd"]);
            (0, bun_test_1.expect)(frontmatter.tools?.remove).toEqual(["baseRemove"]);
        }
        catch (e_6) {
            env_6.error = e_6;
            env_6.hasError = true;
    ...
    (0, bun_test_1.test)("resolveAgentFrontmatter preserves explicit falsy overrides", async () => {
        const env_7 = { stack: [], error: void 0, hasError: false };
        try {
            const tempDir = __addDisposableResource(env_7, new tempDir_1.DisposableTempDir("agent-frontmatter-falsy"), false);
            const agentsRoot = path.join(tempDir.path, ".mux", "agents");
            await fs.mkdir(agentsRoot, { recursive: true });
            await fs.writeFile(path.join(agentsRoot, "base.md"), `---
name: Base
...
`, "utf-8");
            const roots = { projectRoot: agentsRoot, globalRoot: agentsRoot };
            const runtime = new LocalRuntime_1.LocalRuntime(tempDir.path);
            const frontmatter = await (0, agentDefinitionsService_1.resolveAgentFrontmatter)(runtime, tempDir.path, "child", { roots });
            (0, bun_test_1.expect)(frontmatter.ui?.hidden).toBe(false);
            (0, bun_test_1.expect)(frontmatter.subagent?.runnable).toBe(false);
            (0, bun_test_1.expect)(frontmatter.subagent?.skip_init_hook).toBe(false);
        }
        catch (e_7) {
            env_7.error = e_7;
            env_7.hasError = true;
    ...
    (0, bun_test_1.test)("resolveAgentFrontmatter concatenates tools.add/tools.remove (base first)", async () => {
        const env_8 = { stack: [], error: void 0, hasError: false };
        try {
            const tempDir = __addDisposableResource(env_8, new tempDir_1.DisposableTempDir("agent-frontmatter-tools"), false);
            const agentsRoot = path.join(tempDir.path, ".mux", "agents");
            await fs.mkdir(agentsRoot, { recursive: true });
            await fs.writeFile(path.join(agentsRoot, "base.md"), `---
name: Base
...
`, "utf-8");
            ...
            const runtime = new LocalRuntime_1.LocalRuntime(tempDir.path);
            const frontmatter = await (0, agentDefinitionsService_1.resolveAgentFrontmatter)(runtime, tempDir.path, "child", { roots });
            (0, bun_test_1.expect)(frontmatter.tools?.add).toEqual(["a", "c"]);
            (0, bun_test_1.expect)(frontmatter.tools?.remove).toEqual(["b", "d"]);
        }
        catch (e_8) {
            env_8.error = e_8;
            env_8.hasError = true;
    ...
    (0, bun_test_1.test)("resolveAgentFrontmatter detects cycles", async () => {
        const env_9 = { stack: [], error: void 0, hasError: false };
        try {
            const tempDir = __addDisposableResource(env_9, new tempDir_1.DisposableTempDir("agent-frontmatter-cycle"), false);
            const agentsRoot = path.join(tempDir.path, ".mux", "agents");
            await fs.mkdir(agentsRoot, { recursive: true });
...
`, "utf-8");
            const roots = { projectRoot: agentsRoot, globalRoot: agentsRoot };
            const runtime = new LocalRuntime_1.LocalRuntime(tempDir.path);
            (0, bun_test_1.expect)((0, agentDefinitionsService_1.resolveAgentFrontmatter)(runtime, tempDir.path, "a", { roots })).rejects.toThrow("Circular agent inheritance detected");
        }
        catch (e_9) {
            env_9.error = e_9;
            env_9.hasError = true;
