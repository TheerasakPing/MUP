...
exports.resolveAgentFrontmatter = resolveAgentFrontmatter;
...
function deepMergeAgentFrontmatter(base, overlay, path) {
...
async function resolveAgentFrontmatter(runtime, workspacePath, agentId, options) {
    ...
    async function resolve(id, depth, skipScopesAbove) {
        ...
        const baseFrontmatter = await resolve(baseId, depth + 1, mergeSkipScopesAbove(skipScopesAbove, computeBaseSkipScope(baseId, id, pkg.scope)));
        const mergedRaw = deepMergeAgentFrontmatter(baseFrontmatter, pkg.frontmatter, []);
        const merged = schemas_1.AgentDefinitionFrontmatterSchema.safeParse(mergedRaw);
        if (!merged.success) {
            throw new Error(`Invalid merged frontmatter for '${id}': ${formatZodIssues(merged.error.issues)}`);
        }
        return merged.data;
    }
    return resolve(agentId, 0, options?.skipScopesAbove);
