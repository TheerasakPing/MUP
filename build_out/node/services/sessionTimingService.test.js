...
(0, bun_test_1.describe)("SessionTimingService", () => {
    ...
    (0, bun_test_1.it)("uses agentId for the per-model breakdown when available", async () => {
        const telemetry = createMockTelemetryService();
        const service = new sessionTimingService_1.SessionTimingService(config, telemetry);
        service.setStatsTabState({ enabled: true, variant: "stats", override: "default" });
        const workspaceId = "test-workspace";
        ...
        (0, bun_test_1.expect)(snapshot.session?.byModel[key]).toBeDefined();
        (0, bun_test_1.expect)(snapshot.session?.byModel[key]?.agentId).toBe("explore");
        (0, bun_test_1.expect)(snapshot.session?.byModel[key]?.mode).toBe("exec");
        // Regression: splitting should not label explore traffic as plain exec.
        (0, bun_test_1.expect)(snapshot.session?.byModel[`${normalizedModel}:exec`]).toBeUndefined();
    });
    ...
    (0, bun_test_1.it)("throttles delta-driven change events per workspace", async () => {
        const telemetry = createMockTelemetryService();
        const service = new sessionTimingService_1.SessionTimingService(config, telemetry);
        service.setStatsTabState({ enabled: true, variant: "stats", override: "default" });
        const workspaceId = "test-workspace";
        ...
        const onChange = (0, bun_test_1.mock)(() => undefined);
        ...
        try {
            ...
            (0, bun_test_1.expect)(onChange).toHaveBeenCalledTimes(1);
            ...
            (0, bun_test_1.expect)(onChange).toHaveBeenCalledTimes(2);
            ...
            (0, bun_test_1.expect)(onChange).toHaveBeenCalledTimes(2);
            await sleep(250);
            (0, bun_test_1.expect)(onChange).toHaveBeenCalledTimes(3);
    ...
    (0, bun_test_1.it)("clears scheduled delta emits when the last subscriber disconnects", async () => {
        const telemetry = createMockTelemetryService();
        const service = new sessionTimingService_1.SessionTimingService(config, telemetry);
        service.setStatsTabState({ enabled: true, variant: "stats", override: "default" });
        const workspaceId = "test-workspace";
        ...
        try {
            ...
            const deltaEmitState = service.deltaEmitState;
            (0, bun_test_1.expect)(deltaEmitState.get(workspaceId)?.timer).toBeDefined();
            // Unsubscribe before the throttle window elapses; timer should be cleared.
            service.removeSubscriber(workspaceId);
            (0, bun_test_1.expect)(deltaEmitState.has(workspaceId)).toBe(false);
            await sleep(250);
            (0, bun_test_1.expect)(onChange).toHaveBeenCalledTimes(2);
        }
        finally {
