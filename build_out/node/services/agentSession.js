...
class AgentSession {
    ...
    execSubagentHardRestartAttempts = new Set();
    ...
    constructor(options) {
        ...
        const { workspaceId, config, historyService, partialService, aiService, initStateManager, costTrackingService, telemetryService, backgroundProcessManager, keepBackgroundProcesses, onCompactionComplete, onPostCompactionStateChange, } = options;
    ...
    async sendMessage(message, options, internal) {
        ...
        const typedMuxMetadata = options?.muxMetadata;
        ...
        const userMessage = (0, message_1.createMuxMessage)(messageId, "user", message, {
            timestamp: Date.now(),
        ...
        try {
            skillSnapshotResult = await this.materializeAgentSkillSnapshot(typedMuxMetadata, options?.disableWorkspaceAgents);
        }
        catch (error) {
    ...
    normalizeGatewaySendOptions(options) {
        // Keep persisted model IDs canonical; gateway routing is now backend-authoritative (issue #1769).
        const normalizedModel = (0, models_1.normalizeGatewayModel)(options.model.trim());
        const system1Model = options.system1Model?.trim();
        const normalizedSystem1Model = system1Model && system1Model.length > 0 ? (0, models_1.normalizeGatewayModel)(system1Model) : undefined;
        return {
            ...options,
    ...
    async streamWithHistory(modelString, options, openaiTruncationModeOverride, disablePostCompactionAttachments) {
        ...
        if (lastMsg?.role === "assistant" && !lastMsg.metadata?.partial) {
            log_1.log.warn("streamWithHistory: trailing non-partial assistant detected, injecting [CONTINUE]", {
                workspaceId: this.workspaceId,
                messageId: lastMsg.id,
            });
            const sentinelMessage = (0, message_1.createMuxMessage)((0, messageIds_1.createUserMessageId)(), "user", "[CONTINUE]", {
                timestamp: Date.now(),
                synthetic: true,
            });
        ...
        const streamResult = await this.aiService.streamMessage({
            ...
            recordFileState,
            changedFileAttachments: changedFileAttachments.length > 0 ? changedFileAttachments : undefined,
    ...
    supports1MContextRetry(modelString) {
    ...
    async maybeRetryCompactionOnContextExceeded(data) {
        ...
        if (is1MCapable) {
            ...
            const already1M = anthropicOpts?.use1MContext === true ||
    ...
    async maybeHardRestartExecSubagentOnContextExceeded(data) {
        if (data.errorType !== "context_exceeded") {
            return false;
        }
        ...
        const agentIdRaw = (metadata.agentId ?? metadata.agentType ?? workspaceDefaults_1.WORKSPACE_DEFAULTS.agentId)
            .trim()
            .toLowerCase();
        const parsedAgentId = schemas_1.AgentIdSchema.safeParse(agentIdRaw);
        const agentId = parsedAgentId.success ? parsedAgentId.data : "exec";
        ...
        try {
            const parentMetadataResult = await this.aiService.getWorkspaceMetadata(metadata.parentWorkspaceId);
        ...
        for (const agentMetadata of metadataCandidates) {
            try {
                const runtime = (0, runtimeHelpers_1.createRuntimeForWorkspace)(agentMetadata);
                ...
                const agentDiscoveryPath = context.options?.disableWorkspaceAgents === true
                    ? agentMetadata.projectPath
                    : workspacePath;
                const agentDefinition = await (0, agentDefinitionsService_1.readAgentDefinition)(runtime, agentDiscoveryPath, agentId);
                chain = await (0, resolveAgentInheritanceChain_1.resolveAgentInheritanceChain)({
                    runtime,
        ...
        if (!(0, agentTools_1.isExecLikeEditingCapableInResolvedChain)(chain)) {
            return false;
        }
        this.execSubagentHardRestartAttempts.add(requestId);
        const continuationNotice = "Context limit reached. Mux restarted this agent's chat history and will replay your original prompt below. " +
            "Continue using only the current workspace state (files, git history, command output); " +
            "re-inspect the repo as needed.";
        log_1.log.info("Exec-like subagent hit context limit; hard-restarting history and retrying", {
            workspaceId: this.workspaceId,
            requestId,
            model: context.modelString,
            agentId,
        ...
        const messages = historyResult.data;
        const firstPromptIndex = messages.findIndex((msg) => msg.role === "user" && msg.metadata?.synthetic !== true);
        ...
        for (let i = firstPromptIndex - 1; i >= 0; i -= 1) {
            const msg = messages[i];
            const isSnapshot = msg.role === "user" &&
        ...
        try {
            ...
            log_1.log.warn("Failed to discard pending post-compaction state before hard restart", {
                workspaceId: this.workspaceId,
                error: error instanceof Error ? error.message : String(error),
            });
        }
        ...
        if (!partialDeleteResult.success) {
            log_1.log.warn("Failed to delete partial before exec subagent hard restart", {
                workspaceId: this.workspaceId,
        ...
        if (!clearResult.success) {
            log_1.log.warn("Failed to clear history for exec subagent hard restart", {
                workspaceId: this.workspaceId,
        ...
        const continuationMessage = (0, message_1.createMuxMessage)((0, messageIds_1.createUserMessageId)(), "user", continuationNotice, {
            timestamp: Date.now(),
            synthetic: true,
            uiVisible: true,
        });
        ...
        for (const message of messagesToAppend) {
            ...
            if (!appendResult.success) {
                log_1.log.error("Failed to append message during exec subagent hard restart", {
                    workspaceId: this.workspaceId,
        ...
        let retryResult;
        try {
            retryResult = await this.streamWithHistory(context.modelString, retryOptions, context.openaiTruncationModeOverride);
        }
        finally {
            this.streamStarting = false;
        }
        if (!retryResult.success) {
            log_1.log.error("Exec subagent hard restart retry failed to start", {
                workspaceId: this.workspaceId,
                error: retryResult.error,
            });
            return false;
    ...
    async handleStreamError(data) {
        ...
        if (await this.maybeHardRestartExecSubagentOnContextExceeded({
            messageId: data.messageId,
            errorType: data.errorType,
        })) {
            return;
    ...
    async dispatchPendingFollowUp() {
        ...
        if (!(0, message_1.isCompactionSummaryMetadata)(muxMeta) || !muxMeta.pendingFollowUp) {
            return;
        }
        ...
        const effectiveModel = followUp.model ?? knownModels_1.DEFAULT_MODEL;
        log_1.log.debug("Dispatching pending follow-up from compaction summary", {
            workspaceId: this.workspaceId,
    ...
    async materializeFileAtMentionsSnapshot(messageText) {
        ...
        const snapshotMessage = (0, message_1.createMuxMessage)(snapshotId, "user", blocks, {
            timestamp: Date.now(),
            synthetic: true,
    ...
    async materializeAgentSkillSnapshot(muxMetadata, disableWorkspaceAgents) {
        ...
        const parsedName = schemas_1.SkillNameSchema.safeParse(muxMetadata.skillName);
        ...
        const frontmatterYaml = yaml_1.default.stringify(skill.frontmatter).trimEnd();
        ...
        const snapshotMessage = (0, message_1.createMuxMessage)(snapshotId, "user", snapshotText, {
            timestamp: Date.now(),
            synthetic: true,
            agentSkillSnapshot: {
