...
exports.stripEncryptedContent = stripEncryptedContent;
...
const usageHelpers_1 = require("../../common/utils/tokens/usageHelpers");
...
const StreamingTokenTracker_1 = require("../../node/utils/main/StreamingTokenTracker");
const cacheStrategy_1 = require("../../common/utils/ai/cacheStrategy");
const displayUsage_1 = require("../../common/utils/tokens/displayUsage");
const extractToolMediaAsUserMessagesFromModelMessages_1 = require("../../node/utils/messages/extractToolMediaAsUserMessagesFromModelMessages");
const models_1 = require("../../common/utils/ai/models");
const muxGatewayOAuth_1 = require("../../common/constants/muxGatewayOAuth");
const modelStats_1 = require("../../common/utils/tokens/modelStats");
 ...
 */
function stripEncryptedContentFromArray(output) {
...
function summarizeToolResultForLog(output) {
    if (output === null) {
    ...
    if (Array.isArray(output)) {
        return {
            outputType: "array",
            outputLength: output.length,
            firstItemType: output.length > 0 && output[0] !== null
                ? typeof output[0]
                : output.length > 0
                    ? "null"
                    : undefined,
...
function markProviderMetadataCostsIncluded(providerMetadata, costsIncluded) {
    if (!costsIncluded)
        return providerMetadata;
    const muxMetadata = providerMetadata?.mux;
    const existingMux = muxMetadata && typeof muxMetadata === "object"
        ? muxMetadata
        : undefined;
    return {
        ...(providerMetadata ?? {}),
...
function nextPartTimestamp(streamInfo) {
    const now = Date.now();
...
class StreamManager extends events_1.EventEmitter {
    ...
    tokenTracker = new StreamingTokenTracker_1.StreamingTokenTracker();
    ...
    lostResponseIds = new Set();
    constructor(historyService, partialService, sessionUsageService, costTrackingService) {
        super();
    ...
    getWorkspaceLogger(workspaceId, streamInfo) {
        const fields = { workspaceId };
        if (streamInfo?.workspaceName) {
            fields.workspaceName = streamInfo.workspaceName;
        }
        return log_1.log.withFields(fields);
    }
    ...
    async awaitPendingPartialWrite(streamInfo) {
    ...
    async flushPartialWrite(workspaceId, streamInfo) {
        ...
        streamInfo.partialWritePromise = (async () => {
            try {
                const canonicalModel = (0, models_1.normalizeGatewayModel)(streamInfo.model);
                const routedThroughGateway = streamInfo.initialMetadata?.routedThroughGateway ??
                ...
                const partialMessage = {
                    ...
                    metadata: {
                        ...
                        ...(streamInfo.thinkingLevel && {
                            thinkingLevel: streamInfo.thinkingLevel,
                        }),
     ...
     */
    async createTempDirForStream(streamToken, runtime) {
    ...
    cleanupStreamTempDir(runtime, runtimeTempDir) {
        // Use parent directory as cwd for safety - if runtimeTempDir is malformed,
        // we won't accidentally run rm -rf from root.
        const tempDirBasename = paths_1.PlatformPaths.basename(runtimeTempDir);
        ...
        void runtime
            ...
            log_1.log.debug(`Cleaned up temp dir: ${runtimeTempDir}`);
        })
            .catch((error) => {
            log_1.log.error(`Failed to cleanup temp dir ${runtimeTempDir}:`, error);
        });
    }
    /**
     ...
     */
    async emitToolCallDeltaIfPresent(workspaceId, streamInfo, part) {
        const maybeDelta = part;
        if (maybeDelta?.type !== "tool-call-delta") {
            return false;
        }
        const toolDelta = part;
    ...
    async getStreamMetadata(streamInfo, timeoutMs = 1000) {
    ...
    resolveTotalUsageForStreamEnd(streamInfo, totalUsage) {
        ...
        const hasCumulativeUsage = (cumulativeUsage.inputTokens ?? 0) > 0 ||
     ...
     */
    async getAggregatedProviderMetadata(streamInfo, timeoutMs = 1000) {
        try {
            ...
            return {
                ...lastStepMetadata,
                anthropic: {
                    ...lastStepMetadata?.anthropic,
                    cacheCreationInputTokens: totalCacheCreationTokens,
                },
            };
        }
        catch (error) {
            log_1.log.debug("Could not aggregate provider metadata:", error);
            return undefined;
        }
    }
    /**
     ...
     */
    async emitPartAsEvent(workspaceId, messageId, part, options) {
    ...
    async appendPartAndEmit(workspaceId, streamInfo, part, schedulePartialWrite = false) {
    ...
    async cancelStreamSafely(workspaceId, streamInfo, abortReason, abandonPartial) {
        ...
        try {
            ...
            log_1.log.error("Error during stream cancellation:", error);
    ...
    async checkSoftCancelStream(workspaceId, streamInfo) {
        if (!streamInfo.softInterrupt.pending)
            return;
        try {
            streamInfo.state = StreamState.STOPPING;
            // Flush any pending partial write immediately (preserves work on interruption)
    ...
    async cleanupAbortedStream(workspaceId, streamInfo, abortReason, abandonPartial) {
        ...
        const providerMetadata = markProviderMetadataCostsIncluded(streamInfo.cumulativeProviderMetadata, streamInfo.initialMetadata?.costsIncluded);
        ...
        await this.recordSessionUsage(workspaceId, streamInfo.model, usage, providerMetadata, "Failed to record session usage on abort", "error", streamInfo);
        // Emit abort event with usage if available
        this.emitStreamAbort(workspaceId, streamInfo.messageId, { usage, contextUsage, duration, providerMetadata, contextProviderMetadata }, abortReason, abandonPartial);
        // Clean up immediately
        this.workspaceStreams.delete(workspaceId);
    }
    async recordSessionUsage(workspaceId, model, usage, providerMetadata, logMessage, logLevel, streamInfo) {
        if (!usage) {
            return;
        }
        const messageUsage = (0, displayUsage_1.createDisplayUsage)(usage, model, providerMetadata);
        if (!messageUsage) {
            return;
        }
        ...
        if (this.costTrackingService) {
            try {
                await this.costTrackingService.trackCost(workspaceId, (0, models_1.normalizeGatewayModel)(model), usage, providerMetadata);
            }
            catch (error) {
        ...
        if (this.sessionUsageService) {
            try {
                await this.sessionUsageService.recordUsage(workspaceId, (0, models_1.normalizeGatewayModel)(model), messageUsage);
            }
            catch (error) {
    ...
    buildStreamRequestConfig(model, modelString, messages, system, tools, providerOptions, maxOutputTokens, toolPolicy, hasQueuedMessage) {
        ...
        if (tools && toolPolicy) {
            ...
            if (hasRequireAction && Object.keys(tools).length === 1) {
                ...
                log_1.log.debug("Setting toolChoice to tool", { toolName: requiredToolName });
            }
        }
        ...
        const [provider] = (0, models_1.normalizeGatewayModel)(modelString).split(":", 2);
        if (toolChoice &&
            provider === "anthropic" &&
            providerOptions &&
            ...
            if (anthropicOptions &&
                ...
                const { thinking: _thinking, ...rest } = anthropicOptions;
        ...
        const cachedSystemMessage = (0, cacheStrategy_1.createCachedSystemMessage)(system, modelString);
        ...
        if (tools) {
            finalTools = (0, cacheStrategy_1.applyCacheControlToTools)(tools, modelString);
        }
        ...
        const modelStats = (0, modelStats_1.getModelStats)(modelString);
    ...
    createStopWhenCondition(request) {
        ...
        return [(0, ai_1.stepCountIs)(100000), () => request.hasQueuedMessage?.() ?? false];
    }
    createStreamResult(request, abortController, stepTracker) {
        return (0, ai_1.streamText)({
            ...
            prepareStep: ({ messages: stepMessages }) => {
                ...
                const rewritten = (0, extractToolMediaAsUserMessagesFromModelMessages_1.extractToolMediaAsUserMessagesFromModelMessages)(stepMessages);
                const effectiveMessages = rewritten === stepMessages ? stepMessages : rewritten;
                if (stepTracker) {
                    stepTracker.latestMessages = effectiveMessages;
                }
                if (rewritten === stepMessages)
                    return undefined;
                return { messages: rewritten };
            },
            tools: request.tools,
            // eslint-disable-next-line @typescript-eslint/no-explicit-any, @typescript-eslint/no-unsafe-assignment
            toolChoice: request.toolChoice, // Force tool use when required by policy
            ...
            // eslint-disable-next-line @typescript-eslint/no-explicit-any, @typescript-eslint/no-unsafe-assignment
            providerOptions: request.providerOptions, // Pass provider-specific options (thinking/reasoning config)
     ...
     */
    createStreamAtomically(workspaceId, streamToken, runtimeTempDir, runtime, messages, model, modelString, abortController, system, historySequence, messageId, tools, initialMetadata, providerOptions, maxOutputTokens, toolPolicy, hasQueuedMessage, workspaceName, thinkingLevel) {
        // abortController is created and linked to the caller-provided abortSignal in startStream().
        const stepTracker = {};
        const request = this.buildStreamRequestConfig(model, modelString, messages, system, tools, providerOptions, maxOutputTokens, toolPolicy, hasQueuedMessage);
     ...
     */
    async completeToolCall(workspaceId, streamInfo, toolCalls, toolCallId, toolName, output) {
        // Find and update the existing tool part
        const existingPartIndex = streamInfo.parts.findIndex((p) => p.type === "dynamic-tool" && p.toolCallId === toolCallId);
    ...
    async finishToolCall(workspaceId, streamInfo, toolCalls, toolCallId, toolName, output) {
    ...
    logOrphanToolResult(workspaceLog, streamInfo, part, output, orphanCount, trackedToolCallCount) {
        if (orphanCount > MAX_ORPHAN_TOOL_RESULT_WARNINGS_PER_STREAM) {
            return;
        }
        workspaceLog.warn("[streamManager] Received tool-result without matching tool-call map entry; persisting fallback output", {
            messageId: streamInfo.messageId,
        ...
        if (orphanCount === MAX_ORPHAN_TOOL_RESULT_WARNINGS_PER_STREAM) {
            workspaceLog.warn("[streamManager] Suppressing additional orphan tool-result warnings for this stream", {
                messageId: streamInfo.messageId,
                model: streamInfo.model,
     ...
     */
    emitNestedToolEvent(workspaceId, messageId, event) {
        ...
        if (streamInfo) {
            const parentPartIndex = streamInfo.parts.findIndex((p) => p.type === "dynamic-tool" && "toolCallId" in p && p.toolCallId === event.parentToolCallId);
            if (parentPartIndex !== -1) {
                const parentPart = streamInfo.parts[parentPartIndex];
    ...
    emitStreamAbort(workspaceId, messageId, metadata, abortReason, abandonPartial) {
            ...
            while (true) {
                ...
                try {
                    for await (const part of streamInfo.streamResult.fullStream) {
                        ...
                        switch (part.type) {
                            ...
                            case "text-delta": {
                                ...
                                const textDeltaPart = part;
                                const deltaText = typeof textDeltaPart.text === "string"
                                    ? textDeltaPart.text
                                    : typeof textDeltaPart.delta === "string"
                                        ...
                                        : typeof textDeltaPart.textDelta === "string"
                                            ...
                                            : "";
                                if (deltaText.length === 0) {
                                    if (textDeltaPart.text !== undefined ||
                                        textDeltaPart.delta !== undefined ||
                                        textDeltaPart.textDelta !== undefined) {
                                        log_1.log.debug("[streamManager] Ignoring non-string text-delta payload", {
                                            workspaceId,
                                            model: streamInfo.model,
                            ...
                            case "tool-call": {
                                ...
                                const inputText = JSON.stringify(part.input);
                                log_1.log.debug(`[StreamManager] tool-call: toolName=${part.toolName}, input length=${inputText.length}`);
                            ...
                            case "tool-result": {
                                ...
                                const strippedOutput = (0, internalToolResultFields_1.stripInternalToolResultFields)(stripEncryptedContent(toolResultPart.output));
                                ...
                                if (toolCall) {
                                    ...
                                    this.logOrphanToolResult(workspaceLog, streamInfo, {
                                        toolCallId: toolResultPart.toolCallId,
                                        toolName: toolResultPart.toolName,
                                    }, strippedOutput, orphanToolResultCount, toolCalls.size);
                                }
                                // Use shared completion logic (await to ensure partial is flushed before event)
                                await this.finishToolCall(workspaceId, streamInfo, toolCalls, toolResultPart.toolCallId, toolResultPart.toolName, strippedOutput);
                                break;
                            }
                            ...
                            case "tool-error": {
                                ...
                                const logLevel = streamInfo.abortController.signal.aborted ? log_1.log.debug : log_1.log.error;
                                ...
                                // Use shared completion logic (await to ensure partial is flushed before event)
                                await this.finishToolCall(workspaceId, streamInfo, toolCalls, toolErrorPart.toolCallId, toolErrorPart.toolName, errorOutput);
                                break;
                            }
                            ...
                            case "error": {
                                ...
                                if (errorPart.error instanceof Error) {
                                    ...
                                    errorMessage ?? (errorMessage = typeof errorObj.message === "string" ? errorObj.message : undefined);
                                    // Last resort: stringify the error
                                    errorMessage ?? (errorMessage = JSON.stringify(errorObj));
                            ...
                            case "finish-step": {
                                ...
                                streamInfo.cumulativeUsage = (0, usageHelpers_1.addUsage)(streamInfo.cumulativeUsage, finishStepPart.usage);
                                streamInfo.cumulativeProviderMetadata = (0, usageHelpers_1.accumulateProviderMetadata)(streamInfo.cumulativeProviderMetadata, finishStepPart.providerMetadata);
                    ...
                    if (!streamInfo.abortController.signal.aborted) {
                        ...
                        const contextProviderMetadata = streamMeta.contextProviderMetadata ?? streamInfo.lastStepProviderMetadata;
                        const duration = streamMeta.duration;
                        // Aggregated provider metadata across all steps (for cost calculation with cache tokens)
                        const providerMetadata = markProviderMetadataCostsIncluded(await this.getAggregatedProviderMetadata(streamInfo), streamInfo.initialMetadata?.costsIncluded);
                        const canonicalModel = (0, models_1.normalizeGatewayModel)(streamInfo.model);
                        const routedThroughGateway = streamInfo.initialMetadata?.routedThroughGateway ??
                            streamInfo.model.startsWith("mux-gateway:");
                        ...
                        if (streamInfo.parts && streamInfo.parts.length > 0) {
                            ...
                            await this.recordSessionUsage(workspaceId, streamInfo.model, totalUsage, providerMetadata, "Failed to record session usage (stream completion unaffected)", "warn", streamInfo);
                        }
                    ...
                    let retried = false;
                    try {
                        retried = await this.retryStreamWithoutPreviousResponseId(workspaceId, streamInfo, error, didRetryPreviousResponseId);
                    }
                    catch (retryError) {
     ...
     */
    async handleStreamFailure(workspaceId, streamInfo, error) {
    ...
    buildStreamErrorPayload(streamInfo, error) {
        ...
        let errorMessage = (0, sendMessageError_1.stripNoisyErrorPrefix)(error instanceof Error ? error.message : String(error));
        ...
        const isOverloadedApiCallError = (apiError) => {
            return (apiError.statusCode === 529 ||
                (hasErrorProperty(apiError.data) && apiError.data.error.type === "overloaded_error"));
        };
        const isAnthropicOverloaded = isAnthropic &&
            ((ai_1.APICallError.isInstance(actualError) && isOverloadedApiCallError(actualError)) ||
                (ai_1.RetryError.isInstance(actualError) &&
                    actualError.lastError &&
                    ai_1.APICallError.isInstance(actualError.lastError) &&
        ...
        const muxGatewayUnauthorized = streamInfo.model.startsWith("mux-gateway:") &&
            ((ai_1.APICallError.isInstance(actualError) && actualError.statusCode === 401) ||
                (ai_1.RetryError.isInstance(actualError) &&
                    actualError.lastError &&
                    ai_1.APICallError.isInstance(actualError.lastError) &&
        ...
        if (muxGatewayUnauthorized) {
            ...
            errorMessage = muxGatewayOAuth_1.MUX_GATEWAY_SESSION_EXPIRED_MESSAGE;
        }
        errorType = (0, sendMessageError_1.coerceStreamErrorTypeForMessage)(errorType, errorMessage);
        return {
            messageId: streamInfo.messageId,
            error: errorMessage,
            errorType,
     ...
     */
    async persistStreamError(workspaceId, streamInfo, payload) {
        const canonicalModel = (0, models_1.normalizeGatewayModel)(streamInfo.model);
        const routedThroughGateway = streamInfo.initialMetadata?.routedThroughGateway ??
            streamInfo.model.startsWith("mux-gateway:");
        ...
        // Emit error event.
        this.emit("error", (0, sendMessageError_1.createErrorEvent)(workspaceId, payload));
    }
    getOpenAIPreviousResponseId(providerOptions) {
        ...
        const previousResponseId = openaiOptions.previousResponseId;
        return typeof previousResponseId === "string" ? previousResponseId : undefined;
    }
    clearOpenAIPreviousResponseId(providerOptions) {
        if (!providerOptions || typeof providerOptions !== "object" || !("openai" in providerOptions)) {
        ...
        const { previousResponseId: _prev, ...rest } = openaiOptions;
        return {
            ...providerOptions,
            openai: rest,
        };
    }
    async resetStreamStateForRetry(workspaceId, streamInfo, options) {
    ...
    async retryStreamWithoutPreviousResponseId(workspaceId, streamInfo, error, hasRetried) {
        if (hasRetried) {
            return false;
        }
        ...
        const shouldRetry = errorCode === "previous_response_not_found" ||
            statusCode === 404 ||
            statusCode === 500 ||
            statusCode === 400;
        if (!shouldRetry) {
        ...
        streamInfo.streamResult = this.createStreamResult(streamInfo.request, streamInfo.abortController, streamInfo.stepTracker);
        return true;
    }
    /**
    ...
    categorizeError(error) {
        ...
        if (typeof error === "object" &&
            ...
            // Model not found
            if (structuredError.code === "model_not_found" ||
     ...
     */
    async startStream(workspaceId, messages, model, modelString, historySequence, system, runtime, messageId, abortSignal, tools, initialMetadata, providerOptions, maxOutputTokens, toolPolicy, providedStreamToken, hasQueuedMessage, workspaceName, thinkingLevel) {
        ...
        try {
            ...
            try {
                ...
                const unlinkAbortSignal = (0, abort_1.linkAbortSignal)(abortSignal, streamAbortController);
                let runtimeTempDir;
                ...
                try {
                    ...
                    const streamInfo = this.createStreamAtomically(typedWorkspaceId, streamToken, runtimeTempDir, runtime, messages, model, modelString, streamAbortController, system, historySequence, messageId, tools, initialMetadata, providerOptions, maxOutputTokens, toolPolicy, hasQueuedMessage, workspaceName, thinkingLevel);
                    ...
                    streamInfo.processingPromise = this.processStreamWithCleanup(typedWorkspaceId, streamInfo, historySequence).catch((error) => {
                        log_1.log.error("Unexpected error in stream processing:", error);
                    });
                    return (0, result_1.Ok)(streamToken);
                }
                finally {
     ...
     */
    recordLostResponseIdIfApplicable(workspaceId, error, streamInfo, workspaceLog) {
        const responseId = this.extractPreviousResponseIdFromError(error);
        if (!responseId) {
            return;
        }
        ...
        const shouldRecord = errorCode === "previous_response_not_found" ||
            statusCode === 404 ||
            statusCode === 500 ||
            statusCode === 400;
     ...
     */
    extractPreviousResponseIdFromError(error) {
        // Check APICallError.responseBody first
        if (ai_1.APICallError.isInstance(error) && typeof error.responseBody === "string") {
    ...
    extractStatusCode(error) {
        ...
        if (ai_1.APICallError.isInstance(error) && typeof error.statusCode === "number") {
    ...
    async stopStream(workspaceId, options) {
        ...
        try {
            ...
            if (soft) {
                ...
                await this.cancelStreamSafely(typedWorkspaceId, streamInfo, abortReason, options?.abandonPartial);
            }
            return (0, result_1.Ok)(undefined);
        }
        catch (error) {
     ...
     */
    async debugTriggerStreamError(workspaceId, errorMessage) {
        const typedWorkspaceId = workspaceId;
        const streamInfo = this.workspaceStreams.get(typedWorkspaceId);
