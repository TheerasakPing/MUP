...
exports.MockAiRouter = exports.MOCK_STREAM_START_GATE_MARKER = void 0;
exports.buildMockStreamStartGateMessage = buildMockStreamStartGateMessage;
...
exports.MOCK_STREAM_START_GATE_MARKER = "[mock:wait-start]";
/** Build a message that triggers the mock stream-start gate. */
function buildMockStreamStartGateMessage(text) {
    return `${exports.MOCK_STREAM_START_GATE_MARKER} ${text}`.trim();
}
function normalizeText(text) {
    return text.trim().toLowerCase();
}
function shouldWaitForStreamStart(text) {
    return STREAM_START_WAIT_PATTERN.test(text);
}
function applyStreamStartGate(reply, latestUserText) {
...
function hasMockMarker(text, marker) {
    const normalized = normalizeText(text);
    return normalized.includes(`${MOCK_MARKER_PREFIX}${marker.toLowerCase()}`);
}
function hasCompactionHistory(messages) {
...
function readCompactionRequest(message) {
    const muxMeta = message.metadata?.muxMetadata;
    if (!muxMeta || muxMeta.type !== "compaction-request") {
        return undefined;
    }
    return { followUpContent: (0, message_1.getCompactionFollowUpContent)(muxMeta) };
}
function buildUsage(inputTokens, outputTokens) {
    return {
        inputTokens,
        outputTokens,
...
function buildListProgrammingLanguagesReply() {
    return {
        assistantText: [
...
function buildPermissionPlanReply() {
    return {
        assistantText: [
            "Plan summary:",
...
function buildPermissionExecReply() {
    return {
        assistantText: [
...
function buildToolListDirectoryReply() {
    return {
...
function buildToolCreateTestFileReply() {
    return {
...
function buildToolReadTestFileReply() {
    return {
...
function buildReasoningQuicksortReply() {
    return {
        assistantText: [
...
function buildReviewBranchesReply() {
    return {
        assistantText: [
...
function buildReviewOpenDocErrorReply() {
    return {
        assistantText: "",
        error: {
...
function buildReviewShowDocReply() {
    return {
        assistantText: [
...
function buildContextExceededReply() {
    return {
...
function buildNetworkErrorReply() {
    return {
        assistantText: "",
        error: {
...
const defaultHandlers = [
    ...
    {
        match: (request) => hasMockMarker(request.latestUserText, "list-languages") ||
    ...
    {
        match: (request) => hasMockMarker(request.latestUserText, "model-status") ||
    ...
    {
        match: (request) => hasMockMarker(request.latestUserText, "error:rate-limit") ||
            normalizeText(request.latestUserText) === "trigger rate limit error",
        respond: () => buildRateLimitErrorReply(),
    },
    {
        match: (request) => hasMockMarker(request.latestUserText, "error:api") ||
            normalizeText(request.latestUserText) === "trigger api error",
        respond: () => buildApiErrorReply(),
    },
    {
        match: (request) => hasMockMarker(request.latestUserText, "error:network") ||
...
exports.MockAiRouter = MockAiRouter;
//# sourceMappingURL=mockAiRouter.js.map