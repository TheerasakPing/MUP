...
exports.prepareMessagesForProvider = prepareMessagesForProvider;
exports.sanitizeAssistantModelMessages = sanitizeAssistantModelMessages;
 ...
 */
async function prepareMessagesForProvider(opts) {
    const { messagesWithSentinel, effectiveAgentId, toolNamesForSentinel, planContentForTransition, planFilePath, changedFileAttachments, postCompactionAttachments, runtime, workspacePath, abortSignal, providerForMessages, effectiveThinkingLevel, modelString, workspaceId, } = opts;
    ...
    const rawModelMessages = await (0, ai_1.convertToModelMessages)(messagesWithToolMediaExtracted, {
 ...
 */
function sanitizeAssistantModelMessages(messages, workspaceId) {
    const result = messages.flatMap((msg) => {
        ...
        const filteredContent = msg.content.filter((part) => part.type !== "text" || part.text.trim().length > 0);
    ...
    if (result.length < messages.length) {
        log_1.log.debug(`Self-healing: Filtered ${messages.length - result.length} empty ModelMessage(s)${workspaceId ? ` [${workspaceId}]` : ""}`);
    }
    return result;
}
