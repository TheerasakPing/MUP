...
(0, bun_test_1.describe)("StreamManager - createTempDirForStream", () => {
    (0, bun_test_1.test)("creates ~/.mux-tmp/<token> under the runtime's home", async () => {
        ...
        try {
            ...
            try {
                const streamManager = new streamManager_1.StreamManager(historyService, createMockPartialService());
                ...
                (0, bun_test_1.expect)(resolved).toContain(`/.mux-tmp/${token}`);
...
(0, bun_test_1.describe)("StreamManager - stopWhen configuration", () => {
    (0, bun_test_1.test)("uses single-step stopWhen when a tool is required", () => {
        const streamManager = new streamManager_1.StreamManager(historyService, createMockPartialService());
        const buildStopWhen = Reflect.get(streamManager, "createStopWhenCondition");
        (0, bun_test_1.expect)(typeof buildStopWhen).toBe("function");
        ...
        (0, bun_test_1.expect)(stopWhen({ steps: [] })).toBe(false);
        (0, bun_test_1.expect)(stopWhen({ steps: [{}] })).toBe(true);
        (0, bun_test_1.expect)(stopWhen({ steps: [{}, {}] })).toBe(false);
    });
    (0, bun_test_1.test)("uses autonomous step cap and queued-message interrupt conditions", () => {
        const streamManager = new streamManager_1.StreamManager(historyService, createMockPartialService());
        const buildStopWhen = Reflect.get(streamManager, "createStopWhenCondition");
        (0, bun_test_1.expect)(typeof buildStopWhen).toBe("function");
        let queued = false;
        ...
        (0, bun_test_1.expect)(stopWhen).toHaveLength(2);
        const [maxStepCondition, queuedMessageCondition] = stopWhen;
        (0, bun_test_1.expect)(maxStepCondition({ steps: new Array(99999) })).toBe(false);
        (0, bun_test_1.expect)(maxStepCondition({ steps: new Array(100000) })).toBe(true);
        (0, bun_test_1.expect)(queuedMessageCondition({ steps: [] })).toBe(false);
        queued = true;
        (0, bun_test_1.expect)(queuedMessageCondition({ steps: [] })).toBe(true);
    });
    (0, bun_test_1.test)("treats missing queued-message callback as not queued", () => {
        const streamManager = new streamManager_1.StreamManager(historyService, createMockPartialService());
        const buildStopWhen = Reflect.get(streamManager, "createStopWhenCondition");
        (0, bun_test_1.expect)(typeof buildStopWhen).toBe("function");
...
(0, bun_test_1.describe)("StreamManager - stripEncryptedContent", () => {
    (0, bun_test_1.test)("strips encryptedContent from array output shape", () => {
        const output = [
            {
        ...
        (0, bun_test_1.expect)((0, streamManager_1.stripEncryptedContent)(output)).toEqual([
            {
                url: "https://example.com/a",
                title: "Result A",
                pageAge: "2d",
    ...
    (0, bun_test_1.test)("strips encryptedContent from json value output shape", () => {
        const output = {
            type: "json",
            value: [
                {
...
(0, bun_test_1.describe)("StreamManager - Concurrent Stream Prevention", () => {
    ...
    (0, bun_test_1.beforeEach)(() => {
        ...
        streamManager = new streamManager_1.StreamManager(historyService, mockPartialService);
...
(0, bun_test_1.describe)("StreamManager - previousResponseId recovery", () => {
    (0, bun_test_1.test)("isResponseIdLost returns false for unknown IDs", () => {
        ...
        const streamManager = new streamManager_1.StreamManager(historyService, mockPartialService);
...
(0, bun_test_1.describe)("StreamManager - categorizeError", () => {
    ...
    (0, bun_test_1.test)("classifies 402 payment required as quota (avoid auto-retry)", () => {
        const mockPartialService = createMockPartialService();
        const streamManager = new streamManager_1.StreamManager(historyService, mockPartialService);
        const categorizeMethod = Reflect.get(streamManager, "categorizeError");
        (0, bun_test_1.expect)(typeof categorizeMethod).toBe("function");
        const apiError = new ai_1.APICallError({
            ...
            statusCode: 402,
            responseHeaders: {},
            responseBody: '{"error":{"message":"Insufficient balance. Please add credits to continue.","type":"invalid_request_error"}}',
            isRetryable: false,
            data: {
                error: { message: "Insufficient balance. Please add credits to continue." },
            },
        });
        (0, bun_test_1.expect)(categorizeMethod.call(streamManager, apiError)).toBe("quota");
    });
});
