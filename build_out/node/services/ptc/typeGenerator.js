...
exports.getCachedMuxTypes = getCachedMuxTypes;
exports.generateMuxTypes = generateMuxTypes;
...
function hashToolDefinitions(tools) {
    ...
    return (0, crypto_1.createHash)("md5").update(JSON.stringify(toolData)).digest("hex");
}
/**
 * Get cached mux types or generate new ones if tool definitions changed.
 */
async function getCachedMuxTypes(tools) {
...
async function getResultTypeString(toolName) {
    ...
    if (!(toolName in toolDefinitions_1.RESULT_SCHEMAS)) {
        return null;
    }
    const schema = toolDefinitions_1.RESULT_SCHEMAS[toolName];
    // Convert Zod → JSON Schema → TypeScript
    const jsonSchema = zod_1.z.toJSONSchema(schema);
    const tsOutput = await (0, json_schema_to_typescript_1.compile)(jsonSchema, `${pascalCase(toolName)}Result`, {
        bannerComment: "",
 ...
 */
async function generateMuxTypes(tools) {
    ...
    for (const name of sortedNames) {
        ...
        try {
            const argInterface = await (0, json_schema_to_typescript_1.compile)(inputSchema, argsTypeName, {
                bannerComment: "",
