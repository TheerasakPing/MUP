...
(0, bun_test_1.describe)("MCPConfigService", () => {
    let tempDir;
    let config;
    let configService;
    (0, bun_test_1.beforeEach)(async () => {
        ...
        configService = new mcpConfigService_1.MCPConfigService(config);
    });
    (0, bun_test_1.afterEach)(async () => {
        await fs.rm(tempDir, { recursive: true, force: true });
    });
    (0, bun_test_1.test)("writes global config to <rootDir>/mcp.jsonc", async () => {
        const result = await configService.addServer("test", {
            transport: "stdio",
            command: "echo hi",
        });
        (0, bun_test_1.expect)(result).toEqual({ success: true, data: undefined });
        ...
        (0, bun_test_1.expect)(raw).toContain('"test"');
    });
    (0, bun_test_1.test)("listServers merges repo overrides on top of global (override wins by name)", async () => {
        ...
        await fs.writeFile(path.join(projectPath, ".mux", "mcp.jsonc"), `// repo override\n{\n  "servers": {\n    "shared": "repo-shared",\n    "repo-only": { "command": "repo-only", "disabled": true }\n  }\n}\n`, "utf-8");
