...
(0, bun_test_1.describe)("InitStateManager", () => {
    ...
    (0, bun_test_1.describe)("cleanup", () => {
        ...
        (0, bun_test_1.it)("should not recreate session directory if queued persistence runs after state is cleared", async () => {
            const workspaceId = "test-workspace";
            manager.startInit(workspaceId, "/path/to/hook");
            const sessionDir = config.getSessionDir(workspaceId);
            await fs.mkdir(sessionDir, { recursive: true });
            let releaseLock;
            const lockHeld = workspaceFileLocks_1.workspaceFileLocks.withLock(workspaceId, async () => {
            ...
            (0, bun_test_1.expect)(await manager.readInitStatus(workspaceId)).toBeNull();
            const sessionDirExists = await fs
                .access(sessionDir)
                .then(() => true)
                .catch(() => false);
            (0, bun_test_1.expect)(sessionDirExists).toBe(false);
        });
    });
    ...
    (0, bun_test_1.describe)("waitForInit hook phase", () => {
        (0, bun_test_1.it)("should not time out during runtime setup (intentional)", async () => {
            const workspaceId = "test-workspace";
            manager.startInit(workspaceId, "/path/to/hook");
            ...
            (0, bun_test_1.expect)(result).toBe("pending");
            await manager.endInit(workspaceId, 0);
            await waitPromise;
        });
        (0, bun_test_1.it)("should start timeout once hook phase begins", async () => {
            const workspaceId = "test-workspace";
            manager.startInit(workspaceId, "/path/to/hook");
        ...
        (0, bun_test_1.it)("should set hookStartTime when entering hook phase", () => {
            const workspaceId = "test-workspace";
            manager.startInit(workspaceId, "/path/to/hook");
            manager.enterHookPhase(workspaceId);
            const state = manager.getInitState(workspaceId);
            ...
            (0, bun_test_1.expect)(state.phase).toBe("init_hook");
            (0, bun_test_1.expect)(state.hookStartTime).toBeDefined();
            (0, bun_test_1.expect)(typeof state.hookStartTime).toBe("number");
        });
    });
    (0, bun_test_1.describe)("waitForInit with abortSignal", () => {
        (0, bun_test_1.it)("should return immediately if abortSignal is already aborted", async () => {
            ...
            (0, bun_test_1.expect)(Date.now() - start).toBeLessThan(200); // Should be instant
        });
        (0, bun_test_1.it)("should return when abortSignal fires during wait", async () => {
            ...
            const start = Date.now();
            await waitPromise;
            (0, bun_test_1.expect)(Date.now() - start).toBeLessThan(300); // Should return quickly after abort
        });
