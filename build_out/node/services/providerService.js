...
class ProviderService {
    ...
    list() {
        try {
            const providers = [...providers_1.SUPPORTED_PROVIDERS];
    ...
    getConfig() {
        ...
        for (const provider of this.list()) {
            ...
            const filteredModels = Array.isArray(allowedModels) && config.models
                ...
                : config.models;
            const codexOauthSet = provider === "openai" && (0, codexOauthAuth_1.parseCodexOauthAuth)(config.codexOauth) !== null;
            const providerInfo = {
     ...
     */
    setModelMetadata(provider, modelId, metadata) {
        try {
            if (this.policyService?.isEnforced()) {
                if (!this.policyService.isProviderAllowed(provider)) {
                    return { success: false, error: `Provider ${provider} is not allowed by policy` };
