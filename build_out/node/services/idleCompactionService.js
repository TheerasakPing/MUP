...
exports.IdleCompactionService = void 0;
...
class IdleCompactionService {
    ...
    historyService;
    extensionMetadata;
    emitIdleCompactionNeeded;
    initialTimeout = null;
    checkInterval = null;
    constructor(config, historyService, extensionMetadata, emitIdleCompactionNeeded) {
        this.config = config;
    ...
    start() {
        ...
        log_1.log.info("IdleCompactionService started", {
    ...
    stop() {
        ...
        log_1.log.info("IdleCompactionService stopped");
    }
    /**
        ...
        for (const [projectPath, projectConfig] of projectsConfig.projects) {
            ...
            for (const workspace of projectConfig.workspaces) {
                ...
                try {
                    ...
                    log_1.log.error("Idle compaction check failed", { workspaceId, error });
                }
            }
        }
    }
    async checkWorkspace(workspaceId, _projectPath, thresholdMs, now) {
        // Check eligibility
        const eligibility = await this.checkEligibility(workspaceId, thresholdMs, now);
        if (!eligibility.eligible) {
            log_1.log.debug("Workspace not eligible for idle compaction", {
                workspaceId,
        ...
        log_1.log.info("Workspace eligible for idle compaction", {
            workspaceId,
     ...
     */
    async checkEligibility(workspaceId, thresholdMs, now) {
     ...
     */
    notifyNeedsCompaction(workspaceId) {
        this.emitIdleCompactionNeeded(workspaceId);
    }
}
exports.IdleCompactionService = IdleCompactionService;
//# sourceMappingURL=idleCompactionService.js.map