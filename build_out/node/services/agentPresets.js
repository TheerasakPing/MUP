...
const REPORTING_TOOL_NAMES = ["agent_report"];
function enableOnly(...toolNames) {
    return [
        { regex_match: ".*", action: "disable" },
        ...toolNames.map((toolName) => ({ regex_match: toolName, action: "enable" })),
    ];
}
...
const EXPLORE_PRESET = {
    agentType: "explore",
    toolPolicy: enableOnly("file_read", "bash", "task_await", "task_list", "task_terminate", "web_fetch", "web_search", "google_search", ...REPORTING_TOOL_NAMES),
    systemPrompt: buildSystemPrompt({
        agentLabel: "Explore",
        goals: [
...
const PRESETS_BY_AGENT_TYPE = {
    explore: EXPLORE_PRESET,
    exec: EXEC_PRESET,
};
function getAgentPreset(agentType) {
    const normalized = (agentType ?? "").trim().toLowerCase();
    return PRESETS_BY_AGENT_TYPE[normalized] ?? null;
}
//# sourceMappingURL=agentPresets.js.map