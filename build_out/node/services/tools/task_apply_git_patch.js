...
exports.createTaskApplyGitPatchTool = void 0;
...
const createTaskApplyGitPatchTool = (config) => {
    ...
    async function tryRevParseHead(params) {
        try {
            const headResult = await (0, helpers_1.execBuffered)(config.runtime, "git rev-parse HEAD", {
                cwd: params.cwd,
                timeout: 10,
            });
    ...
    async function getAppliedCommits(params) {
        ...
        async function tryGitLog(args) {
            try {
                const result = await (0, helpers_1.execBuffered)(config.runtime, args.cmd, {
                    cwd: params.cwd,
                    timeout: 30,
                });
                if (result.exitCode !== 0) {
                    log_1.log.debug("task_apply_git_patch: git log failed", {
                        cwd: params.cwd,
                ...
                log_1.log.debug("task_apply_git_patch: git log threw", { cwd: params.cwd, error });
                return undefined;
            }
        }
    ...
    function inferMuxRootFromWorkspaceSessionDir(workspaceSessionDir) {
        (0, strict_1.default)(workspaceSessionDir.length > 0, "inferMuxRootFromWorkspaceSessionDir: workspaceSessionDir must be non-empty");
    ...
    function parseFailedPatchSubjectFromGitAmOutput(output) {
    ...
    async function tryGetConflictPaths(params) {
        (0, strict_1.default)(params.cwd.length > 0, "tryGetConflictPaths: cwd must be non-empty");
        try {
            const diffResult = await (0, helpers_1.execBuffered)(config.runtime, "git diff --name-only --diff-filter=U", {
                cwd: params.cwd,
                timeout: 30,
            });
            if (diffResult.exitCode !== 0) {
                log_1.log.debug("task_apply_git_patch: git diff --name-only --diff-filter=U failed", {
                    cwd: params.cwd,
            ...
            log_1.log.debug("task_apply_git_patch: git diff --name-only --diff-filter=U threw", {
                cwd: params.cwd,
                error,
            });
            return [];
    ...
    async function findGitPatchArtifactInWorkspaceOrAncestors(params) {
        (0, strict_1.default)(params.workspaceId.length > 0, "findGitPatchArtifactInWorkspaceOrAncestors: workspaceId must be non-empty");
        (0, strict_1.default)(params.workspaceSessionDir.length > 0, "findGitPatchArtifactInWorkspaceOrAncestors: workspaceSessionDir must be non-empty");
        (0, strict_1.default)(params.childTaskId.length > 0, "findGitPatchArtifactInWorkspaceOrAncestors: childTaskId must be non-empty");
        const direct = await (0, subagentGitPatchArtifacts_1.readSubagentGitPatchArtifact)(params.workspaceSessionDir, params.childTaskId);
        if (direct) {
            return {
                artifact: direct,
        ...
        if (!muxRootDir) {
            log_1.log.debug("task_apply_git_patch: workspaceSessionDir not under sessions/; skipping ancestor lookup", {
        ...
        const configService = new config_1.Config(muxRootDir);
        let cfg;
        try {
            cfg = configService.loadConfigOrDefault();
        }
        catch (error) {
            log_1.log.debug("task_apply_git_patch: failed to load mux config for ancestor lookup", {
                workspaceId: params.workspaceId,
                muxRootDir,
                error,
            });
        ...
        for (let i = 0; i < MAX_PARENT_WORKSPACE_DEPTH; i++) {
            ...
            if (visited.has(parent)) {
                log_1.log.warn("task_apply_git_patch: possible parentWorkspaceId cycle during ancestor lookup", {
                    workspaceId: params.workspaceId,
                    childTaskId: params.childTaskId,
                    current,
                    parent,
            ...
            const artifact = await (0, subagentGitPatchArtifacts_1.readSubagentGitPatchArtifact)(parentSessionDir, params.childTaskId);
            if (artifact) {
                return {
                    artifact,
        ...
        log_1.log.warn("task_apply_git_patch: exceeded parentWorkspaceId depth during ancestor lookup", {
            workspaceId: params.workspaceId,
            childTaskId: params.childTaskId,
        });
        return null;
    ...
    return (0, ai_1.tool)({
        description: toolDefinitions_1.TOOL_DEFINITIONS.task_apply_git_patch.description,
        inputSchema: toolDefinitions_1.TOOL_DEFINITIONS.task_apply_git_patch.schema,
        execute: async (args, { abortSignal }) => {
            const workspaceId = (0, toolUtils_1.requireWorkspaceId)(config, "task_apply_git_patch");
            (0, strict_1.default)(config.cwd, "task_apply_git_patch requires cwd");
            (0, strict_1.default)(config.runtimeTempDir, "task_apply_git_patch requires runtimeTempDir");
            const workspaceSessionDir = config.workspaceSessionDir;
            (0, strict_1.default)(workspaceSessionDir, "task_apply_git_patch requires workspaceSessionDir");
            ...
            if (!artifactLookup) {
                return (0, toolUtils_1.parseToolResult)(toolDefinitions_1.TaskApplyGitPatchToolResultSchema, {
                    success: false,
                    taskId,
                    dryRun,
            ...
            if (artifact.parentWorkspaceId !== artifactWorkspaceId) {
                return (0, toolUtils_1.parseToolResult)(toolDefinitions_1.TaskApplyGitPatchToolResultSchema, {
                    ...
                    note: mergeNotes(artifactLookupNote, `Expected parent workspace ${artifactWorkspaceId} but artifact metadata says ${artifact.parentWorkspaceId}.`),
                }, "task_apply_git_patch");
            }
            ...
            const expectedPatchPath = (0, subagentGitPatchArtifacts_1.getSubagentGitPatchMboxPath)(artifactSessionDir, taskId);
            ...
            const safeMboxPath = typeof artifact.mboxPath === "string" && artifact.mboxPath.length > 0
                ? isPathInsideDir(artifactSessionDir, artifact.mboxPath)
                    ? artifact.mboxPath
                    : undefined
                : undefined;
            let patchPathNote = mergeNotes(artifactLookupNote, artifact.mboxPath && !safeMboxPath
                ? "Ignoring unsafe mboxPath in patch artifact metadata; using canonical patch location."
                : undefined);
            const patchCandidates = [safeMboxPath, expectedPatchPath].filter((candidate) => typeof candidate === "string");
            let patchPath = null;
            ...
            if (!patchPath) {
                const checkedPaths = Array.from(new Set(patchCandidates))
                    .map((candidate) => isPathInsideDir(artifactSessionDir, candidate)
                    ? path.relative(artifactSessionDir, candidate) || path.basename(candidate)
                    : candidate)
                    .join(", ");
                log_1.log.debug("task_apply_git_patch: patch file missing", {
                    taskId,
                    workspaceId,
                    cwd: config.cwd,
                    checkedPaths,
                ...
                return (0, toolUtils_1.parseToolResult)(toolDefinitions_1.TaskApplyGitPatchToolResultSchema, {
                    ...
                    note: mergeNotes(patchPathNote, checkedPaths.length > 0 ? `Checked patch locations: ${checkedPaths}` : undefined),
                }, "task_apply_git_patch");
            }
            if (safeMboxPath && patchPath === expectedPatchPath && safeMboxPath !== expectedPatchPath) {
                patchPathNote = mergeNotes(patchPathNote, "Patch file not found at metadata mboxPath; using canonical patch location.");
            }
            if (!force) {
                const statusResult = await (0, helpers_1.execBuffered)(config.runtime, "git status --porcelain", {
                    cwd: config.cwd,
                    timeout: 10,
                });
                ...
                if (statusResult.stdout.trim().length > 0) {
                    return (0, toolUtils_1.parseToolResult)(toolDefinitions_1.TaskApplyGitPatchToolResultSchema, {
                        ...
                        note: mergeNotes(patchPathNote, "Commit/stash your changes (or pass force=true) before applying patches."),
                    }, "task_apply_git_patch");
                }
            }
            ...
            const remotePatchPath = path.posix.join(config.runtimeTempDir, `mux-task-${taskId}-series.mbox`);
            ...
            if (dryRun) {
                ...
                const dryRunWorktreePath = path.posix.join(config.runtimeTempDir, `mux-git-am-dry-run-${taskId}-${dryRunId}`);
                const addResult = await (0, helpers_1.execBuffered)(config.runtime, `git worktree add --detach ${(0, shell_1.shellQuote)(dryRunWorktreePath)} HEAD`, { cwd: config.cwd, timeout: 60 });
                ...
                try {
                    ...
                    const amCmd = `git am ${flags.join(" ")} ${(0, shell_1.shellQuote)(remotePatchPath)}`.trim();
                    const amResult = await (0, helpers_1.execBuffered)(config.runtime, amCmd, {
                    ...
                    if (amResult.exitCode !== 0) {
                        ...
                        return (0, toolUtils_1.parseToolResult)(toolDefinitions_1.TaskApplyGitPatchToolResultSchema, {
                            ...
                            note: mergeNotes(patchPathNote, "Dry run failed; the patch does not apply cleanly. Applying for real will likely require conflict resolution."),
                        }, "task_apply_git_patch");
                    }
                    ...
                    try {
                        const abortResult = await (0, helpers_1.execBuffered)(config.runtime, "git am --abort", {
                            cwd: dryRunWorktreePath,
                            timeout: 30,
                        });
                        if (abortResult.exitCode !== 0) {
                            log_1.log.debug("task_apply_git_patch: dry-run git am --abort failed", {
                                taskId,
                                workspaceId,
                                cwd: config.cwd,
                                dryRunWorktreePath,
                        ...
                        log_1.log.debug("task_apply_git_patch: dry-run git am --abort threw", {
                            taskId,
                            workspaceId,
                            cwd: config.cwd,
                            dryRunWorktreePath,
                    ...
                    try {
                        const removeResult = await (0, helpers_1.execBuffered)(config.runtime, `git worktree remove --force ${(0, shell_1.shellQuote)(dryRunWorktreePath)}`, { cwd: config.cwd, timeout: 60 });
                        if (removeResult.exitCode !== 0) {
                            log_1.log.debug("task_apply_git_patch: dry-run git worktree remove failed", {
                                taskId,
                                workspaceId,
                                cwd: config.cwd,
                                dryRunWorktreePath,
                        ...
                        log_1.log.debug("task_apply_git_patch: dry-run git worktree remove threw", {
                            taskId,
                            workspaceId,
                            cwd: config.cwd,
                            dryRunWorktreePath,
                    ...
                    try {
                        const pruneResult = await (0, helpers_1.execBuffered)(config.runtime, "git worktree prune", {
                            cwd: config.cwd,
                            timeout: 60,
                        });
                        if (pruneResult.exitCode !== 0) {
                            log_1.log.debug("task_apply_git_patch: dry-run git worktree prune failed", {
                                taskId,
                                workspaceId,
                                cwd: config.cwd,
                        ...
                        log_1.log.debug("task_apply_git_patch: dry-run git worktree prune threw", {
                            taskId,
                            workspaceId,
                            cwd: config.cwd,
                            error,
            ...
            const amCmd = `git am ${flags.join(" ")} ${(0, shell_1.shellQuote)(remotePatchPath)}`.trim();
            const amResult = await (0, helpers_1.execBuffered)(config.runtime, amCmd, { cwd: config.cwd, timeout: 300 });
            if (amResult.exitCode !== 0) {
                const stderr = amResult.stderr.trim();
                const stdout = amResult.stdout.trim();
                const errorOutput = [stderr, stdout]
                ...
                return (0, toolUtils_1.parseToolResult)(toolDefinitions_1.TaskApplyGitPatchToolResultSchema, {
                    ...
                    error: errorOutput.length > 0
                        ? errorOutput
                        : `git am failed (exitCode=${amResult.exitCode})`,
                    note: mergeNotes(patchPathNote, "If git am stopped due to conflicts, resolve them then run `git am --continue` or `git am --abort`."),
                }, "task_apply_git_patch");
            }
            ...
            if (!dryRun && !isReplay) {
                await (0, subagentGitPatchArtifacts_1.markSubagentGitPatchArtifactApplied)({
...
exports.createTaskApplyGitPatchTool = createTaskApplyGitPatchTool;
//# sourceMappingURL=task_apply_git_patch.js.map