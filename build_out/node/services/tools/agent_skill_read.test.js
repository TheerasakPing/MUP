...
const agent_skill_read_1 = require("./agent_skill_read");
...
(0, bun_test_1.describe)("agent_skill_read (Chat with Mux sandbox)", () => {
    (0, bun_test_1.it)("allows reading built-in skills", async () => {
        const env_1 = { stack: [], error: void 0, hasError: false };
        try {
            const tempDir = __addDisposableResource(env_1, new testHelpers_1.TestTempDir("test-agent-skill-read-mux-chat"), false);
            const baseConfig = (0, testHelpers_1.createTestToolConfig)(tempDir.path, {
                workspaceId: muxChat_1.MUX_HELP_CHAT_WORKSPACE_ID,
            });
            const tool = (0, agent_skill_read_1.createAgentSkillReadTool)(baseConfig);
            const raw = await Promise.resolve(tool.execute({ name: "mux-docs" }, mockToolCallOptions));
            const parsed = toolDefinitions_1.AgentSkillReadToolResultSchema.safeParse(raw);
            ...
            if (result.success) {
                (0, bun_test_1.expect)(result.skill.scope).toBe("built-in");
                (0, bun_test_1.expect)(result.skill.frontmatter.name).toBe("mux-docs");
            }
        }
        catch (e_1) {
            env_1.error = e_1;
    ...
    (0, bun_test_1.it)("rejects project/global skills on disk", async () => {
        const env_2 = { stack: [], error: void 0, hasError: false };
        try {
            const tempDir = __addDisposableResource(env_2, new testHelpers_1.TestTempDir("test-agent-skill-read-mux-chat-reject"), false);
