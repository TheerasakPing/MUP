...
(0, bun_test_1.describe)("task_await tool", () => {
    (0, bun_test_1.it)("includes gitFormatPatch artifacts written during waitForAgentReport", async () => {
        const env_1 = { stack: [], error: void 0, hasError: false };
        try {
            const tempDir = __addDisposableResource(env_1, new testHelpers_1.TestTempDir("test-task-await-tool-artifacts"), false);
            ...
            const artifactsPath = (0, subagentGitPatchArtifacts_1.getSubagentGitPatchArtifactsFilePath)(workspaceSessionDir);
            ...
            const taskService = {
                ...
                isDescendantAgentTask: (0, bun_test_1.mock)(() => Promise.resolve(true)),
                waitForAgentReport: (0, bun_test_1.mock)(async (taskId) => {
    ...
    (0, bun_test_1.it)("returns completed results for all awaited tasks", async () => {
        ...
        try {
            const tempDir = __addDisposableResource(env_2, new testHelpers_1.TestTempDir("test-task-await-tool"), false);
            const baseConfig = (0, testHelpers_1.createTestToolConfig)(tempDir.path, { workspaceId: "parent-workspace" });
            const taskService = {
    ...
    (0, bun_test_1.it)("supports filterDescendantAgentTaskIds without losing this binding", async () => {
        ...
        try {
            const tempDir = __addDisposableResource(env_3, new testHelpers_1.TestTempDir("test-task-await-tool-this-binding"), false);
            const baseConfig = (0, testHelpers_1.createTestToolConfig)(tempDir.path, { workspaceId: "parent-workspace" });
            const waitForAgentReport = (0, bun_test_1.mock)(() => Promise.resolve({ reportMarkdown: "ok" }));
            const isDescendantAgentTask = (0, bun_test_1.mock)(() => Promise.resolve(true));
            const taskService = {
    ...
    (0, bun_test_1.it)("marks invalid_scope without calling waitForAgentReport", async () => {
        ...
        try {
            const tempDir = __addDisposableResource(env_4, new testHelpers_1.TestTempDir("test-task-await-tool-invalid-scope"), false);
            const baseConfig = (0, testHelpers_1.createTestToolConfig)(tempDir.path, { workspaceId: "parent-workspace" });
    ...
    (0, bun_test_1.it)("defaults to waiting on all active descendant tasks when task_ids is omitted", async () => {
        ...
        try {
            const tempDir = __addDisposableResource(env_5, new testHelpers_1.TestTempDir("test-task-await-tool-descendants"), false);
            const baseConfig = (0, testHelpers_1.createTestToolConfig)(tempDir.path, { workspaceId: "parent-workspace" });
    ...
    (0, bun_test_1.it)("maps wait errors to running/not_found/error statuses", async () => {
        ...
        try {
            const tempDir = __addDisposableResource(env_6, new testHelpers_1.TestTempDir("test-task-await-tool-errors"), false);
            const baseConfig = (0, testHelpers_1.createTestToolConfig)(tempDir.path, { workspaceId: "parent-workspace" });
    ...
    (0, bun_test_1.it)("treats timeout_secs=0 as non-blocking for agent tasks", async () => {
        ...
        try {
            const tempDir = __addDisposableResource(env_7, new testHelpers_1.TestTempDir("test-task-await-tool-timeout-zero"), false);
            const baseConfig = (0, testHelpers_1.createTestToolConfig)(tempDir.path, { workspaceId: "parent-workspace" });
    ...
    (0, bun_test_1.it)("returns completed result when timeout_secs=0 and a cached report is available", async () => {
        ...
        try {
            const tempDir = __addDisposableResource(env_8, new testHelpers_1.TestTempDir("test-task-await-tool-timeout-zero-cached"), false);
            const baseConfig = (0, testHelpers_1.createTestToolConfig)(tempDir.path, { workspaceId: "parent-workspace" });
