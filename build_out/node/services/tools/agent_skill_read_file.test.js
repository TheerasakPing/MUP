...
(0, bun_test_1.describe)("agent_skill_read_file (Chat with Mux sandbox)", () => {
    (0, bun_test_1.it)("allows reading built-in skill files", async () => {
        const env_1 = { stack: [], error: void 0, hasError: false };
        try {
            const tempDir = __addDisposableResource(env_1, new testHelpers_1.TestTempDir("test-agent-skill-read-file-mux-chat"), false);
            ...
            const raw = await Promise.resolve(tool.execute({ name: "mux-docs", filePath: "SKILL.md", offset: 1, limit: 25 }, mockToolCallOptions));
            ...
            if (result.success) {
                (0, bun_test_1.expect)(result.content).toMatch(/name:\s*mux-docs/i);
            }
        }
        catch (e_1) {
            env_1.error = e_1;
    ...
    (0, bun_test_1.it)("rejects project/global skill file reads on disk", async () => {
        ...
        try {
            const tempDir = __addDisposableResource(env_2, new testHelpers_1.TestTempDir("test-agent-skill-read-file-mux-chat-reject"), false);
