...
(0, bun_test_1.describe)("createCodeExecutionTool", () => {
    ...
    (0, bun_test_1.describe)("static analysis", () => {
        ...
        (0, bun_test_1.it)("does not reject 'require(' inside string literals", async () => {
            const tool = await (0, code_execution_1.createCodeExecutionTool)(runtimeFactory, new toolBridge_1.ToolBridge({}));
            ...
            (0, bun_test_1.expect)(result.result).toContain("require(");
        });
        (0, bun_test_1.it)("does not reject 'import(' inside string literals", async () => {
            const tool = await (0, code_execution_1.createCodeExecutionTool)(runtimeFactory, new toolBridge_1.ToolBridge({}));
            const result = (await tool.execute({ code: 'return `this is a template string containing import("fs")`' }, mockToolCallOptions));
            (0, bun_test_1.expect)(result.success).toBe(true);
            (0, bun_test_1.expect)(result.result).toContain("import(");
        });
        (0, bun_test_1.it)("rejects code using dynamic import()", async () => {
            const tool = await (0, code_execution_1.createCodeExecutionTool)(runtimeFactory, new toolBridge_1.ToolBridge({}));
            const result = (await tool.execute({ code: 'return import("fs")' }, mockToolCallOptions));
            (0, bun_test_1.expect)(result.success).toBe(false);
            (0, bun_test_1.expect)(result.error).toContain("Code analysis failed");
            (0, bun_test_1.expect)(result.error).toContain("Dynamic import() is not available");
        });
