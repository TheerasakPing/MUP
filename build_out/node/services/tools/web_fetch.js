...
function isAllowedMuxMdHost(url) {
    try {
        return (0, muxMd_1.getMuxMdAllowedHosts)().includes(new URL(url).host);
    }
    catch {
        return false;
    }
    ...
    return (0, ai_1.tool)({
        ...
        execute: async ({ url }, { abortSignal }) => {
            try {
                ...
                if ((0, muxMd_1.isMuxMdUrl)(url)) {
                    const muxMdParsed = (0, muxMd_1.parseMuxMdUrl)(url);
                    if (!muxMdParsed) {
                        return { success: false, error: "Invalid mux.md URL format" };
                    }
                    const baseUrl = new URL(url).origin;
                    try {
                        const result = await (0, muxMd_1.downloadFromMuxMd)(muxMdParsed.id, muxMdParsed.key, abortSignal, {
                            baseUrl,
                        });
