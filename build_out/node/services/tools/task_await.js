        ...
        execute: async (args, { abortSignal }) => {
            ...
            if (!requestedIds && config.backgroundProcessManager) {
                ...
                for (const proc of processes) {
                    ...
                    if (!inScope)
                        continue;
                    bashTaskIds.push((0, taskId_1.toBashTaskId)(proc.id));
                }
            ...
            const readGitFormatPatchArtifact = async (childTaskId) => {
                if (!config.workspaceSessionDir)
                    return null;
                return await (0, subagentGitPatchArtifacts_1.readSubagentGitPatchArtifact)(config.workspaceSessionDir, childTaskId);
            };
            const descendantAgentTaskIds = typeof bulkFilter === "function"
                ? await bulkFilter.call(taskService, workspaceId, agentTaskIds)
                : (await Promise.all(agentTaskIds.map(async (taskId) => (await taskService.isDescendantAgentTask(workspaceId, taskId)) ? taskId : null))).filter((taskId) => typeof taskId === "string");
            ...
            const results = await Promise.all(uniqueTaskIds.map(async (taskId) => {
                ...
                if (maybeProcessId) {
                    ...
                    const outputResult = await config.backgroundProcessManager.getOutput(maybeProcessId, args.filter ?? undefined, args.filter_exclude ?? undefined, timeoutSecsForBash, abortSignal, workspaceId, "task_await");
