...
(0, bun_test_1.describe)("file_edit_insert tool", () => {
    ...
    (0, bun_test_1.it)("inserts content using insert_after guard", async () => {
    ...
    (0, bun_test_1.it)("inserts content using insert_after guard when file uses CRLF", async () => {
        await fs.writeFile(testFilePath, "Line 1\r\nLine 3\r\n");
        const tool = createTestTool(testDir);
        const args = {
            file_path: path.relative(testDir, testFilePath),
    ...
    (0, bun_test_1.it)("inserts content using insert_before guard", async () => {
        const tool = createTestTool(testDir);
        const args = {
            file_path: path.relative(testDir, testFilePath),
            content: "Header\n",
    ...
    (0, bun_test_1.it)("fails when both insert_before and insert_after are provided", async () => {
        const tool = createTestTool(testDir);
        const args = {
            file_path: path.relative(testDir, testFilePath),
            content: "oops",
        ...
        (0, bun_test_1.expect)(result.success).toBe(false);
        if (!result.success) {
            (0, bun_test_1.expect)(result.error).toContain("only one of insert_before or insert_after");
        }
    });
    ...
    (0, bun_test_1.it)("fails when no guards are provided", async () => {
        ...
        (0, bun_test_1.expect)(result.success).toBe(false);
        if (!result.success) {
            (0, bun_test_1.expect)(result.error).toContain("Provide either insert_before or insert_after guard");
        }
    });
});
(0, bun_test_1.describe)("file_edit_insert plan mode enforcement", () => {
    ...
    (0, bun_test_1.it)("blocks creating .mux/plan.md (wrong path) when real plan file is elsewhere", async () => {
        ...
        if (!result.success) {
            ...
            (0, bun_test_1.expect)(result.error).toContain(".mux/plan.md");
        }
