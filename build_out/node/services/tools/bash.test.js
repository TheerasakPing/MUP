...
(0, bun_test_1.describe)("bash tool", () => {
    ...
    (0, bun_test_1.it)("should warn when using cat to read a file", async () => {
        const env_3 = { stack: [], error: void 0, hasError: false };
        try {
            const tempDir = __addDisposableResource(env_3, new testHelpers_1.TestTempDir("test-bash-cat-read"), false);
            ...
            if (isForegroundSuccess(result)) {
                (0, bun_test_1.expect)(result.output).toBe("hello\nworld");
                (0, bun_test_1.expect)(result.note).toContain("DO NOT use `cat`");
                (0, bun_test_1.expect)(result.note).toContain("file_read");
            }
        }
        catch (e_3) {
            env_3.error = e_3;
    ...
    (0, bun_test_1.it)("should not warn on cat heredoc", async () => {
        const env_4 = { stack: [], error: void 0, hasError: false };
        try {
            const tempDir = __addDisposableResource(env_4, new testHelpers_1.TestTempDir("test-bash-cat-heredoc"), false);
            const config = (0, testHelpers_1.createTestToolConfig)(tempDir.path);
            config.runtimeTempDir = tempDir.path;
            const tool = (0, bash_1.createBashTool)(config);
            const args = {
            ...
            if (isForegroundSuccess(result)) {
                (0, bun_test_1.expect)(result.output).toBe("hello");
                (0, bun_test_1.expect)(result.note).toBeUndefined();
            }
        }
        catch (e_4) {
            env_4.error = e_4;
    ...
    (0, bun_test_1.it)("should warn when using grep to dump a file", async () => {
        const env_5 = { stack: [], error: void 0, hasError: false };
        try {
            const tempDir = __addDisposableResource(env_5, new testHelpers_1.TestTempDir("test-bash-grep-read"), false);
            const filePath = path.join(tempDir.path, "input.txt");
            fs.writeFileSync(filePath, "hello\nworld", "utf-8");
            const config = (0, testHelpers_1.createTestToolConfig)(tempDir.path);
            config.runtimeTempDir = tempDir.path;
            ...
            if (isForegroundSuccess(result)) {
                (0, bun_test_1.expect)(result.output).toBe("hello\nworld");
                (0, bun_test_1.expect)(result.note).toContain("file_read");
                (0, bun_test_1.expect)(result.note).toContain("`grep`");
            }
        }
        catch (e_5) {
            env_5.error = e_5;
    ...
    (0, bun_test_1.it)("should not warn on grep search", async () => {
        const env_6 = { stack: [], error: void 0, hasError: false };
        try {
            const tempDir = __addDisposableResource(env_6, new testHelpers_1.TestTempDir("test-bash-grep-search"), false);
            const filePath = path.join(tempDir.path, "input.txt");
            fs.writeFileSync(filePath, "hello\nworld", "utf-8");
            const config = (0, testHelpers_1.createTestToolConfig)(tempDir.path);
            config.runtimeTempDir = tempDir.path;
    ...
    (0, bun_test_1.it)("should warn when using rg to dump a file", async () => {
        const env_7 = { stack: [], error: void 0, hasError: false };
        try {
            const tempDir = __addDisposableResource(env_7, new testHelpers_1.TestTempDir("test-bash-rg-read"), false);
            const filePath = path.join(tempDir.path, "input.txt");
            fs.writeFileSync(filePath, "hello\nworld", "utf-8");
            const config = (0, testHelpers_1.createTestToolConfig)(tempDir.path);
            config.runtimeTempDir = tempDir.path;
            ...
            if (isForegroundSuccess(result)) {
                (0, bun_test_1.expect)(result.output).toBe("hello\nworld");
                (0, bun_test_1.expect)(result.note).toContain("file_read");
                (0, bun_test_1.expect)(result.note).toContain("`rg`");
            }
        }
        catch (e_7) {
            env_7.error = e_7;
    ...
    (0, bun_test_1.it)("should not warn on rg search", async () => {
        const env_8 = { stack: [], error: void 0, hasError: false };
        try {
            const tempDir = __addDisposableResource(env_8, new testHelpers_1.TestTempDir("test-bash-rg-search"), false);
            const filePath = path.join(tempDir.path, "input.txt");
            fs.writeFileSync(filePath, "hello\nworld", "utf-8");
            const config = (0, testHelpers_1.createTestToolConfig)(tempDir.path);
            config.runtimeTempDir = tempDir.path;
    ...
    (0, bun_test_1.it)("should warn on cat file reads even when output overflows", async () => {
        const env_9 = { stack: [], error: void 0, hasError: false };
        try {
            const tempDir = __addDisposableResource(env_9, new testHelpers_1.TestTempDir("test-bash-cat-overflow"), false);
    ...
    (0, bun_test_1.describe)("script sanitization", () => {
        const shouldRewriteNullRedirects = process.platform === "win32";
        (0, bun_test_1.it)("should rewrite >nul to /dev/null (and not create a `nul` file)", async () => {
            const env_30 = { stack: [], error: void 0, hasError: false };
            try {
                const tempDir = __addDisposableResource(env_30, new testHelpers_1.TestTempDir("test-bash-nul-redirect"), false);
                const tool = (0, bash_1.createBashTool)((0, testHelpers_1.createTestToolConfig)(tempDir.path));
                const args = {
                ...
                if (isForegroundSuccess(result)) {
                    if (shouldRewriteNullRedirects) {
                        (0, bun_test_1.expect)(result.note).toContain("Rewrote `>nul`/`2>nul`");
                    }
                    else {
                        (0, bun_test_1.expect)(result.note).toBeUndefined();
                    }
                }
                (0, bun_test_1.expect)(fs.existsSync(path.join(tempDir.path, "nul"))).toBe(!shouldRewriteNullRedirects);
            }
            catch (e_30) {
                env_30.error = e_30;
        ...
        (0, bun_test_1.it)("should rewrite 2>nul to /dev/null (and not create a `nul` file)", async () => {
            const env_31 = { stack: [], error: void 0, hasError: false };
            try {
                const tempDir = __addDisposableResource(env_31, new testHelpers_1.TestTempDir("test-bash-nul-redirect-stderr"), false);
                const tool = (0, bash_1.createBashTool)((0, testHelpers_1.createTestToolConfig)(tempDir.path));
                const args = {
        ...
        (0, bun_test_1.it)("should not rewrite an explicit path like >./nul", async () => {
            const env_32 = { stack: [], error: void 0, hasError: false };
            try {
                const tempDir = __addDisposableResource(env_32, new testHelpers_1.TestTempDir("test-bash-dot-nul"), false);
                const tool = (0, bash_1.createBashTool)((0, testHelpers_1.createTestToolConfig)(tempDir.path));
                const args = {
                    script: "echo hello >./nul",
                    timeout_secs: 5,
                ...
                (0, bun_test_1.expect)(fs.existsSync(path.join(tempDir.path, "nul"))).toBe(true);
            }
            catch (e_32) {
                env_32.error = e_32;
...
(0, bun_test_1.describe)("muxEnv environment variables", () => {
    (0, bun_test_1.it)("should inject MUX_ environment variables when muxEnv is provided", async () => {
        const env_43 = { stack: [], error: void 0, hasError: false };
        try {
            const tempDir = __addDisposableResource(env_43, new testHelpers_1.TestTempDir("test-mux-env"), false);
            const config = (0, testHelpers_1.createTestToolConfig)(process.cwd());
            config.runtimeTempDir = tempDir.path;
            config.muxEnv = {
    ...
    (0, bun_test_1.it)("should allow secrets to override muxEnv", async () => {
        ...
        try {
            const tempDir = __addDisposableResource(env_44, new testHelpers_1.TestTempDir("test-mux-env-override"), false);
            const config = (0, testHelpers_1.createTestToolConfig)(process.cwd());
            config.runtimeTempDir = tempDir.path;
            config.muxEnv = {
            ...
            if (result.success) {
                ...
                (0, bun_test_1.expect)(result.output).toContain("MUX:/mux/path");
...
(0, bun_test_1.describe)("bash tool - tool_env", () => {
    (0, bun_test_1.it)("should source .mux/tool_env before running script", async () => {
        const env_46 = { stack: [], error: void 0, hasError: false };
        try {
            const tempDir = __addDisposableResource(env_46, new testHelpers_1.TestTempDir("test-bash-tool-env"), false);
    ...
    (0, bun_test_1.it)("should fail with clear error if tool_env sourcing fails", async () => {
        ...
        try {
            const tempDir = __addDisposableResource(env_47, new testHelpers_1.TestTempDir("test-bash-tool-env-fail"), false);
            const muxDir = `${tempDir.path}/.mux`;
            fs.mkdirSync(muxDir, { recursive: true });
    ...
    (0, bun_test_1.it)("should run script normally when no tool_env exists", async () => {
        ...
        try {
            const tempDir = __addDisposableResource(env_48, new testHelpers_1.TestTempDir("test-bash-no-tool-env"), false);
            const config = (0, testHelpers_1.createTestToolConfig)(tempDir.path);
            config.runtimeTempDir = tempDir.path;
            const tool = (0, bash_1.createBashTool)(config);
            const args = {
...
(0, bun_test_1.describe)("bash tool - background execution", () => {
    ...
    (0, bun_test_1.it)("should accept timeout with background mode for auto-termination", async () => {
        const manager = new backgroundProcessManager_1.BackgroundProcessManager("/tmp/mux-test-bg");
    ...
    (0, bun_test_1.it)("should inject muxEnv environment variables in background mode", async () => {
        const manager = new backgroundProcessManager_1.BackgroundProcessManager("/tmp/mux-test-bg");
        const tempDir = new testHelpers_1.TestTempDir("test-bash-bg-mux-env");
        const config = (0, testHelpers_1.createTestToolConfig)(tempDir.path);
        config.backgroundProcessManager = manager;
        config.muxEnv = {
