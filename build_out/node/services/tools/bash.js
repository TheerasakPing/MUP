...
function detectCatFileRead(script) {
    ...
    for (const segment of segments) {
        ...
        if (args.length === 0)
            continue; // "cat" (stdin passthrough)
...
function getGrepCommandTokenIndex(tokens) {
    if (tokens.length === 0) {
        return null;
    }
...
function parseSearchCommandArgs(command, args) {
    let pattern = null;
    const tokensAfterPattern = [];
    ...
    for (let i = 0; i < args.length; i++) {
        ...
        if (pattern === null) {
            ...
            if (isFlag) {
                const patternFlagInline = command === "rg" && token.startsWith("--regexp=")
                ...
                const isExplicitPatternFlag = (command === "rg" && (token === "-e" || token === "--regexp")) ||
                ...
                const consumesNextArg = (command === "rg" && rgConsumesNextArg.has(token)) ||
...
function hasFileReadTargetToken(tokensAfterPattern) {
    let expectInputFile = false;
    let skipNextOutputTarget = false;
...
function detectRipgrepFileDump(script) {
...
function detectGrepFileDump(script) {
...
function isForegroundBashToolResult(result) {
    return !("backgroundProcessId" in result);
}
function addNoticeToBashToolResult(result, notice) {
 ...
 */
function rewriteWindowsNullRedirects(script) {
 ...
 */
function createLineHandler(lines, totalBytesRef, limits, state, triggerDisplayTruncation, triggerFileTruncation) {
    return (line) => {
        if (state.fileTruncated)
            return;
        const lineBytes = Buffer.byteLength(line, "utf-8");
        // Check if line exceeds per-line limit (hard stop - likely corrupt data)
        if (lineBytes > limits.maxLineBytes) {
            triggerFileTruncation(`Line ${lines.length + 1} exceeded per-line limit: ${lineBytes} bytes > ${limits.maxLineBytes} bytes`);
            return;
        }
        ...
        if (bytesAfterLine > limits.maxFileBytes) {
            triggerFileTruncation(`Total output would exceed file preservation limit: ${bytesAfterLine} bytes > ${limits.maxFileBytes} bytes (at line ${lines.length + 1})`);
            return;
        }
        ...
        if (!state.displayTruncated) {
            if (totalBytesRef.current > limits.maxTotalBytes) {
                triggerDisplayTruncation(`Total output exceeded display limit: ${totalBytesRef.current} bytes > ${limits.maxTotalBytes} bytes (at line ${lines.length})`);
                return;
            }
            if (lines.length >= limits.maxLines) {
                triggerDisplayTruncation(`Line count exceeded display limit: ${lines.length} lines >= ${limits.maxLines} lines (${totalBytesRef.current} bytes read)`);
            }
        }
    };
}
 ...
 */
function formatResult(exitCode, lines, truncated, overflowReason, wall_duration_ms, overflowPolicy, effectiveTimeout) {
    const output = lines.join("\n");
    // Check for special error codes from runtime
    if (exitCode === exitCodes_1.EXIT_CODE_ABORTED) {
        return {
            success: false,
    ...
    if (exitCode === exitCodes_1.EXIT_CODE_TIMEOUT) {
        return {
            success: false,
 ...
 */
function buildToolEnvPrelude(toolEnvPath) {
    if (!toolEnvPath)
        return "";
...
const createBashTool = (config) => {
    ...
    const maxTotalBytes = overflowPolicy === "truncate" ? toolLimits_1.BASH_TRUNCATE_MAX_TOTAL_BYTES : toolLimits_1.BASH_MAX_TOTAL_BYTES;
    const maxFileBytes = overflowPolicy === "truncate" ? toolLimits_1.BASH_TRUNCATE_MAX_FILE_BYTES : toolLimits_1.BASH_MAX_FILE_BYTES;
    const maxLines = overflowPolicy === "truncate" ? Infinity : toolLimits_1.BASH_HARD_MAX_LINES;
    const maxLineBytes = overflowPolicy === "truncate" ? Infinity : toolLimits_1.BASH_MAX_LINE_BYTES;
    return (0, ai_1.tool)({
        ...
        execute: async ({ script, timeout_secs, run_in_background, display_name }, { abortSignal, toolCallId }) => {
            // Validate script input
            // Treat display_name as untrusted input: it ends up in filesystem paths.
            const safeDisplayName = (0, bashDisplayName_1.resolveBashDisplayName)(script, display_name);
            ...
            const fileReadNotice = detectCatFileRead(script) || detectRipgrepFileDump(script) || detectGrepFileDump(script)
                ? CAT_FILE_READ_NOTICE
                : undefined;
            // Look up .mux/tool_env to source before script (for direnv, nvm, venv, etc.)
            const toolEnvPath = config.runtime ? await (0, hooks_1.getToolEnvPath)(config.runtime, config.cwd) : null;
            ...
            const shouldRewriteNullRedirects = process.platform === "win32" && config.runtime instanceof LocalBaseRuntime_1.LocalBaseRuntime;
            ...
            const withNotice = (result) => addNoticeToBashToolResult(addNoticeToBashToolResult(result, nulRedirectNote), fileReadNotice);
            if (run_in_background) {
                if (!config.workspaceId || !config.backgroundProcessManager || !config.runtime) {
                    return withNotice({
                        success: false,
                        error: "Background execution is only available for AI tool calls, not direct IPC invocation",
                        exitCode: -1,
                        wall_duration_ms: 0,
                    });
                }
                const startTime = performance.now();
                const spawnResult = await config.backgroundProcessManager.spawn(config.runtime, config.workspaceId, scriptWithEnv, {
                    cwd: config.cwd,
                ...
                return withNotice({
                    ...
                    exitCode: 0,
                    wall_duration_ms: Math.round(performance.now() - startTime),
                    taskId: (0, taskId_1.toBashTaskId)(spawnResult.processId),
                    backgroundProcessId: spawnResult.processId,
                });
            }
            // Setup execution parameters
            const effectiveTimeout = timeout_secs ?? toolLimits_1.BASH_DEFAULT_TIMEOUT_SECS;
            const startTime = performance.now();
            ...
            const backgroundedRef = { current: false };
            let foregroundCompleted = false;
            let backgroundResolve = null;
            ...
            const fgRegistration = config.backgroundProcessManager && config.workspaceId && toolCallId
                ? config.backgroundProcessManager.registerForegroundProcess(config.workspaceId, toolCallId, script, safeDisplayName, () => {
...
${scriptWithEnv}`;
            const execStream = await config.runtime.exec(scriptWithClosedStdin, {
                cwd: config.cwd,
                env: { ...config.muxEnv, ...config.secrets, ...env_1.NON_INTERACTIVE_ENV_VARS },
            ...
            const triggerDisplayTruncation = (reason) => {
            ...
            const triggerFileTruncation = (reason) => {
            ...
            const lineHandler = createLineHandler(lines, totalBytesRef, { maxLineBytes, maxFileBytes, maxTotalBytes, maxLines }, truncationState, triggerDisplayTruncation, triggerFileTruncation);
            ...
            const emitBashOutput = (isError, text) => {
                if (!config.emitChatEvent || !config.workspaceId || !toolCallId)
                    return;
                if (liveOutputStopped)
                    return;
                if (text.length === 0)
                    return;
            ...
            const appendLiveOutput = (isError, text) => {
                if (!config.emitChatEvent || !config.workspaceId || !toolCallId)
                    return;
                if (liveOutputStopped)
                    return;
                ...
                if (isError) {
                    ...
                    if (liveStderrBuffer.length >= MAX_LIVE_EVENT_CHARS)
                        flushLiveOutput();
                }
                else {
                    liveStdoutBuffer += text;
                    if (liveStdoutBuffer.length >= MAX_LIVE_EVENT_CHARS)
                        flushLiveOutput();
                }
            };
            ...
            const consumeStream = async (stream, isError) => {
            ...
            try {
                const result = await Promise.race([
                    ...
                    backgroundPromise.then(() => "backgrounded"),
                ]);
                const shouldBackground = result === "backgrounded" || (backgroundedRef.current && !foregroundCompleted);
                // If the process already exited, drain the foreground streams for reliable output
                // instead of backgrounding based on timing.
                if (shouldBackground) {
                    const didExit = exitCodeResolved ||
                        (await Promise.race([
                            execStream.exitCode.then(() => true).catch(() => true),
                            new Promise((resolve) => setTimeout(() => resolve(false), BACKGROUND_EXIT_GRACE_MS)),
                        ]));
                    if (didExit) {
                        ...
                        if (config.backgroundProcessManager && config.workspaceId) {
                            const processId = config.backgroundProcessManager.generateUniqueProcessId(safeDisplayName);
                            ...
                            const migrateResult = await (0, backgroundProcessExecutor_1.migrateToBackground)(migrationStream, {
                                cwd: config.cwd,
                                workspaceId: config.workspaceId,
                                processId,
                                script,
                            ...
                            if (migrateResult.success) {
                                ...
                                config.backgroundProcessManager.registerMigratedProcess(migrateResult.handle, processId, config.workspaceId, script, migrateResult.outputDir, safeDisplayName);
                                return withNotice({
                                    success: true,
                                    output: `Process sent to background with ID: ${processId}\n\nOutput so far (${lines.length} lines):\n${lines.slice(-20).join("\n")}${lines.length > 20 ? "\n...(showing last 20 lines)" : ""}`,
                                    exitCode: 0,
                                    wall_duration_ms,
                                    taskId: (0, taskId_1.toBashTaskId)(processId),
...
File will be automatically cleaned up when stream ends.`;
            ...
            return withNotice(formatResult(exitCode, lines, truncated, overflowReason, wall_duration_ms, config.overflow_policy ?? "tmpfile", effectiveTimeout));
        },
    });
};
