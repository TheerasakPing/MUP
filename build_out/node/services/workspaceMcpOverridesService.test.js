...
(0, bun_test_1.describe)("WorkspaceMcpOverridesService", () => {
    ...
    (0, bun_test_1.it)("returns empty overrides when no file and no legacy config", async () => {
        ...
        (0, bun_test_1.expect)(await pathExists(path.join(workspacePath, ".mux", "mcp.local.jsonc"))).toBe(false);
    });
    (0, bun_test_1.it)("adds .mux/mcp.local.jsonc to git exclude when writing overrides", async () => {
        const projectPath = "/fake/project";
        const workspaceId = "ws-id";
        const workspaceName = "branch";
        const workspacePath = getWorkspacePath({
        ...
        (0, bun_test_1.expect)(before).not.toContain(".mux/mcp.local.jsonc");
        ...
        (0, bun_test_1.expect)(after).toContain(".mux/mcp.local.jsonc");
    });
    (0, bun_test_1.it)("persists overrides to .mux/mcp.local.jsonc and reads them back", async () => {
        const projectPath = "/fake/project";
        const workspaceId = "ws-id";
        const workspaceName = "branch";
        const workspacePath = getWorkspacePath({
