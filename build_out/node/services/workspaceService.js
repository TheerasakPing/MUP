...
const devcontainerCli_1 = require("../../node/runtime/devcontainerCli");
...
const subagentTranscriptArtifacts_1 = require("../../node/services/subagentTranscriptArtifacts");
...
function getNextCompactionEpochForAppendBoundary(workspaceId, messages) {
    let epochCursor = 0;
...
async function copyFileBestEffort(params) {
    try {
        ...
        log_1.log.error("Failed to copy session artifact file", {
            ...params.logContext,
...
async function copyDirIfMissingBestEffort(params) {
    try {
        ...
        log_1.log.error("Failed to copy session artifact directory", {
            ...params.logContext,
...
function coerceUpdatedAtMs(entry) {
...
async function archiveChildSessionArtifactsIntoParentSessionDir(params) {
    ...
    try {
        ...
        const archivedChatPath = (0, subagentTranscriptArtifacts_1.getSubagentTranscriptChatPath)(params.parentSessionDir, params.childWorkspaceId);
        const archivedPartialPath = (0, subagentTranscriptArtifacts_1.getSubagentTranscriptPartialPath)(params.parentSessionDir, params.childWorkspaceId);
        // Defensive: avoid path traversal in workspace IDs.
        if (!isPathInsideDir(params.parentSessionDir, archivedChatPath)) {
            log_1.log.error("Refusing to archive session transcript outside parent session dir", {
            ...
            if (didCopyChat || didCopyPartial) {
                ...
                const model = typeof params.childTaskModelString === "string" &&
                    params.childTaskModelString.trim().length > 0
                    ? params.childTaskModelString.trim()
                    : undefined;
                const thinkingLevel = (0, thinking_1.coerceThinkingLevel)(params.childTaskThinkingLevel);
                await (0, subagentTranscriptArtifacts_1.upsertSubagentTranscriptArtifactIndexEntry)({
        ...
        log_1.log.error("Failed to archive child transcript into parent session dir", {
            parentWorkspaceId: params.parentWorkspaceId,
            childWorkspaceId: params.childWorkspaceId,
            error: error instanceof Error ? error.message : String(error),
        });
    ...
    try {
        const childArtifacts = await (0, subagentGitPatchArtifacts_1.readSubagentGitPatchArtifactsFile)(params.childSessionDir);
        const childEntries = Object.entries(childArtifacts.artifactsByChildTaskId);
        for (const [taskId] of childEntries) {
            if (!taskId)
                continue;
            const srcDir = path.dirname((0, subagentGitPatchArtifacts_1.getSubagentGitPatchMboxPath)(params.childSessionDir, taskId));
            const destDir = path.dirname((0, subagentGitPatchArtifacts_1.getSubagentGitPatchMboxPath)(params.parentSessionDir, taskId));
            if (!isPathInsideDir(params.childSessionDir, srcDir)) {
                log_1.log.error("Refusing to roll up patch artifact outside child session dir", {
                    parentWorkspaceId: params.parentWorkspaceId,
                    childWorkspaceId: params.childWorkspaceId,
                    taskId,
            ...
            if (!isPathInsideDir(params.parentSessionDir, destDir)) {
                log_1.log.error("Refusing to roll up patch artifact outside parent session dir", {
                    parentWorkspaceId: params.parentWorkspaceId,
                    childWorkspaceId: params.childWorkspaceId,
                    taskId,
                    parentSessionDir: params.parentSessionDir,
        ...
        if (childEntries.length > 0) {
            await (0, subagentGitPatchArtifacts_1.updateSubagentGitPatchArtifactsFile)({
                workspaceId: params.parentWorkspaceId,
                workspaceSessionDir: params.parentSessionDir,
                update: (parentFile) => {
                    for (const [taskId, childEntry] of childEntries) {
                        ...
                        if (!existing || childUpdated > existingUpdated) {
                            parentFile.artifactsByChildTaskId[taskId] = {
                                ...
                                childTaskId: taskId,
                                parentWorkspaceId: params.parentWorkspaceId,
                                mboxPath: (0, subagentGitPatchArtifacts_1.getSubagentGitPatchMboxPath)(params.parentSessionDir, taskId),
                            };
                        }
                    }
                },
        ...
        log_1.log.error("Failed to roll up subagent patch artifacts into parent", {
            parentWorkspaceId: params.parentWorkspaceId,
            childWorkspaceId: params.childWorkspaceId,
            error: error instanceof Error ? error.message : String(error),
        });
    ...
    try {
        const childArtifacts = await (0, subagentReportArtifacts_1.readSubagentReportArtifactsFile)(params.childSessionDir);
        const childEntries = Object.entries(childArtifacts.artifactsByChildTaskId);
        for (const [taskId] of childEntries) {
            if (!taskId)
                continue;
            const srcDir = path.dirname((0, subagentReportArtifacts_1.getSubagentReportArtifactPath)(params.childSessionDir, taskId));
            const destDir = path.dirname((0, subagentReportArtifacts_1.getSubagentReportArtifactPath)(params.parentSessionDir, taskId));
            if (!isPathInsideDir(params.childSessionDir, srcDir)) {
                log_1.log.error("Refusing to roll up report artifact outside child session dir", {
                    parentWorkspaceId: params.parentWorkspaceId,
                    childWorkspaceId: params.childWorkspaceId,
                    taskId,
                    childSessionDir: params.childSessionDir,
            ...
            if (!isPathInsideDir(params.parentSessionDir, destDir)) {
                log_1.log.error("Refusing to roll up report artifact outside parent session dir", {
                    parentWorkspaceId: params.parentWorkspaceId,
                    childWorkspaceId: params.childWorkspaceId,
                    taskId,
                    parentSessionDir: params.parentSessionDir,
        ...
        if (childEntries.length > 0) {
            await (0, subagentReportArtifacts_1.updateSubagentReportArtifactsFile)({
                workspaceId: params.parentWorkspaceId,
                workspaceSessionDir: params.parentSessionDir,
                update: (parentFile) => {
                    for (const [taskId, childEntry] of childEntries) {
        ...
        log_1.log.error("Failed to roll up subagent report artifacts into parent", {
            parentWorkspaceId: params.parentWorkspaceId,
            childWorkspaceId: params.childWorkspaceId,
            error: error instanceof Error ? error.message : String(error),
        });
    ...
    try {
        const childArtifacts = await (0, subagentTranscriptArtifacts_1.readSubagentTranscriptArtifactsFile)(params.childSessionDir);
        const childEntries = Object.entries(childArtifacts.artifactsByChildTaskId);
        for (const [taskId] of childEntries) {
            if (!taskId)
                continue;
            const srcDir = path.dirname((0, subagentTranscriptArtifacts_1.getSubagentTranscriptChatPath)(params.childSessionDir, taskId));
            const destDir = path.dirname((0, subagentTranscriptArtifacts_1.getSubagentTranscriptChatPath)(params.parentSessionDir, taskId));
            if (!isPathInsideDir(params.childSessionDir, srcDir)) {
                log_1.log.error("Refusing to roll up transcript artifact outside child session dir", {
                    parentWorkspaceId: params.parentWorkspaceId,
                    childWorkspaceId: params.childWorkspaceId,
                    taskId,
                    childSessionDir: params.childSessionDir,
            ...
            if (!isPathInsideDir(params.parentSessionDir, destDir)) {
                log_1.log.error("Refusing to roll up transcript artifact outside parent session dir", {
                    parentWorkspaceId: params.parentWorkspaceId,
                    childWorkspaceId: params.childWorkspaceId,
                    taskId,
                    parentSessionDir: params.parentSessionDir,
        ...
        if (childEntries.length > 0) {
            await (0, subagentTranscriptArtifacts_1.updateSubagentTranscriptArtifactsFile)({
                workspaceId: params.parentWorkspaceId,
                workspaceSessionDir: params.parentSessionDir,
                update: (parentFile) => {
                    for (const [taskId, childEntry] of childEntries) {
                        ...
                        if (!existing || childUpdated > existingUpdated) {
                            parentFile.artifactsByChildTaskId[taskId] = {
                                ...
                                chatPath: childEntry.chatPath
                                    ? (0, subagentTranscriptArtifacts_1.getSubagentTranscriptChatPath)(params.parentSessionDir, taskId)
                                    : undefined,
                                partialPath: childEntry.partialPath
                                    ? (0, subagentTranscriptArtifacts_1.getSubagentTranscriptPartialPath)(params.parentSessionDir, taskId)
                                    : undefined,
                            };
                        }
                    }
        ...
        log_1.log.error("Failed to roll up subagent transcript artifacts into parent", {
            parentWorkspaceId: params.parentWorkspaceId,
            childWorkspaceId: params.childWorkspaceId,
            error: error instanceof Error ? error.message : String(error),
        });
...
async function forEachWithConcurrencyLimit(items, limit, fn) {
    (0, assert_1.default)(Number.isInteger(limit) && limit > 0, "Concurrency limit must be a positive integer");
    let nextIndex = 0;
...
class WorkspaceService extends events_1.EventEmitter {
    ...
    initAbortControllers = new Map();
    /** Check if a workspace is currently being removed. */
    isRemoving(workspaceId) {
        return this.removingWorkspaces.has(workspaceId);
    }
    constructor(config, historyService, partialService, aiService, initStateManager, extensionMetadata, backgroundProcessManager, sessionUsageService, costTrackingService, policyService, telemetryService, experimentsService, sessionTimingService) {
        super();
        this.config = config;
    ...
    setWorkspaceLifecycleHooks(hooks) {
     ...
     */
    debugTriggerStreamError(workspaceId, errorMessage) {
    ...
    setupMetadataListeners() {
        ...
        const isWorkspaceEvent = (v) => isObj(v) && "workspaceId" in v && typeof v.workspaceId === "string";
        const isStreamStartEvent = (v) => isWorkspaceEvent(v) && "model" in v && typeof v.model === "string";
        const isStreamEndEvent = (v) => isWorkspaceEvent(v) &&
            (!("metadata" in v) || isObj(v.metadata));
        const isStreamAbortEvent = (v) => isWorkspaceEvent(v);
    ...
    setupInitMetadataListeners() {
        ...
        this.initStateManager.on("init-end", (event) => {
    ...
    emitWorkspaceActivity(workspaceId, snapshot) {
        this.emit("activity", { workspaceId, activity: snapshot });
    }
    async updateRecencyTimestamp(workspaceId, timestamp) {
        try {
            const snapshot = await this.extensionMetadata.updateRecency(workspaceId, timestamp ?? Date.now());
            this.emitWorkspaceActivity(workspaceId, snapshot);
        }
        catch (error) {
            log_1.log.error("Failed to update workspace recency", { workspaceId, error });
        }
    }
    async updateStreamingStatus(workspaceId, streaming, model, agentId) {
        try {
            let thinkingLevel;
            if (model) {
                ...
                if (found) {
                    ...
                    const normalizedAgentId = typeof agentId === "string" && agentId.trim().length > 0
                        ...
                        : workspaceDefaults_1.WORKSPACE_DEFAULTS.agentId;
                    const aiSettings = workspace?.aiSettingsByAgent?.[normalizedAgentId] ?? workspace?.aiSettings;
                    thinkingLevel = aiSettings?.thinkingLevel;
                }
            }
            const snapshot = await this.extensionMetadata.setStreaming(workspaceId, streaming, model, thinkingLevel);
            this.emitWorkspaceActivity(workspaceId, snapshot);
        }
        catch (error) {
            log_1.log.error("Failed to update workspace streaming status", { workspaceId, error });
        }
    }
    async handleStreamCompletion(workspaceId, timestamp) {
        await this.updateRecencyTimestamp(workspaceId, timestamp);
        await this.updateStreamingStatus(workspaceId, false);
    }
    createInitLogger(workspaceId) {
    ...
    async emitPostCompactionMetadata(workspaceId) {
        try {
            ...
            log_1.log.debug("Failed to emit post-compaction metadata", { workspaceId, error });
        }
    }
    getOrCreateSession(workspaceId) {
        ...
        session = new agentSession_1.AgentSession({
            workspaceId: trimmed,
            config: this.config,
     ...
     */
    registerSession(workspaceId, session) {
        workspaceId = workspaceId.trim();
        (0, assert_1.default)(workspaceId.length > 0, "workspaceId must not be empty");
        (0, assert_1.default)(!this.sessions.has(workspaceId), `session already registered for ${workspaceId}`);
    ...
    async getPersistedPostCompactionDiffPaths(workspaceId) {
        const postCompactionPath = path.join(this.config.getSessionDir(workspaceId), "post-compaction.json");
        try {
            const raw = await fsPromises.readFile(postCompactionPath, "utf-8");
            const parsed = JSON.parse(raw);
            const diffsRaw = parsed.diffs;
     ...
     */
    async getPostCompactionState(workspaceId) {
        ...
        const planPath = (0, planStorage_1.getPlanFilePath)(metadata.name, metadata.projectName, muxHome);
        ...
        const expandedPlanPath = muxHome.startsWith("~") ? (0, tildeExpansion_1.expandTilde)(planPath) : planPath;
        ...
        const expandedLegacyPlanPath = (0, tildeExpansion_1.expandTilde)(legacyPlanPath);
        // Check both new and legacy plan paths, prefer new path
        const newPlanExists = await (0, fileExists_1.fileExists)(runtime, planPath);
        const legacyPlanExists = !newPlanExists && (await (0, fileExists_1.fileExists)(runtime, legacyPlanPath));
        ...
        const allPaths = (0, extractEditedFiles_1.extractEditedFilePaths)(messages);
     ...
     */
    async getPostCompactionExclusions(workspaceId) {
     ...
     */
    async setPostCompactionExclusion(workspaceId, itemId, excluded) {
        try {
            const exclusions = await this.getPostCompactionExclusions(workspaceId);
            ...
            await fsPromises.writeFile(exclusionsPath, JSON.stringify({ excludedItems: [...set] }, null, 2));
            return (0, result_1.Ok)(undefined);
        }
        catch (error) {
            const message = error instanceof Error ? error.message : String(error);
            return (0, result_1.Err)(`Failed to set exclusion: ${message}`);
        }
    }
    async create(projectPath, branchName, trunkBranch, title, runtimeConfig, sectionId) {
        // Chat with Mux is a built-in system workspace; it cannot host additional workspaces.
        if (projectPath === (0, muxChat_2.getMuxHelpChatProjectPath)(this.config.rootDir)) {
            return (0, result_1.Err)("Cannot create workspaces in the Chat with Mux system project");
        }
        ...
        if (this.policyService?.isEnforced()) {
            if (!this.policyService.isRuntimeAllowed(finalRuntimeConfig)) {
                return (0, result_1.Err)("Selected runtime is not allowed by policy");
            }
        }
        ...
        if (!isLocalRuntime && normalizedTrunkBranch.length === 0) {
            return (0, result_1.Err)("Trunk branch is required for worktree and SSH runtimes");
        }
        let runtime;
        try {
            runtime = (0, runtimeFactory_1.createRuntime)(finalRuntimeConfig, { projectPath });
            ...
            const srcBaseDir = (0, runtime_1.getSrcBaseDir)(finalRuntimeConfig);
            if (srcBaseDir && !runtime.createFlags?.deferredRuntimeAccess) {
                const resolvedSrcBaseDir = await runtime.resolvePath(srcBaseDir);
                if (resolvedSrcBaseDir !== srcBaseDir && (0, runtime_1.hasSrcBaseDir)(finalRuntimeConfig)) {
                    finalRuntimeConfig = {
                        ...finalRuntimeConfig,
            ...
            return (0, result_1.Err)(errorMsg);
        }
        ...
        try {
            ...
            if (runtime.createFlags?.configLevelCollisionDetection) {
                const existingNames = new Set((this.config.loadConfigOrDefault().projects.get(projectPath)?.workspaces ?? []).map((w) => w.name));
                for (let i = 0; i < MAX_WORKSPACE_NAME_COLLISION_RETRIES && existingNames.has(finalBranchName); i++) {
                    log_1.log.debug(`Workspace name collision for "${finalBranchName}", adding suffix`);
            ...
            for (let attempt = 0; attempt <= MAX_WORKSPACE_NAME_COLLISION_RETRIES; attempt++) {
                ...
                if (isWorkspaceNameCollision(createResult.error) &&
                    attempt < MAX_WORKSPACE_NAME_COLLISION_RETRIES) {
                    log_1.log.debug(`Workspace name collision for "${finalBranchName}", retrying with suffix`);
                    finalBranchName = appendCollisionSuffix(branchName);
                    continue;
                }
                break;
            ...
            if (!createResult.success || !createResult.workspacePath) {
                ...
                return (0, result_1.Err)(createResult.error ?? "Failed to create workspace");
            }
            ...
            if (runtime.finalizeConfig) {
                ...
                if (!finalizeResult.success) {
                    initLogger.logComplete(-1);
                    return (0, result_1.Err)(finalizeResult.error);
                }
            ...
            if (runtime.validateBeforePersist) {
                const validateResult = await runtime.validateBeforePersist(finalBranchName, finalRuntimeConfig);
                if (!validateResult.success) {
                    initLogger.logComplete(-1);
                    return (0, result_1.Err)(validateResult.error);
                }
            }
            ...
            if (!completeMetadata) {
                initLogger.logComplete(-1);
                return (0, result_1.Err)("Failed to retrieve workspace metadata");
            }
            ...
            const secrets = (0, secrets_1.secretsToRecord)(this.config.getEffectiveSecrets(projectPath));
            ...
            if (!this.removingWorkspaces.has(workspaceId) && !initAbortController.signal.aborted) {
                (0, runtimeFactory_1.runBackgroundInit)(runtime, {
                    projectPath,
                    branchName: finalBranchName,
                    trunkBranch: normalizedTrunkBranch,
            ...
            return (0, result_1.Ok)({ metadata: this.enrichFrontendMetadata(completeMetadata) });
        }
        catch (error) {
            initLogger.logComplete(-1);
            const message = error instanceof Error ? error.message : String(error);
            return (0, result_1.Err)(`Failed to create workspace: ${message}`);
        }
    }
    async remove(workspaceId, force = false) {
        if (workspaceId === muxChat_1.MUX_HELP_CHAT_WORKSPACE_ID) {
            return (0, result_1.Err)("Cannot remove the Chat with Mux system workspace");
        }
        ...
        try {
            ...
            const streamStoppedEvent = wasStreaming
                ? new Promise((resolve) => {
            ...
            try {
                ...
                if (!stopResult.success) {
                    log_1.log.debug("Failed to stop stream during workspace removal", {
                        workspaceId,
                ...
                log_1.log.debug("Failed to stop stream during workspace removal (threw)", { workspaceId, error });
            }
            if (streamStoppedEvent) {
                const stopEvent = await streamStoppedEvent;
                if (!stopEvent) {
                    log_1.log.debug("Timed out waiting for stream to stop during workspace removal", {
                        workspaceId,
                    });
                }
            ...
            let childTaskModelString;
            let childTaskThinkingLevel;
            ...
            if (metadataResult.success) {
                ...
                const deleteResult = await runtime.deleteWorkspace(projectPath, metadata.name, // use branch name
                force);
                if (!deleteResult.success) {
                    // If force is true, we continue to remove from config even if fs removal failed
                    if (!force) {
                        return (0, result_1.Err)(deleteResult.error ?? "Failed to delete workspace from disk");
                    }
                    log_1.log.error(`Failed to delete workspace from disk, but force=true. Removing from config. Error: ${deleteResult.error}`);
                }
                ...
                childTaskThinkingLevel = (0, thinking_1.coerceThinkingLevel)(metadata.taskThinkingLevel);
                ...
                if (parentWorkspaceId && this.sessionTimingService) {
                    try {
                        ...
                        log_1.log.error("Failed to roll up child session timing into parent", {
                            workspaceId,
                            parentWorkspaceId,
                            error: error instanceof Error ? error.message : String(error),
                        });
                ...
                if (parentWorkspaceId && this.sessionUsageService) {
                    try {
                        ...
                        if (childUsage && Object.keys(childUsage.byModel).length > 0) {
                            const rollup = await this.sessionUsageService.rollUpUsageIntoParent(parentWorkspaceId, workspaceId, childUsage.byModel);
                            if (rollup.didRollUp) {
                        ...
                        log_1.log.error("Failed to roll up child session usage into parent", {
                            workspaceId,
                            parentWorkspaceId,
                            error: error instanceof Error ? error.message : String(error),
                        });
                ...
                log_1.log.error(`Could not find metadata for workspace ${workspaceId}, creating phantom cleanup`);
            }
            ...
            try {
                ...
                if (parentWorkspaceId) {
                    try {
                        ...
                        log_1.log.error("Failed to roll up child session artifacts into parent", {
                            workspaceId,
                            parentWorkspaceId,
                            error: error instanceof Error ? error.message : String(error),
                        });
                ...
                log_1.log.error(`Failed to remove session directory for ${workspaceId}:`, error);
            }
            ...
            const message = error instanceof Error ? error.message : String(error);
            return (0, result_1.Err)(`Failed to remove workspace: ${message}`);
        }
        finally {
            this.removingWorkspaces.delete(workspaceId);
        }
    }
    enrichFrontendMetadata(metadata) {
        const isInitializing = this.initStateManager.getInitState(metadata.id)?.status === "running" || undefined;
        return {
            ...metadata,
    ...
    enrichMaybeFrontendMetadata(metadata) {
        if (!metadata) {
            return null;
        }
    ...
    async list() {
        try {
            ...
            log_1.log.error("Failed to list workspaces:", error);
            return [];
        }
    }
    /**
     ...
     */
    async getDevcontainerInfo(workspaceId) {
        const metadata = await this.getInfo(workspaceId);
        ...
        const runtime = (0, runtimeFactory_1.createRuntime)(runtimeConfig, {
        ...
        const containerName = await (0, devcontainerCli_1.getDevcontainerContainerName)(hostWorkspacePath);
        if (!containerName) {
     ...
     */
    async refreshAndEmitMetadata(workspaceId) {
        const metadata = await this.getInfo(workspaceId);
        if (metadata) {
    ...
    async rename(workspaceId, newName) {
        try {
            if (this.aiService.isStreaming(workspaceId)) {
                return (0, result_1.Err)("Cannot rename workspace while AI stream is active. Please wait for the stream to complete.");
            }
            ...
            if (newName === oldName) {
                return (0, result_1.Ok)({ newWorkspaceId: workspaceId });
            }
            const allWorkspaces = await this.config.getAllWorkspaceMetadata();
            const collision = allWorkspaces.find((ws) => (ws.name === newName || ws.id === newName) && ws.id !== workspaceId);
            if (collision) {
                return (0, result_1.Err)(`Workspace with name "${newName}" already exists`);
            }
            const workspace = this.config.findWorkspace(workspaceId);
            if (!workspace) {
                return (0, result_1.Err)("Failed to find workspace in config");
            }
            const { projectPath } = workspace;
            const runtime = (0, runtimeFactory_1.createRuntime)(oldMetadata.runtimeConfig, {
                projectPath,
                workspaceName: oldName,
            });
            const renameResult = await runtime.renameWorkspace(projectPath, oldName, newName);
            if (!renameResult.success) {
                return (0, result_1.Err)(renameResult.error);
            }
            ...
            await (0, helpers_1.movePlanFile)(runtime, oldName, newName, oldMetadata.projectName);
            ...
            if (!updatedMetadata) {
                return (0, result_1.Err)("Failed to retrieve updated workspace metadata");
            }
     ...
     */
    async updateTitle(workspaceId, title) {
        try {
            const workspace = this.config.findWorkspace(workspaceId);
            if (!workspace) {
                return (0, result_1.Err)("Workspace not found");
            }
            ...
            const message = error instanceof Error ? error.message : String(error);
            return (0, result_1.Err)(`Failed to update workspace title: ${message}`);
        }
    }
    /**
    ...
    async archive(workspaceId) {
        if (workspaceId === muxChat_1.MUX_HELP_CHAT_WORKSPACE_ID) {
            return (0, result_1.Err)("Cannot archive the Chat with Mux system workspace");
        }
        ...
        try {
            ...
            if (initState?.status === "running") {
                ...
                try {
                    ...
                    log_1.log.debug("Failed to emit metadata after init cancellation during archive", {
                        workspaceId,
                        error: error instanceof Error ? error.message : String(error),
                    });
                }
            ...
            if (this.workspaceLifecycleHooks) {
                const metadataResult = await this.aiService.getWorkspaceMetadata(workspaceId);
                if (!metadataResult.success) {
                    return (0, result_1.Err)(metadataResult.error);
                }
                ...
                if (!hookResult.success) {
                    return (0, result_1.Err)(hookResult.error);
                }
            }
            ...
            if (this.aiService.isStreaming(workspaceId)) {
                ...
                if (!stopResult.success) {
                    log_1.log.debug("Failed to stop stream during workspace archive", {
                        workspaceId,
                        error: stopResult.error,
                    });
                }
            ...
            const message = error instanceof Error ? error.message : String(error);
            return (0, result_1.Err)(`Failed to archive workspace: ${message}`);
        }
        finally {
    ...
    async unarchive(workspaceId) {
        if (workspaceId === muxChat_1.MUX_HELP_CHAT_WORKSPACE_ID) {
            return (0, result_1.Err)("Cannot unarchive the Chat with Mux system workspace");
        }
        try {
            const workspace = this.config.findWorkspace(workspaceId);
            if (!workspace) {
            ...
            await this.config.editConfig((config) => {
                ...
                if (projectConfig) {
                    const workspaceEntry = projectConfig.workspaces.find((w) => w.id === workspaceId) ??
                        projectConfig.workspaces.find((w) => w.path === workspacePath);
                    if (workspaceEntry) {
                        const wasArchived = (0, archive_1.isWorkspaceArchived)(workspaceEntry.archivedAt, workspaceEntry.unarchivedAt);
                        if (wasArchived) {
            ...
            if (this.workspaceLifecycleHooks) {
                let hookMetadata = updatedMetadata;
                if (!hookMetadata) {
                    const metadataResult = await this.aiService.getWorkspaceMetadata(workspaceId);
                    if (metadataResult.success) {
                        ...
                        log_1.log.debug("Failed to load workspace metadata for afterUnarchive hooks", {
                            workspaceId,
            ...
            const message = error instanceof Error ? error.message : String(error);
            return (0, result_1.Err)(`Failed to unarchive workspace: ${message}`);
        }
    }
    /**
     ...
     */
    async archiveMergedInProject(projectPath) {
        const targetProjectPath = projectPath.trim();
        if (!targetProjectPath) {
            return (0, result_1.Err)("projectPath is required");
        }
        ...
        try {
            ...
            const candidates = allMetadata.filter((metadata) => {
                if (metadata.id === muxChat_1.MUX_HELP_CHAT_WORKSPACE_ID) {
                    return false;
                }
                if (metadata.projectPath !== targetProjectPath) {
                    return false;
                }
                return !(0, archive_1.isWorkspaceArchived)(metadata.archivedAt, metadata.unarchivedAt);
            });
            ...
            await forEachWithConcurrencyLimit(candidates, GH_CONCURRENCY_LIMIT, async (metadata) => {
                ...
                try {
                    const result = await this.executeBash(workspaceId, `gh pr view --json state 2>/dev/null || echo '{"no_pr":true}'`, { timeout_secs: GH_TIMEOUT_SECS });
                    if (!result.success) {
            ...
            const message = error instanceof Error ? error.message : String(error);
            return (0, result_1.Err)(`Failed to archive merged workspaces: ${message}`);
        }
    }
    normalizeWorkspaceAISettings(aiSettings) {
        const rawModel = aiSettings.model;
        const model = (0, models_1.normalizeGatewayModel)(rawModel).trim();
        if (!model) {
            return (0, result_1.Err)("Model is required");
        }
        if (!(0, models_1.isValidModelFormat)(model)) {
            return (0, result_1.Err)(`Invalid model format: ${rawModel}`);
        }
        return (0, result_1.Ok)({
            model,
            thinkingLevel: aiSettings.thinkingLevel,
    ...
    normalizeSendMessageAgentId(options) {
        // agentId is required by the schema, so this just normalizes the value.
        const rawAgentId = options.agentId;
        const normalizedAgentId = typeof rawAgentId === "string" && rawAgentId.trim().length > 0
    ...
    extractWorkspaceAISettingsFromSendOptions(options) {
     ...
     */
    async maybePersistAISettingsFromOptions(workspaceId, options, context) {
        ...
        if (!extractedSettings)
            return;
        const rawAgentId = options?.agentId;
        const agentId = typeof rawAgentId === "string" && rawAgentId.trim().length > 0
            ? rawAgentId.trim().toLowerCase()
            : workspaceDefaults_1.WORKSPACE_DEFAULTS.agentId;
        const persistResult = await this.persistWorkspaceAISettingsForAgent(workspaceId, agentId, extractedSettings, {
            emitMetadata: false,
        });
        if (!persistResult.success) {
            log_1.log.debug(`Failed to persist workspace AI settings from ${context} options`, {
                workspaceId,
                error: persistResult.error,
            });
        }
    }
    async persistWorkspaceAISettingsForAgent(workspaceId, agentId, aiSettings, options) {
        const found = this.config.findWorkspace(workspaceId);
        if (!found) {
            return (0, result_1.Err)("Workspace not found");
        }
        ...
        const workspaceEntry = projectConfig.workspaces.find((w) => w.id === workspaceId);
        const workspaceEntryWithFallback = workspaceEntry ?? projectConfig.workspaces.find((w) => w.path === workspacePath);
        if (!workspaceEntryWithFallback) {
            return (0, result_1.Err)("Workspace not found");
        }
        const normalizedAgentId = agentId.trim().toLowerCase();
        if (!normalizedAgentId) {
            return (0, result_1.Err)("Agent ID is required");
        }
        const prev = workspaceEntryWithFallback.aiSettingsByAgent?.[normalizedAgentId];
        const changed = prev?.model !== aiSettings.model || prev?.thinkingLevel !== aiSettings.thinkingLevel;
        if (!changed) {
    ...
    async updateModeAISettings(workspaceId, mode, aiSettings) {
        // Mode-based updates use mode as the agentId.
        return this.updateAgentAISettings(workspaceId, mode, aiSettings);
    }
    async updateAgentAISettings(workspaceId, agentId, aiSettings) {
        try {
            const normalized = this.normalizeWorkspaceAISettings(aiSettings);
            if (!normalized.success) {
                return (0, result_1.Err)(normalized.error);
            }
            const persistResult = await this.persistWorkspaceAISettingsForAgent(workspaceId, agentId, normalized.data, {
                emitMetadata: true,
            });
            if (!persistResult.success) {
            ...
            const message = error instanceof Error ? error.message : String(error);
            return (0, result_1.Err)(`Failed to update workspace AI settings: ${message}`);
        }
    }
    async fork(sourceWorkspaceId, newName) {
        try {
            if (sourceWorkspaceId === muxChat_1.MUX_HELP_CHAT_WORKSPACE_ID) {
                return (0, result_1.Err)("Cannot fork the Chat with Mux system workspace");
            }
            const validation = (0, workspaceValidation_1.validateWorkspaceName)(newName);
            if (!validation.valid) {
                return (0, result_1.Err)(validation.error ?? "Invalid workspace name");
            ...
            if (!sourceMetadataResult.success) {
                return (0, result_1.Err)(`Failed to get source workspace metadata: ${sourceMetadataResult.error}`);
            }
            ...
            if (this.policyService?.isEnforced()) {
                if (!this.policyService.isRuntimeAllowed(sourceRuntimeConfig)) {
                    return (0, result_1.Err)("Forking this workspace is not allowed by policy (runtime disabled)");
                }
            }
            ...
            if ((0, runtime_1.isDockerRuntime)(sourceRuntimeConfig)) {
                return (0, result_1.Err)("Forking Docker workspaces is not supported. Create a new workspace instead.");
            }
            const runtime = (0, runtimeFactory_1.createRuntime)(sourceRuntimeConfig, {
            ...
            if (!forkResult.success) {
                initLogger.logComplete(-1);
                return (0, result_1.Err)(forkResult.error ?? "Failed to fork workspace");
            }
            ...
            const secrets = (0, secrets_1.secretsToRecord)(this.config.getEffectiveSecrets(foundProjectPath));
            const initAbortController = new AbortController();
            ...
            try {
                ...
                try {
                    ...
                    log_1.log.error(`Failed to clean up session dir ${newSessionDir}:`, cleanupError);
                }
                initLogger.logComplete(-1);
                const message = copyError instanceof Error ? copyError.message : String(copyError);
                return (0, result_1.Err)(`Failed to copy chat history: ${message}`);
            }
            // Copy plan file if it exists (checks both new and legacy paths)
            await (0, helpers_1.copyPlanFile)(runtime, sourceMetadata.name, sourceWorkspaceId, newName, projectName);
            // Apply runtime-provided config updates (e.g., Coder marks shared workspaces)
            const { forkedRuntimeConfig } = await (0, forkRuntimeUpdates_1.applyForkRuntimeUpdates)(this.config, sourceWorkspaceId, sourceRuntimeConfig, forkResult);
            ...
            session.emitMetadata(enrichedMetadata);
            return (0, result_1.Ok)({ metadata: enrichedMetadata, projectPath: foundProjectPath });
        }
        catch (error) {
            const message = error instanceof Error ? error.message : String(error);
            return (0, result_1.Err)(`Failed to fork workspace: ${message}`);
        }
    }
    async sendMessage(workspaceId, message, options, internal) {
        ...
        try {
            ...
            if (this.renamingWorkspaces.has(workspaceId)) {
                log_1.log.debug("sendMessage blocked: workspace is being renamed", { workspaceId });
            ...
            if (this.removingWorkspaces.has(workspaceId)) {
                log_1.log.debug("sendMessage blocked: workspace is being removed", { workspaceId });
                return (0, result_1.Err)({
                    type: "unknown",
            ...
            if (!internal?.allowQueuedAgentTask) {
                ...
                for (const [_projectPath, project] of config.projects) {
                    ...
                    if (ws.parentWorkspaceId && ws.taskStatus === "queued") {
                        (0, taskQueueDebug_1.taskQueueDebug)("WorkspaceService.sendMessage blocked (queued task)", {
                            workspaceId,
                ...
                (0, taskQueueDebug_1.taskQueueDebug)("WorkspaceService.sendMessage allowed (internal dequeue)", {
                    workspaceId,
            ...
            const system1Experiment = experiments_1.EXPERIMENTS[experiments_1.EXPERIMENT_IDS.SYSTEM_1];
            ...
            if (system1Experiment.userOverridable && system1FrontendValue !== undefined) {
                ...
                system1Enabled = this.experimentsService.isExperimentEnabled(experiments_1.EXPERIMENT_IDS.SYSTEM_1);
            }
            else {
            ...
            const resolvedOptions = Object.keys(resolvedExperiments).length === 0
                ? options
                : {
                    ...options,
                    experiments: {
            ...
            const shouldQueue = !normalizedOptions?.editMessageId &&
                (this.aiService.isStreaming(workspaceId) || session.isStreamStarting());
            if (shouldQueue) {
                const pendingAskUserQuestion = askUserQuestionManager_1.askUserQuestionManager.getLatestPending(workspaceId);
                if (pendingAskUserQuestion) {
                    try {
                        askUserQuestionManager_1.askUserQuestionManager.cancel(workspaceId, pendingAskUserQuestion.toolCallId, "User responded in chat; questions canceled");
                    }
                    catch (error) {
                        log_1.log.debug("Failed to cancel pending ask_user_question", {
                            workspaceId,
            ...
            if (!result.success) {
                log_1.log.error("sendMessage handler: session returned error", {
                    workspaceId,
                    error: result.error,
                });
            }
            ...
            if (error instanceof runtimeFactory_1.IncompatibleRuntimeError) {
                const sendError = {
                    type: "incompatible_workspace",
                    message: error.message,
                };
                return (0, result_1.Err)(sendError);
            }
            const sendError = {
                type: "unknown",
    ...
    async resumeStream(workspaceId, options, internal) {
        try {
            // Block streaming while workspace is being renamed to prevent path conflicts
            if (this.renamingWorkspaces.has(workspaceId)) {
                log_1.log.debug("resumeStream blocked: workspace is being renamed", { workspaceId });
                return (0, result_1.Err)({
                    type: "unknown",
                    raw: "Workspace is being renamed. Please wait and try again.",
                });
            ...
            // Block streaming while workspace is being removed to prevent races with config/session deletion.
            if (this.removingWorkspaces.has(workspaceId)) {
                log_1.log.debug("resumeStream blocked: workspace is being removed", { workspaceId });
                return (0, result_1.Err)({
                    type: "unknown",
                    raw: "Workspace is being deleted. Please wait and try again.",
                });
            ...
            if (!internal?.allowQueuedAgentTask) {
                ...
                for (const [_projectPath, project] of config.projects) {
                    ...
                    if (!ws)
                        continue;
                    if (ws.parentWorkspaceId && ws.taskStatus === "queued") {
                        (0, taskQueueDebug_1.taskQueueDebug)("WorkspaceService.resumeStream blocked (queued task)", {
                            workspaceId,
                ...
                (0, taskQueueDebug_1.taskQueueDebug)("WorkspaceService.resumeStream allowed (internal dequeue)", {
                    workspaceId,
            ...
            if (!result.success) {
                log_1.log.error("resumeStream handler: session returned error", {
                    workspaceId,
                    error: result.error,
                });
            }
            ...
            log_1.log.error("Unexpected error in resumeStream handler:", error);
            // Handle incompatible workspace errors from downgraded configs
            if (error instanceof runtimeFactory_1.IncompatibleRuntimeError) {
                const sendError = {
                    type: "incompatible_workspace",
    ...
    async interruptStream(workspaceId, options) {
        try {
            this.taskService?.resetAutoResumeCount(workspaceId);
            const session = this.getOrCreateSession(workspaceId);
            ...
            if (options?.abandonPartial && !options?.soft) {
                log_1.log.debug("Abandoning partial for workspace:", workspaceId);
            ...
            const errorMessage = error instanceof Error ? error.message : String(error);
            log_1.log.error("Unexpected error in interruptStream handler:", error);
            return (0, result_1.Err)(`Failed to interrupt stream: ${errorMessage}`);
        }
    }
    async answerAskUserQuestion(workspaceId, toolCallId, answers) {
        try {
            // Fast path: normal in-memory execution (stream still running, tool is awaiting input).
            askUserQuestionManager_1.askUserQuestionManager.answer(workspaceId, toolCallId, answers);
            return (0, result_1.Ok)(undefined);
        }
        catch (error) {
            ...
            try {
                ...
                const tryFinalizeMessage = (msg) => {
                    let foundToolCall = false;
                    let output = null;
                    let errorMessage = null;
                    const updatedParts = msg.parts.map((part) => {
                        if (!(0, toolParts_1.isDynamicToolPart)(part) || part.toolCallId !== toolCallId) {
                            return part;
                        }
                        foundToolCall = true;
                        ...
                        if (part.state === "output-available") {
                            const parsedOutput = toolDefinitions_1.AskUserQuestionToolResultSchema.safeParse(part.output);
                        ...
                        const parsedArgs = toolDefinitions_1.AskUserQuestionToolArgsSchema.safeParse(part.input);
                    ...
                    if (!foundToolCall) {
                        return (0, result_1.Err)("ask_user_question toolCallId not found in message");
                    }
                    if (!output) {
                        return (0, result_1.Err)("ask_user_question output missing after update");
                    }
                    return (0, result_1.Ok)({ updated: { ...msg, parts: updatedParts }, output });
                };
                ...
                if (partial) {
                    ...
                    if (finalized.success) {
                        const writeResult = await this.partialService.writePartial(workspaceId, finalized.data.updated);
                        if (!writeResult.success) {
                            return (0, result_1.Err)(writeResult.error);
                        }
                        const session = this.getOrCreateSession(workspaceId);
                ...
                for (const msg of historyResult.data) {
                    ...
                    if (seq === undefined)
                        continue;
                    const hasTool = msg.parts.some((p) => (0, toolParts_1.isDynamicToolPart)(p) && p.toolCallId === toolCallId);
                ...
                if (!best) {
                    const errorMessage = error instanceof Error ? error.message : String(error);
                    return (0, result_1.Err)(`Failed to answer ask_user_question: ${errorMessage}`);
                }
                // Guard against answering stale tool calls.
                const maxSeq = Math.max(...historyResult.data
                    .map((m) => m.metadata?.historySequence)
                    .filter((n) => typeof n === "number"));
                if (bestSeq !== maxSeq) {
                    return (0, result_1.Err)(`Refusing to answer ask_user_question: tool call is not the latest message (toolSeq=${bestSeq}, latestSeq=${maxSeq})`);
                }
                const finalized = tryFinalizeMessage(best);
                if (!finalized.success) {
                    return (0, result_1.Err)(finalized.error);
                }
                const updateResult = await this.historyService.updateHistory(workspaceId, finalized.data.updated);
                if (!updateResult.success) {
                    return (0, result_1.Err)(updateResult.error);
                }
                const session = this.getOrCreateSession(workspaceId);
                session.emitChatEvent({
                    type: "tool-call-end",
    ...
    clearQueue(workspaceId) {
        try {
            ...
            const errorMessage = error instanceof Error ? error.message : String(error);
            log_1.log.error("Unexpected error in clearQueue handler:", error);
            return (0, result_1.Err)(`Failed to clear queue: ${errorMessage}`);
        }
    }
    /**
     ...
     */
    async deletePlanFilesForWorkspace(workspaceId, metadata) {
        // Create runtime to get correct muxHome (Docker uses /var/mux, others use ~/.mux)
        const runtime = (0, runtimeHelpers_1.createRuntimeForWorkspace)(metadata);
        const muxHome = runtime.getMuxHome();
        const planPath = (0, planStorage_1.getPlanFilePath)(metadata.name, metadata.projectName, muxHome);
        const legacyPlanPath = (0, planStorage_1.getLegacyPlanFilePath)(workspaceId);
        const isDocker = (0, runtime_1.isDockerRuntime)(metadata.runtimeConfig);
        const isSSH = (0, runtime_1.isSSHRuntime)(metadata.runtimeConfig);
        ...
        const quotedPlanPath = isDocker
            ? (0, backgroundCommands_1.shellQuote)(planPath)
            : isSSH
                ? (0, tildeExpansion_1.expandTildeForSSH)(planPath)
                : (0, backgroundCommands_1.shellQuote)((0, tildeExpansion_1.expandTilde)(planPath));
        // For legacy path: SSH/Docker use $HOME expansion, local expands tilde
        const quotedLegacyPlanPath = isDocker || isSSH
            ? (0, tildeExpansion_1.expandTildeForSSH)(legacyPlanPath)
            : (0, backgroundCommands_1.shellQuote)((0, tildeExpansion_1.expandTilde)(legacyPlanPath));
        if (isDocker || isSSH) {
            try {
        ...
        const planPathAbs = (0, tildeExpansion_1.expandTilde)(planPath);
        const legacyPlanPathAbs = (0, tildeExpansion_1.expandTilde)(legacyPlanPath);
    ...
    async truncateHistory(workspaceId, percentage) {
        if (this.aiService.isStreaming(workspaceId)) {
            return (0, result_1.Err)("Cannot truncate history while stream is active. Press Esc to stop the stream first.");
        }
        const truncateResult = await this.historyService.truncateHistory(workspaceId, percentage ?? 1.0);
        if (!truncateResult.success) {
            return (0, result_1.Err)(truncateResult.error);
        }
    ...
    async replaceHistory(workspaceId, summaryMessage, options) {
        ...
        try {
            ...
            if (replaceMode === "append-compaction-boundary") {
                (0, assert_1.default)(summaryMessage.role === "assistant", "append-compaction-boundary replace mode requires an assistant summary message");
                ...
                const historyResult = await this.historyService.getHistoryFromLatestBoundary(workspaceId);
                if (!historyResult.success) {
                    return (0, result_1.Err)(`Failed to read history for append-compaction-boundary mode: ${historyResult.error}`);
                }
                const nextCompactionEpoch = getNextCompactionEpochForAppendBoundary(workspaceId, historyResult.data);
                (0, assert_1.default)(isPositiveInteger(nextCompactionEpoch), "append-compaction-boundary replace mode must compute a positive compaction epoch");
                ...
                (0, assert_1.default)(hasDurableCompactedMarker(messageToAppend.metadata?.compacted), "append-compaction-boundary replace mode requires a durable compacted marker");
                (0, assert_1.default)(messageToAppend.metadata?.compactionBoundary === true, "append-compaction-boundary replace mode must persist compactionBoundary=true");
                (0, assert_1.default)(isPositiveInteger(messageToAppend.metadata?.compactionEpoch), "append-compaction-boundary replace mode must persist a positive compactionEpoch");
            }
            else {
                (0, assert_1.default)(replaceMode === "destructive", `replaceHistory received unsupported replace mode: ${String(replaceMode)}`);
            ...
            if (!appendResult.success) {
                return (0, result_1.Err)(`Failed to append summary message: ${appendResult.error}`);
            }
            ...
            const typedSummaryMessage = { ...messageToAppend, type: "message" };
            if (session) {
    ...
    async getActivityList() {
        try {
            ...
            log_1.log.error("Failed to list activity:", error);
            return {};
        }
    }
    async getChatHistory(workspaceId) {
        try {
            ...
            log_1.log.error("Failed to get chat history:", error);
            return [];
        }
    }
    async getFileCompletions(workspaceId, query, limit = 20) {
        (0, assert_1.default)(workspaceId, "workspaceId is required");
        (0, assert_1.default)(typeof query === "string", "query must be a string");
        ...
        if (!cached) {
            cached = { index: fileCompletionsIndex_1.EMPTY_FILE_COMPLETIONS_INDEX, fetchedAt: 0 };
        ...
        if (isStale && !cacheEntry.refreshing) {
            cacheEntry.refreshing = (async () => {
                ...
                try {
                    const result = await (0, helpers_1.execBuffered)(runtime, "git ls-files -co --exclude-standard", {
                        cwd: workspacePath,
                        timeout: 5,
                    });
    ...
    async getFullReplay(workspaceId) {
        try {
            const session = this.getOrCreateSession(workspaceId);
            const events = [];
            ...
            log_1.log.error("Failed to get full replay:", error);
            return [];
        }
    }
    async executeBash(workspaceId, script, options) {
        ...
        if (this.removingWorkspaces.has(workspaceId)) {
            return (0, result_1.Err)(`Workspace ${workspaceId} is being removed`);
        }
        ...
        if (this.archivingWorkspaces.has(workspaceId)) {
            return (0, result_1.Err)(`Workspace ${workspaceId} is being archived; cannot execute bash`);
        }
        const metadataResult = await this.aiService.getWorkspaceMetadata(workspaceId);
        if (!metadataResult.success) {
            return (0, result_1.Err)(`Failed to get workspace metadata: ${metadataResult.error}`);
        ...
        const metadata = metadataResult.data;
        if ((0, archive_1.isWorkspaceArchived)(metadata.archivedAt, metadata.unarchivedAt)) {
            return (0, result_1.Err)(`Workspace ${workspaceId} is archived; cannot execute bash`);
        }
        ...
        try {
            ...
            try {
                ...
                const workspace = this.config.findWorkspace(workspaceId);
                if (!workspace) {
                    return (0, result_1.Err)(`Workspace ${workspaceId} not found in config`);
                }
                ...
                const tempDir = __addDisposableResource(env_1, new tempDir_1.DisposableTempDir("mux-ipc-bash"), false);
                ...
                const readyResult = await runtime.ensureReady();
                if (!readyResult.ready) {
                    return (0, result_1.Err)(readyResult.error ?? "Runtime not ready");
                }
                const workspacePath = runtime.getWorkspacePath(metadata.projectPath, metadata.name);
                // Create bash tool
                const bashTool = (0, bash_1.createBashTool)({
                    cwd: workspacePath,
                    runtime,
     ...
     */
    async listBackgroundProcesses(workspaceId) {
     ...
     */
    async terminateBackgroundProcess(workspaceId, processId) {
        // Get process to verify workspace ownership
        const proc = await this.backgroundProcessManager.getProcess(processId);
        if (!proc) {
            return (0, result_1.Err)(`Process not found: ${processId}`);
        }
        if (proc.workspaceId !== workspaceId) {
            return (0, result_1.Err)(`Process ${processId} does not belong to workspace ${workspaceId}`);
        }
     ...
     */
    async getBackgroundProcessOutput(workspaceId, processId, options) {
        const proc = await this.backgroundProcessManager.getProcess(processId);
        if (!proc) {
            return (0, result_1.Err)(`Process not found: ${processId}`);
        }
