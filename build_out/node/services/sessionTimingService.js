...
function getModelKey(model, mode, agentId) {
    if (agentId)
...
class SessionTimingService {
    ...
    clearDeltaEmitState(workspaceId) {
    ...
    emitDeltaChangeImmediate(workspaceId) {
    ...
    emitDeltaChangeThrottled(workspaceId) {
        // Avoid allocating timers/state when nothing is subscribed.
        if ((this.subscriberCounts.get(workspaceId) ?? 0) === 0) {
            return;
        }
    ...
    applyCompletedStreamToFile(file, completed) {
        ...
        base.mode ?? (base.mode = completed.mode);
        base.agentId ?? (base.agentId = completed.agentId);
    ...
    queuePersistCompletedStream(workspaceId, completed) {
    ...
    async rollUpTimingIntoParent(parentWorkspaceId, childWorkspaceId) {
        ...
        return this.fileLocks.withLock(parentWorkspaceId, async () => {
            ...
            for (const childEntry of Object.values(childTiming.session.byModel)) {
                ...
                // Upgrade legacy entries (mode-only) as we observe agent ids.
                base.mode ?? (base.mode = childEntry.mode);
                base.agentId ?? (base.agentId = childEntry.agentId);
