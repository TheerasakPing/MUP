...
(0, bun_test_1.describe)("hooks", () => {
    ...
    (0, bun_test_1.describe)("runPreHook", () => {
        ...
        (0, bun_test_1.test)("receives MUX_TOOL env var", async () => {
            const hookDir = path.join(tempDir, ".mux");
            const hookPath = path.join(hookDir, "tool_pre");
            await fs.mkdir(hookDir, { recursive: true });
            await fs.writeFile(hookPath, '#!/bin/bash\necho "tool=$MUX_TOOL"');
        ...
        (0, bun_test_1.test)("receives flattened tool input env vars", async () => {
            const hookDir = path.join(tempDir, ".mux");
            const hookPath = path.join(hookDir, "tool_pre");
            await fs.mkdir(hookDir, { recursive: true });
            ...
            (0, bun_test_1.expect)(result.allowed).toBe(true);
            (0, bun_test_1.expect)(result.output).toContain("arg=value");
        });
    });
    (0, bun_test_1.describe)("runPostHook", () => {
        ...
        (0, bun_test_1.test)("receives MUX_TOOL_RESULT env var", async () => {
            const hookDir = path.join(tempDir, ".mux");
            const hookPath = path.join(hookDir, "tool_post");
            await fs.mkdir(hookDir, { recursive: true });
        ...
        (0, bun_test_1.test)("receives flattened tool input/result env vars", async () => {
            const hookDir = path.join(tempDir, ".mux");
            const hookPath = path.join(hookDir, "tool_post");
            await fs.mkdir(hookDir, { recursive: true });
            ...
            (0, bun_test_1.expect)(result.success).toBe(true);
            (0, bun_test_1.expect)(result.output).toContain("arg=value");
            (0, bun_test_1.expect)(result.output).toContain("value=42");
        });
        (0, bun_test_1.test)("omits MUX_TOOL_RESULT_PATH when writing result file fails", async () => {
            const hookDir = path.join(tempDir, ".mux");
            const hookPath = path.join(hookDir, "tool_post");
            await fs.mkdir(hookDir, { recursive: true });
            ...
            (0, bun_test_1.expect)(result.success).toBe(true);
            (0, bun_test_1.expect)(result.output).toContain("path=unset");
        });
        (0, bun_test_1.test)("can read result from MUX_TOOL_RESULT_PATH", async () => {
            const hookDir = path.join(tempDir, ".mux");
            const hookPath = path.join(hookDir, "tool_post");
            await fs.mkdir(hookDir, { recursive: true });
    ...
    (0, bun_test_1.describe)("runWithHook", () => {
        (0, bun_test_1.test)("executes tool when hook prints $MUX_EXEC", async () => {
            const hookDir = path.join(tempDir, ".mux");
            const hookPath = path.join(hookDir, "tool_hook");
            await fs.mkdir(hookDir, { recursive: true });
...
`);
        ...
        (0, bun_test_1.test)("blocks tool when hook exits before $MUX_EXEC", async () => {
            const hookDir = path.join(tempDir, ".mux");
            const hookPath = path.join(hookDir, "tool_hook");
            await fs.mkdir(hookDir, { recursive: true });
...
`);
        ...
        (0, bun_test_1.test)("receives tool input via MUX_TOOL_INPUT env var", async () => {
            const hookDir = path.join(tempDir, ".mux");
            const hookPath = path.join(hookDir, "tool_hook");
            await fs.mkdir(hookDir, { recursive: true });
...
`);
            ...
            (0, bun_test_1.expect)(hook.stderr).toContain("SCRIPT=echo hello");
        });
...
`);
        ...
        (0, bun_test_1.test)("succeeds when hook exits without reading MUX_RESULT", async () => {
            const hookDir = path.join(tempDir, ".mux");
            const hookPath = path.join(hookDir, "tool_hook");
            await fs.mkdir(hookDir, { recursive: true });
...
`);
            ...
            (0, bun_test_1.expect)(hook.toolExecuted).toBe(false);
            (0, bun_test_1.expect)(hook.success).toBe(false);
            (0, bun_test_1.expect)(hook.stderr).toContain("Hook timed out before $MUX_EXEC");
            (0, bun_test_1.expect)(result).toBeUndefined();
        });
...
`);
        ...
        (0, bun_test_1.test)("writes large tool input to MUX_TOOL_INPUT_PATH", async () => {
            const hookDir = path.join(tempDir, ".mux");
            const hookPath = path.join(hookDir, "tool_hook");
            await fs.mkdir(hookDir, { recursive: true });
            await fs.writeFile(hookPath, `#!/bin/bash
...
`);
            ...
            (0, bun_test_1.expect)(hook.success).toBe(true);
            (0, bun_test_1.expect)(hook.stderr).toContain("ENV_INPUT=__MUX_TOOL_INPUT_FILE__");
