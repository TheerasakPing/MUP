...
class LocalBaseRuntime {
    ...
    async runInitHook(workspacePath, muxEnv, initLogger, abortSignal) {
        ...
        if (abortSignal?.aborted) {
            initLogger.logComplete(exitCodes_1.EXIT_CODE_ABORTED);
            return;
        }
        ...
        return new Promise((resolve) => {
            ...
            const onAbort = () => {
                ...
                if (proc.pid !== undefined) {
                    (0, disposableExec_1.killProcessTree)(proc.pid);
                    return;
                }
                try {
                    proc.kill("SIGKILL");
            ...
            proc.on("close", (code) => {
                ...
                initLogger.logComplete(aborted || abortSignal?.aborted ? exitCodes_1.EXIT_CODE_ABORTED : (code ?? 0));
                resolve();
            });
            proc.on("error", (err) => {
                abortSignal?.removeEventListener("abort", onAbort);
