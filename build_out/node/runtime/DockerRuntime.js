...
class DockerRuntime extends RemoteRuntime_1.RemoteRuntime {
    ...
    async initWorkspace(params) {
        ...
        try {
            ...
            if (hookExists) {
                ...
                const muxEnv = { ...env, ...(0, initHook_1.getMuxEnv)(projectPath, "docker", branchName) };
    ...
    async provisionContainer(params) {
        ...
        const mkdirResult = await runDockerCommand(`docker exec --user root ${containerName} sh -c 'mkdir -p ${CONTAINER_SRC_DIR} /var/mux/plans && chown ${this.containerUid}:${this.containerGid} ${CONTAINER_SRC_DIR} /var/mux /var/mux/plans'`, 10000);
    ...
    async forkWorkspace(params) {
        ...
        try {
            ...
            const mkdirResult = await runDockerCommand(`docker exec --user root ${destContainerName} sh -c 'mkdir -p ${CONTAINER_SRC_DIR} /var/mux/plans && chown ${destUid}:${destGid} ${CONTAINER_SRC_DIR} /var/mux /var/mux/plans'`, 10000);
            if (mkdirResult.exitCode !== 0) {
                return {
                    success: false,
