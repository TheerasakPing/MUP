...
(0, bun_test_1.describe)("getMuxEnv", () => {
    (0, bun_test_1.it)("should include base MUX_ environment variables", () => {
        const env = (0, initHook_1.getMuxEnv)("/path/to/project", "worktree", "feature-branch");
        (0, bun_test_1.expect)(env.MUX_PROJECT_PATH).toBe("/path/to/project");
        (0, bun_test_1.expect)(env.MUX_RUNTIME).toBe("worktree");
        (0, bun_test_1.expect)(env.MUX_WORKSPACE_NAME).toBe("feature-branch");
        (0, bun_test_1.expect)(env.MUX_MODEL_STRING).toBeUndefined();
        (0, bun_test_1.expect)(env.MUX_THINKING_LEVEL).toBeUndefined();
        (0, bun_test_1.expect)(env.MUX_COSTS_USD).toBeUndefined();
    });
    (0, bun_test_1.it)("should include model + thinking env vars when provided", () => {
        ...
        (0, bun_test_1.expect)(env.MUX_MODEL_STRING).toBe("openai:gpt-5.2-pro");
        (0, bun_test_1.expect)(env.MUX_THINKING_LEVEL).toBe("medium");
    });
    (0, bun_test_1.it)("should allow explicit thinkingLevel=off", () => {
        const env = (0, initHook_1.getMuxEnv)("/path/to/project", "local", "main", {
            modelString: "anthropic:claude-3-5-sonnet",
            thinkingLevel: "off",
        });
        (0, bun_test_1.expect)(env.MUX_MODEL_STRING).toBe("anthropic:claude-3-5-sonnet");
        (0, bun_test_1.expect)(env.MUX_THINKING_LEVEL).toBe("off");
    });
    (0, bun_test_1.it)("should include MUX_COSTS_USD when costsUsd is provided", () => {
        const env = (0, initHook_1.getMuxEnv)("/path/to/project", "worktree", "feature-branch", {
            modelString: "anthropic:claude-opus-4-5",
            thinkingLevel: "high",
            costsUsd: 1.2345,
        });
        (0, bun_test_1.expect)(env.MUX_COSTS_USD).toBe("1.23");
    });
    (0, bun_test_1.it)("should include MUX_COSTS_USD=0.00 when costsUsd is 0", () => {
        const env = (0, initHook_1.getMuxEnv)("/path/to/project", "worktree", "main", {
            costsUsd: 0,
        });
        (0, bun_test_1.expect)(env.MUX_COSTS_USD).toBe("0.00");
    });
    (0, bun_test_1.it)("should not include MUX_COSTS_USD when costsUsd is undefined", () => {
        const env = (0, initHook_1.getMuxEnv)("/path/to/project", "worktree", "main", {
