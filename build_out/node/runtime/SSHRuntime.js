...
exports.SSHRuntime = void 0;
exports.computeBaseRepoPath = computeBaseRepoPath;
...
const tildeExpansion_2 = require("./tildeExpansion");
 ...
 * Extends RemoteRuntime for shared exec/file operations.
 */
class SSHRuntime extends RemoteRuntime_1.RemoteRuntime {
    config;
    transport;
    ensureReadyProjectPath;
    ensureReadyWorkspaceName;
    /** Cached resolved bgOutputDir (tilde expanded to absolute path) */
    resolvedBgOutputDir = null;
    ...
    async getBgOutputDir() {
        ...
        if (dir === "~" || dir.startsWith("~/")) {
            const result = await (0, helpers_1.execBuffered)(this, 'echo "$HOME"', { cwd: "/", timeout: 10 });
            let home;
            if (result.exitCode === 0 && result.stdout.trim()) {
                home = result.stdout.trim();
            }
            else {
                log_1.log.warn(`SSHRuntime: Failed to resolve $HOME (exitCode=${result.exitCode}). Falling back to /tmp.`);
                home = "/tmp";
            }
    ...
    quoteForRemote(filePath) {
        return (0, tildeExpansion_2.expandTildeForSSH)(filePath);
    }
    cdCommand(cwd) {
        return (0, tildeExpansion_2.cdCommandForSSH)(cwd);
    }
    /**
    ...
    async resolvePath(filePath) {
        ...
        const script = [
            `p=${streamUtils_1.shescape.quote(filePath)}`,
            'if [ "$p" = "~" ]; then',
            '  echo "$HOME"',
            'elif [ "${p#\\~/}" != "$p" ]; then',
            '  echo "$HOME/${p#\\~/}"',
        ...
        const command = `bash -lc ${streamUtils_1.shescape.quote(script)}`;
        const abortController = createAbortController(10_000);
        try {
            const result = await (0, helpers_1.execBuffered)(this, command, {
                cwd: "/tmp",
    ...
    async ensureBaseRepo(projectPath, initLogger, abortSignal) {
        ...
        const baseRepoPathArg = (0, tildeExpansion_2.expandTildeForSSH)(baseRepoPath);
        const check = await (0, helpers_1.execBuffered)(this, `test -d ${baseRepoPathArg}`, {
            cwd: "/tmp",
            timeout: 10,
            abortSignal,
        });
        if (check.exitCode !== 0) {
            ...
            await (0, helpers_1.execBuffered)(this, `mkdir -p ${(0, tildeExpansion_2.expandTildeForSSH)(parentDir)}`, {
                cwd: "/tmp",
                timeout: 10,
                abortSignal,
            });
            const initResult = await (0, helpers_1.execBuffered)(this, `git init --bare ${baseRepoPathArg}`, {
                cwd: "/tmp",
                timeout: 30,
                abortSignal,
            });
    ...
    async isWorktreeWorkspace(workspacePath, abortSignal) {
        ...
        const result = await (0, helpers_1.execBuffered)(this, `test -f ${this.quoteForRemote(gitPath)}`, {
            cwd: "/tmp",
            timeout: 10,
            abortSignal,
        });
    ...
    async resolveBundleTrunkRef(baseRepoPathArg, trunkBranch, abortSignal) {
        ...
        const check = await (0, helpers_1.execBuffered)(this, `git -C ${baseRepoPathArg} rev-parse --verify ${streamUtils_1.shescape.quote(preferredRef)}`, { cwd: "/tmp", timeout: 10, abortSignal });
        if (check.exitCode === 0) {
            return preferredRef;
        }
        // Fallback: pick the first ref under refs/mux-bundle/ (handles main↔master mismatch).
        const listResult = await (0, helpers_1.execBuffered)(this, `git -C ${baseRepoPathArg} for-each-ref --format='%(refname)' ${BUNDLE_REF_PREFIX} --count=1`, { cwd: "/tmp", timeout: 10, abortSignal });
        const fallbackRef = listResult.stdout.trim();
        if (listResult.exitCode === 0 && fallbackRef.length > 0) {
            log_1.log.info(`Bundle trunk ref mismatch: expected ${preferredRef}, using ${fallbackRef}`);
            return fallbackRef;
        }
        return null;
    }
    ...
    async checkWorkspaceRepo(options) {
        ...
        const workspacePath = this.getWorkspacePath(this.ensureReadyProjectPath, this.ensureReadyWorkspaceName);
        ...
        try {
            ...
            testResult = await (0, helpers_1.execBuffered)(this, `test -d ${gitDirProbe} || test -f ${gitDirProbe}`, {
                cwd: "~",
                timeout: 10,
            ...
            return {
                ready: false,
                error: `Failed to reach SSH host: ${(0, errors_1.getErrorMessage)(error)}`,
        ...
        if (testResult.exitCode !== 0) {
            ...
            return {
                ready: false,
                error: Runtime_1.WORKSPACE_REPO_MISSING_ERROR,
                errorType: "runtime_not_ready",
            };
        }
        let revResult;
        try {
            revResult = await (0, helpers_1.execBuffered)(this, `git -C ${this.quoteForRemote(workspacePath)} rev-parse --git-dir`, {
                cwd: "~",
                timeout: 10,
                abortSignal: options?.signal,
            });
            ...
            return {
                ready: false,
                error: `Failed to verify repository: ${(0, errors_1.getErrorMessage)(error)}`,
                errorType: "runtime_start_failed",
            };
        }
    ...
    async transferBundleToRemote(projectPath, initLogger, abortSignal) {
        ...
        const gitProc = (0, child_process_1.spawn)("git", ["-C", projectPath, "bundle", "create", "-", "--branches", "--tags"], {
        ...
        try {
            ...
            if (remoteAbortController.didTimeout()) {
                throw new Error(`SSH command timed out after 300000ms: cat > ${this.quoteForRemote(remoteBundlePath)}`);
            }
            if (abortSignal?.aborted) {
                throw new Error("Bundle creation aborted");
            }
            ...
            if (remoteExitCode !== 0) {
                const remoteStderr = await (0, streamUtils_1.streamToString)(remoteStream.stderr);
    ...
    async syncProjectToRemote(projectPath, _workspacePath, initLogger, abortSignal) {
        ...
        const remoteBundlePath = await this.transferBundleToRemote(projectPath, initLogger, abortSignal);
        ...
        try {
            ...
            const fetchResult = await (0, helpers_1.execBuffered)(this, `git -C ${baseRepoPathArg} fetch ${remoteBundlePathArg} '+refs/heads/*:${BUNDLE_REF_PREFIX}*' '+refs/tags/*:refs/tags/*'`, { cwd: "/tmp", timeout: 300, abortSignal });
            if (fetchResult.exitCode !== 0) {
                throw new Error(`Failed to import bundle into base repo: ${fetchResult.stderr || fetchResult.stdout}`);
            }
            ...
            if (originUrl) {
                ...
                await (0, helpers_1.execBuffered)(this, `git -C ${baseRepoPathArg} remote set-url origin ${streamUtils_1.shescape.quote(originUrl)} 2>/dev/null || git -C ${baseRepoPathArg} remote add origin ${streamUtils_1.shescape.quote(originUrl)}`, { cwd: "/tmp", timeout: 10, abortSignal });
            }
            ...
            try {
                await (0, helpers_1.execBuffered)(this, `rm -f ${remoteBundlePathArg}`, {
                    cwd: "/tmp",
                    timeout: 10,
                });
            }
    ...
    async getOriginUrlForSync(projectPath, initLogger) {
        return (0, gitBundleSync_1.getOriginUrlForBundle)(projectPath, initLogger, /* logErrors */ false);
    }
    async createWorkspace(params) {
        try {
            ...
            try {
                ...
                const expandedParentDir = (0, tildeExpansion_2.expandTildeForSSH)(parentDir);
                ...
                if (mkdirExitCode !== 0) {
                    const stderr = await (0, streamUtils_1.streamToString)(mkdirStream.stderr);
                    return {
                        success: false,
                ...
                return {
                    success: false,
                    error: `Failed to prepare remote workspace: ${(0, errors_1.getErrorMessage)(error)}`,
                };
            }
    ...
    async initWorkspace(params) {
        ...
        try {
            ...
            const workspacePathArg = (0, tildeExpansion_2.expandTildeForSSH)(workspacePath);
            let shouldSync = true;
            try {
                const dirCheck = await (0, helpers_1.execBuffered)(this, `test -d ${workspacePathArg}`, {
                    cwd: "/tmp",
                    timeout: 10,
                    abortSignal,
                });
                if (dirCheck.exitCode === 0) {
                    const gitCheck = await (0, helpers_1.execBuffered)(this, `git -C ${workspacePathArg} rev-parse --is-inside-work-tree`, {
                        cwd: "/tmp",
                        timeout: 20,
                        abortSignal,
                    });
            ...
            if (shouldSync) {
                ...
                for (let attempt = 1; attempt <= maxSyncAttempts; attempt++) {
                    try {
                        ...
                        const isRetryable = errorMsg.includes("pack-objects died") ||
                        ...
                        log_1.log.info(`Sync failed (attempt ${attempt}/${maxSyncAttempts}), will retry: ${errorMsg}`);
                        initLogger.logStep(`Sync failed, retrying (attempt ${attempt + 1}/${maxSyncAttempts})...`);
                ...
                const fetchedOrigin = await this.fetchOriginTrunk(baseRepoPath, trunkBranch, initLogger, abortSignal);
                ...
                const bundleTrunkRef = await this.resolveBundleTrunkRef(baseRepoPathArg, trunkBranch, abortSignal);
                const shouldUseOrigin = fetchedOrigin &&
                    bundleTrunkRef != null &&
                    (await this.canFastForwardToOrigin(baseRepoPath, bundleTrunkRef, trunkBranch, initLogger, abortSignal));
                ...
                const worktreeCmd = `git -C ${baseRepoPathArg} worktree add ${workspacePathArg} -B ${streamUtils_1.shescape.quote(branchName)} ${streamUtils_1.shescape.quote(newBranchBase)}`;
                const worktreeResult = await (0, helpers_1.execBuffered)(this, worktreeCmd, {
                    cwd: "/tmp",
                    timeout: 300,
                    abortSignal,
                });
                ...
                const fetchedOrigin = await this.fetchOriginTrunk(workspacePath, trunkBranch, initLogger, abortSignal);
                const shouldUseOrigin = fetchedOrigin &&
                    (await this.canFastForwardToOrigin(workspacePath, trunkBranch, trunkBranch, initLogger, abortSignal));
                if (shouldUseOrigin) {
            ...
            if (skipInitHook) {
                ...
                if (hookExists) {
                    ...
                    const muxEnv = { ...env, ...(0, initHook_1.getMuxEnv)(projectPath, "ssh", branchName) };
                    // Expand tilde in hook path (quoted paths don't auto-expand on remote)
                    const hookPath = (0, tildeExpansion_1.expandTildeForSSH)(`${workspacePath}/.mux/init`);
    ...
    async fetchOriginTrunk(workspacePath, trunkBranch, initLogger, abortSignal) {
        try {
            ...
            const fetchCmd = `git fetch origin ${streamUtils_1.shescape.quote(trunkBranch)}`;
            ...
            if (fetchExitCode !== 0) {
                const fetchStderr = await (0, streamUtils_1.streamToString)(fetchStream.stderr);
                ...
                if (fetchStderr.includes("couldn't find remote ref")) {
                    ...
                    initLogger.logStderr(`Note: Could not fetch from origin (${fetchStderr}), using local branch state`);
                }
                return false;
            }
    ...
    async canFastForwardToOrigin(workspacePath, localRef, originBranch, initLogger, abortSignal) {
        try {
            ...
            const checkCmd = `git merge-base --is-ancestor ${streamUtils_1.shescape.quote(localRef)} origin/${streamUtils_1.shescape.quote(originBranch)}`;
            ...
            initLogger.logStderr(`Note: Local ${localRef} is ahead of or diverged from origin/${originBranch}, using local state`);
            return false;
        }
        catch {
    ...
    async fastForwardToOrigin(workspacePath, trunkBranch, initLogger, abortSignal) {
        try {
            ...
            const mergeCmd = `git merge --ff-only origin/${streamUtils_1.shescape.quote(trunkBranch)}`;
            ...
            const [mergeStderr, mergeExitCode] = await Promise.all([
                (0, streamUtils_1.streamToString)(mergeStream.stderr),
                mergeStream.exitCode,
            ]);
            if (mergeExitCode !== 0) {
                // Fast-forward not possible (diverged branches) - just warn
                initLogger.logStderr(`Note: Fast-forward skipped (${mergeStderr || "branches diverged"}), using local branch state`);
            }
            else {
    ...
    async renameWorkspace(projectPath, oldName, newName, abortSignal) {
        ...
        try {
            const expandedOldPath = (0, tildeExpansion_2.expandTildeForSSH)(oldPath);
            const expandedNewPath = (0, tildeExpansion_2.expandTildeForSSH)(newPath);
            ...
            if (isWorktree) {
                ...
                const baseRepoPathArg = (0, tildeExpansion_2.expandTildeForSSH)(this.getBaseRepoPath(projectPath));
            ...
            return {
                success: false,
                error: `Failed to rename directory: ${(0, errors_1.getErrorMessage)(error)}`,
            };
        }
    }
        ...
        try {
            ...
            const checkScript = force
                ? // When force=true, only check existence
                    `test -d ${streamUtils_1.shescape.quote(deletedPath)} || exit 3`
                : // When force=false, perform all safety checks
                    `
            test -d ${streamUtils_1.shescape.quote(deletedPath)} || exit 3
            cd ${streamUtils_1.shescape.quote(deletedPath)} || exit 1
          ...
          `;
            ...
            if (checkExitCode === 2) {
                ...
                const stderr = await (0, streamUtils_1.streamToString)(checkStream.stderr);
            ...
            if (isWorktree) {
                ...
                if (exitCode !== 0) {
                    ...
                    `rm -rf ${this.quoteForRemote(deletedPath)} && (git -C ${baseRepoPathArg} worktree prune 2>/dev/null || true)`, { cwd: this.config.srcBaseDir, timeout: 30, abortSignal });
                    await fallbackStream.stdin.abort();
                    const fallbackExitCode = await fallbackStream.exitCode;
                    if (fallbackExitCode !== 0) {
                        const fallbackStderr = await (0, streamUtils_1.streamToString)(fallbackStream.stderr);
                        return {
                            success: false,
                ...
                if (!PROTECTED_BRANCHES.includes(workspaceName)) {
                    await (0, helpers_1.execBuffered)(this, `git -C ${baseRepoPathArg} branch -D ${streamUtils_1.shescape.quote(workspaceName)} 2>/dev/null || true`, { cwd: "/tmp", timeout: 10 }).catch(() => undefined);
                }
            }
            else {
                // Legacy full clone: rm -rf to remove the directory on the remote host.
                const removeCommand = `rm -rf ${streamUtils_1.shescape.quote(deletedPath)}`;
            ...
            return { success: false, error: `Failed to delete directory: ${(0, errors_1.getErrorMessage)(error)}` };
        }
    }
    async forkWorkspace(params) {
        ...
        const sourceWorkspacePathArg = (0, tildeExpansion_2.expandTildeForSSH)(sourceWorkspacePath);
        const newWorkspacePathArg = (0, tildeExpansion_2.expandTildeForSSH)(newWorkspacePath);
        try {
            // Guard: avoid clobbering an existing directory.
            {
                const exists = await (0, helpers_1.execBuffered)(this, `test -e ${newWorkspacePathArg}`, {
                    cwd: "/tmp",
                    timeout: 10,
                    abortSignal,
                });
            ...
            const branchResult = await (0, helpers_1.execBuffered)(this, `git -C ${sourceWorkspacePathArg} branch --show-current`, {
                cwd: "/tmp",
                timeout: 30,
                abortSignal,
            });
            ...
            const hasBaseRepo = await (0, helpers_1.execBuffered)(this, `test -d ${baseRepoPathArg}`, {
                cwd: "/tmp",
                timeout: 10,
                abortSignal,
            });
            if (hasBaseRepo.exitCode === 0) {
                ...
                const worktreeCmd = `git -C ${baseRepoPathArg} worktree add ${newWorkspacePathArg} -b ${streamUtils_1.shescape.quote(newWorkspaceName)} ${streamUtils_1.shescape.quote(sourceBranch)}`;
                const worktreeResult = await (0, helpers_1.execBuffered)(this, worktreeCmd, {
                    cwd: "/tmp",
                    timeout: 60,
                    abortSignal,
                ...
                if (worktreeResult.exitCode === 0) {
                    ...
                    await (0, helpers_1.execBuffered)(this, `rm -rf ${newWorkspacePathArg}`, {
                        cwd: "/tmp",
                        timeout: 10,
                    ...
                    log_1.log.info(`Worktree fork failed (${(worktreeResult.stderr || worktreeResult.stdout).trim()}); falling back to full copy`);
            ...
            if (!usedWorktree) {
                ...
                const mkdirResult = await (0, helpers_1.execBuffered)(this, `mkdir -p ${(0, tildeExpansion_2.expandTildeForSSH)(parentDir)}`, {
                    cwd: "/tmp",
                    timeout: 10,
                    abortSignal,
                });
                ...
                const copyResult = await (0, helpers_1.execBuffered)(this, `cp -R -P ${sourceWorkspacePathArg} ${newWorkspacePathArg}`, { cwd: "/tmp", timeout: 300, abortSignal });
                ...
                try {
                    await (0, helpers_1.execBuffered)(this, `cd ${newWorkspacePathArg} && for branch in $(git for-each-ref --format='%(refname:short)' refs/remotes/origin/ | grep -v 'origin/HEAD'); do localname=\${branch#origin/}; git show-ref --verify --quiet refs/heads/$localname || git branch $localname $branch; done`, { cwd: "/tmp", timeout: 30 });
                }
                catch {
                    // Ignore - best-effort.
                }
                ...
                try {
                    const originResult = await (0, helpers_1.execBuffered)(this, `git -C ${sourceWorkspacePathArg} remote get-url origin 2>/dev/null || true`, { cwd: "/tmp", timeout: 10 });
                    const originUrl = originResult.stdout.trim();
                    if (originUrl.length > 0) {
                        await (0, helpers_1.execBuffered)(this, `git -C ${newWorkspacePathArg} remote set-url origin ${streamUtils_1.shescape.quote(originUrl)}`, { cwd: "/tmp", timeout: 10 });
                    }
                    else {
                        await (0, helpers_1.execBuffered)(this, `git -C ${newWorkspacePathArg} remote remove origin 2>/dev/null || true`, { cwd: "/tmp", timeout: 10 });
                    }
                }
                catch {
                    // Ignore - best-effort.
                ...
                const checkoutResult = await (0, helpers_1.execBuffered)(this, checkoutCmd, {
...
exports.SSHRuntime = SSHRuntime;
//# sourceMappingURL=SSHRuntime.js.map