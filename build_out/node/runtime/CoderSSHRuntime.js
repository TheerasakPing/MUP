...
class CoderSSHRuntime extends SSHRuntime_1.SSHRuntime {
    ...
    constructor(config, transport, coderService, options) {
     ...
     */
    async finalizeConfig(finalBranchName, config) {
        ...
        if (!coder.existingWorkspace) {
            ...
            if (!CODER_NAME_REGEX.test(workspaceName)) {
                return (0, result_1.Err)(`Workspace name "${finalBranchName}" cannot be converted to a valid Coder name. ` +
            ...
            if (!workspaceName) {
                return (0, result_1.Err)("Coder workspace name is required for existing workspaces");
            }
        }
        // Final validation
        if (!workspaceName) {
            return (0, result_1.Err)("Coder workspace name is required");
        }
        let hostnameSuffix;
        try {
            ...
            return (0, result_1.Err)(`Failed to read Coder deployment SSH config. ` +
    ...
    async deleteWorkspace(projectPath, workspaceName, force, abortSignal) {
        ...
        if (statusResult.kind === "ok") {
            ...
            if (statusResult.status === "stopped") {
                ...
                try {
                    log_1.log.debug("Coder workspace is stopped; deleting without SSH cleanup", {
                        coderWorkspaceName,
                    });
                    const deleteResult = await this.coderService.deleteWorkspaceEventually(coderWorkspaceName, {
                        timeoutMs: 60_000,
                    ...
                    const message = error instanceof Error ? error.message : String(error);
                    log_1.log.error("Failed to delete stopped Coder workspace", {
                        coderWorkspaceName,
                        error: message,
                    });
    ...
    async forkWorkspace(params) {
        ...
        if (!result.success)
            return { ...result, failureIsFatal: true };
    ...
    async postCreateSetup(params) {
        ...
        if (!this.coderConfig.existingWorkspace) {
            ...
            try {
                for await (const line of this.coderService.createWorkspace(coderWorkspaceName, this.coderConfig.template, this.coderConfig.preset, abortSignal, this.coderConfig.templateOrg, provisioningSession)) {
