...
class DevcontainerRuntime extends LocalBaseRuntime_1.LocalBaseRuntime {
    ...
    async initWorkspace(params) {
        ...
        try {
            ...
            if (hookExists) {
                ...
                const muxEnv = { ...env, ...(0, initHook_1.getMuxEnv)(projectPath, "devcontainer", branchName) };
    ...
    async ensureReady(options) {
        ...
        const hasRepo = await (0, pathUtils_1.isGitRepository)(this.currentWorkspacePath);
        if (!hasRepo) {
            statusSink?.({
                phase: "error",
                runtimeType: "devcontainer",
                detail: Runtime_1.WORKSPACE_REPO_MISSING_ERROR,
            });
            return {
                ready: false,
