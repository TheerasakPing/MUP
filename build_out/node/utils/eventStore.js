...
const sessionFile_1 = require("../../node/utils/sessionFile");
...
class EventStore {
     ...
     */
    constructor(config, filename, serializeState, emitEvent, storeName = "EventStore") {
        this.fileManager = new sessionFile_1.SessionFileManager(config, filename);
     ...
     */
    async readPersisted(workspaceId) {
     ...
     */
    async persist(workspaceId, state, options) {
        const result = await this.fileManager.write(workspaceId, state, options);
        if (!result.success) {
            log_1.log.error(`[${this.storeName}] Failed to persist state for ${workspaceId}: ${result.error}`);
        }
    }
    /**
     ...
     */
    async deletePersisted(workspaceId) {
        const result = await this.fileManager.delete(workspaceId);
        if (!result.success) {
            log_1.log.error(`[${this.storeName}] Failed to delete persisted state for ${workspaceId}: ${result.error}`);
        }
    }
    /**
     ...
     */
    async replay(workspaceId, context) {
        // Try in-memory state first (most recent)
        let state = this.stateMap.get(workspaceId);
