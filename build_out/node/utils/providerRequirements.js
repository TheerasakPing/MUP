...
function hasAnyConfiguredProvider(providers) {
    ...
    for (const [providerKey, rawConfig] of Object.entries(providers)) {
        ...
        if (providerKey === "openai" &&
            (0, codexOauthAuth_1.parseCodexOauthAuth)(rawConfig.codexOauth) !== null) {
            return true;
        }
        if (!(providerKey in providers_1.PROVIDER_DEFINITIONS)) {
        ...
        if (checkProviderConfigured(providerKey, rawConfig)
            .isConfigured) {
            return true;
        }
    }
