...
import { p as W } from "./chunk-4BX2VUAB-DD9VcsVt.js";
import { p as _ } from "./treemap-KMMF4GRG-CUI7aynp.js";
...
var N = F.packet,
    ...
    for (let { start: a, end: i, bits: d, label: c } of t.blocks) {
      ...
      for (
        i ??= a + (d ?? 1) - 1,
          d ??= i - a + 1,
          r = i,
          u.debug(`Packet block ${a} - ${r} with label ${c}`);
        o.length <= l + 1 && e.getPacket().length < L;
      ) {
        const [p, s] = M({ start: a, end: i, bits: d, label: c }, n, l);
        if ((o.push(p), p.end + 1 === n * l && (e.pushWord(o), (o = []), n++), !s)) break;
  ...
  M = b((t, e, r) => {
    ...
    const o = e * r - 1,
      n = e * r;
    return [
      { start: t.start, end: o, label: t.label, bits: o - t.start },
      { start: n, end: t.end, label: t.label, bits: t.end - n },
    ];
  }, "getNextFittingBlock"),
  v = {
    parser: { yy: void 0 },
    parse: b(async (t) => {
      const e = await _("packet", t),
        r = v.parser?.yy;
      if (!(r instanceof w))
        throw new Error(
  ...
  Y = b((t, e, r, o) => {
    ...
    for (const [x, y] of p.entries()) K(f, y, x, l);
    f.append("text")
      .text(s)
      .attr("x", k / 2)
      .attr("y", g - h / 2)
  ...
  K = b(
    (
      ...
      e,
      r,
      { rowHeight: o, paddingX: n, paddingY: l, bitWidth: a, bitsPerRow: i, showBits: d }
    ) => {
      const c = t.append("g"),
        p = r * (o + l) + l;
      for (const s of e) {
