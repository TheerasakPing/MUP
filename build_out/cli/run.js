...
function parseThinkingLevel(value) {
  if (!value) return thinking_1.DEFAULT_THINKING_LEVEL; // Default for mux run
...
program
  ...
  .option(
    ...
    thinking_1.THINKING_DISPLAY_LABELS[thinking_1.DEFAULT_THINKING_LEVEL]
  )
...
async function main() {
  ...
  try {
    ...
    const tempDir = __addDisposableResource(
      env_1,
      new tempDir_1.DisposableTempDir("mux-run"),
      false
    );
    ...
    const thinkingLevel = (0, policy_1.resolveThinkingInput)(
      parseThinkingLevel(opts.thinking),
      model
    );
    ...
    const {
      ...
      workspaceService,
      costTrackingService,
    } = (0, coreServices_1.createCoreServices)({
      config,
    ...
    if (runtimeConfig.type === "docker") {
      ...
      try {
        const symbolic = (0, child_process_1.execSync)(
    ...
    const handleToolStart = (payload) => {
      if (!(0, types_1.isToolCallStart)(payload)) return false;
    ...
    const handleToolDelta = (payload) => {
      if (!(0, types_1.isToolCallDelta)(payload)) return false;
    ...
    const handleToolEnd = (payload) => {
      if (!(0, types_1.isToolCallEnd)(payload)) return false;
    ...
    const chatListener = (event) => {
      ...
      if ((0, types_1.isStreamEnd)(payload)) {
        ...
        if (payload.metadata.usage) {
          displayUsage = (0, displayUsage_1.createDisplayUsage)(
      ...
      if ((0, types_1.isUsageDelta)(payload)) {
        ...
        // Budget enforcement
        if (budget !== undefined) {
          const displayUsage = (0, displayUsage_1.createDisplayUsage)(
