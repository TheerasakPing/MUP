...
const THINKING_LABELS_LIST = Object.values(thinking_1.THINKING_DISPLAY_LABELS).join(", ");
...
function parseThinkingLevel(value) {
    if (!value)
        return thinking_1.DEFAULT_THINKING_LEVEL; // Default for mux run
    // Accepts named levels (off, low, med, high, max, xhigh) and numeric (0–N)
    const level = (0, thinking_1.parseThinkingInput)(value);
    if (level != null) {
        return level;
    }
...
program
    ...
    .option("-t, --thinking <level>", `thinking level: ${THINKING_LABELS_LIST}`, thinking_1.THINKING_DISPLAY_LABELS[thinking_1.DEFAULT_THINKING_LEVEL])
    ...
    .option("--keep-background-processes", "do not terminate background processes on exit (for CI/bench)")
    .addHelpText("after", `
Examples:
  $ mux run "Fix the failing tests"
  $ mux run --dir /path/to/project "Add authentication"
...
const keepBackgroundProcesses = opts.keepBackgroundProcesses === true || process.env.MUX_KEEP_BACKGROUND_PROCESSES === "1";
...
async function main() {
    ...
    try {
        ...
        const tempDir = __addDisposableResource(env_1, new tempDir_1.DisposableTempDir("mux-run"), false);
        ...
        const thinkingLevel = (0, policy_1.resolveThinkingInput)(parseThinkingLevel(opts.thinking), model);
        ...
        const { aiService, historyService, partialService, initStateManager, backgroundProcessManager, mcpServerManager, providerService, workspaceService, costTrackingService, } = (0, coreServices_1.createCoreServices)({
            config,
        ...
        const codexOauthService = new codexOauthService_1.CodexOauthService(config, providerService);
        ...
        const buildSendOptions = (cliMode) => ({
            ...
            toolPolicy: [
                { regex_match: "status_set", action: "disable" },
                { regex_match: "todo_write", action: "disable" },
                { regex_match: "todo_read", action: "disable" },
                { regex_match: "notify", action: "disable" },
            ],
