...
function formatBashStart(_toolName, args) {
  ...
  const timeout = bashArgs.timeout_secs
    ? chalk_1.default.dim(` timeout:${bashArgs.timeout_secs}s`)
    : "";
...
function formatFileReadEnd(_toolName, _args, result) {
  ...
  if (readResult?.success) {
    const size = readResult.file_size
      ? chalk_1.default.dim(` (${formatBytes(readResult.file_size)})`)
      : "";
    const lines = readResult.lines_read
      ? chalk_1.default.dim(` ${readResult.lines_read} lines`)
      : "";
...
function formatBashEnd(_toolName, _args, result) {
  ...
  const exitStr =
    exitCode === 0 ? chalk_1.default.green("exit:0") : chalk_1.default.red(`exit:${exitCode}`);
...
function formatTaskEnd(_toolName, _args, result) {
  ...
  if (status === "completed" && typeof reportMarkdown === "string") {
    ...
    const id = typeof taskId === "string" ? ` ${chalk_1.default.dim(taskId)}` : "";
    return `${chalk_1.default.green("✓")}${id}\n${indent(chalk_1.default.dim(truncated))}`;
  }
  if ((status === "queued" || status === "running") && typeof taskId === "string") {
    return `${chalk_1.default.blue("→")} ${status}: ${chalk_1.default.dim(taskId)}`;
  }
  return null;
}
function formatWebFetchEnd(_toolName, _args, result) {
  ...
  if (fetchResult?.success) {
    ...
    const len = fetchResult.length
      ? chalk_1.default.dim(` ${formatBytes(fetchResult.length)}`)
      : "";
