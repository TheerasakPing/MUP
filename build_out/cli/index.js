...
const version_gen_1 = require("../version_gen");
const argv_1 = require("./argv");
const env = (0, argv_1.detectCliEnvironment)();
const subcommand = (0, argv_1.getSubcommand)(process.argv, env);
...
if (subcommand === "run") {
    if (!(0, argv_1.isCommandAvailable)("run", env)) {
...
else if (subcommand === "desktop" ||
    (env.isElectron && (subcommand === undefined || (0, argv_1.isElectronLaunchArg)(subcommand, env)))) {
    // Explicit `mux desktop`, or Electron runtime with no subcommand / Electron launch args
    if (!(0, argv_1.isCommandAvailable)("desktop", env)) {
    ...
    const versionRecord = version_gen_1.VERSION;
    const gitDescribe = typeof versionRecord.git_describe === "string" ? versionRecord.git_describe : "unknown";
    const gitCommit = typeof versionRecord.git_commit === "string" ? versionRecord.git_commit : "unknown";
    ...
    if (process.env.NODE_ENV !== "production") {
        ...
        for (const flag of versionFlags) {
            if (!argv_1.CLI_GLOBAL_FLAGS.includes(flag)) {
    ...
    if ((0, argv_1.isCommandAvailable)("desktop", env)) {
        program
            .command("desktop")
            .description(env.isElectron ? "Launch the desktop app" : "Launch the desktop app (requires Electron)");
    }
    program.parse(process.argv, (0, argv_1.getParseOptions)(env));
}
