...
const deepLink_1 = require("../common/utils/deepLink");
...
if (isE2ETest) {
    ...
    const e2eUserData = path.join((0, paths_1.getMuxHome)(), "user-data");
    try {
...
console.log(`Mux starting - version: ${version_1.VERSION.git ?? "(dev)"} (built: ${version_1.VERSION.buildTime ?? "dev-mode"})`);
...
function handleMuxDeepLink(raw) {
    try {
        const payload = (0, deepLink_1.parseMuxDeepLink)(raw);
        if (!payload)
            return;
...
function handleArgvMuxDeepLinks(argv) {
...
function registerMuxProtocolClient() {
    try {
        if (!electron_1.app.isPackaged && process.defaultApp && process.argv[1]) {
            // On Windows dev builds, Electron needs the executable + app path to register.
            electron_1.app.setAsDefaultProtocolClient("mux", process.execPath, [path.resolve(process.argv[1])]);
            return;
        }
        electron_1.app.setAsDefaultProtocolClient("mux");
    }
    catch (error) {
...
function getTrayIconPath() {
    ...
    const fileName = electron_1.nativeTheme.shouldUseDarkColors ? "tray-icon-white.png" : "tray-icon-black.png";
...
function loadTrayIconImage() {
    ...
    for (const scaleFactor of [2, 3]) {
        ...
        const hqImage = electron_1.nativeImage.createFromPath(hqPath);
        if (!hqImage.isEmpty()) {
...
async function loadServices() {
    ...
    const authToken = process.env.MUX_SERVER_AUTH_TOKEN ?? (0, crypto_1.randomBytes)(32).toString("hex");
    ...
    electron_1.ipcMain.handle("mux:get-is-windows-wsl-shell", async () => {
        ...
        if (looksLikeWsl && (0, bashPath_1.isBashAvailable)()) {
            return false;
        }
        return looksLikeWsl;
    });
...
if (gotTheLock) {
    void electron_1.app.whenReady().then(async () => {
        try {
            ...
            (0, paths_1.migrateLegacyMuxHome)();
    ...
    electron_1.app.on("activate", () => {
        ...
        if (electron_1.app.isReady() && services) {
            openMuxFromTray();
        }
    });
}
