...
console.log(
  `Mux starting - version: ${version_1.VERSION.git ?? "(dev)"} (built: ${version_1.VERSION.buildTime ?? "dev-mode"})`
);
...
function registerMuxProtocolClient() {
  try {
    if (!electron_1.app.isPackaged && process.defaultApp && process.argv[1]) {
      ...
      electron_1.app.setAsDefaultProtocolClient("mux", process.execPath, [
...
function getTrayIconPath() {
  ...
  const fileName = electron_1.nativeTheme.shouldUseDarkColors
    ? "tray-icon-white.png"
    : "tray-icon-black.png";
...
async function loadServices() {
  ...
  const authToken =
    process.env.MUX_SERVER_AUTH_TOKEN ?? (0, crypto_1.randomBytes)(32).toString("hex");
  ...
  electron_1.ipcMain.handle("mux:get-is-rosetta", async () => {
    ...
    try {
      ...
      const { execSync } = await Promise.resolve().then(() =>
        __importStar(require("node:child_process"))
      );
        ...
        try {
          const { default: installExtension, REACT_DEVELOPER_TOOLS } =
            ...
            await Promise.resolve().then(() =>
              __importStar(require("electron-devtools-installer"))
            );
