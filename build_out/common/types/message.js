...
exports.pickPreservedSendOptions = pickPreservedSendOptions;
...
exports.getCompactionFollowUpContent = getCompactionFollowUpContent;
exports.isCompactionSummaryMetadata = isCompactionSummaryMetadata;
exports.createMuxMessage = createMuxMessage;
 ...
 */
function pickPreservedSendOptions(options) {
  return {
...
function prepareUserMessageForSend(content, existingMetadata) {
  ...
  const reviewsText = reviews?.length
    ? reviews.map(review_1.formatReviewForModel).join("\n\n")
    : "";
...
function getCompactionFollowUpContent(metadata) {
...
function isCompactionSummaryMetadata(metadata) {
  return metadata?.type === "compaction-summary";
}
// Helper to create a simple text message
function createMuxMessage(id, role, content, metadata, additionalParts) {
  const textPart = content ? [{ type: "text", text: content, state: "done" }] : [];
