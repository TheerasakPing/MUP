...
exports.buildGitDiffCommand = buildGitDiffCommand;
/**
 * Generate a stable content-based ID for a hunk
 * Uses file path + line range + diff content to ensure uniqueness
 */
function generateHunkId(filePath, oldStart, newStart, content) {
 ...
 */
function extractAllHunks(fileDiffs) {
 ...
 */
function buildGitDiffCommand(diffBase, includeUncommitted, pathFilter, command) {
  const flags =
    command === "numstat"
      ? " -M --numstat"
      : command === "name-status"
