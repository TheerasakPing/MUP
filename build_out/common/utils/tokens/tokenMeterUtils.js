...
const SEGMENT_DEFS = [
  {
    type: "cached",
    key: "cached",
    color: exports.TOKEN_COMPONENT_COLORS.cached,
    label: "Cache Read",
  },
  {
    type: "cacheCreate",
 ...
 */
function calculateTokenMeterData(usage, model, use1M, verticalProportions = false, customConfig) {
  if (!usage) return { segments: [], totalTokens: 0, totalPercentage: 0 };
  const modelStats = (0, modelStats_1.getModelStats)(model, customConfig);
  const maxTokens =
    use1M && (0, models_1.supports1MContext)(model) ? 1_000_000 : modelStats?.max_input_tokens;
