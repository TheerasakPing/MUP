...
const cloneToolPreservingDescriptors_1 = require("../../../common/utils/tools/cloneToolPreservingDescriptors");
...
const mux_global_agents_read_1 = require("../../../node/services/tools/mux_global_agents_read");
const mux_global_agents_write_1 = require("../../../node/services/tools/mux_global_agents_write");
...
function wrapToolExecuteWithModelOnlyNotifications(toolName, baseTool, engine) {
  ...
  const wrappedTool = (0, cloneToolPreservingDescriptors_1.cloneToolPreservingDescriptors)(
    baseTool
  );
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  ...
  wrappedToolRecord.execute = async (args, options) => {
    try {
      ...
      return (0, internalToolResultFields_1.attachModelOnlyToolNotifications)(
        result,
        notifications
      );
    } catch (error) {
...
function wrapToolsWithModelOnlyNotifications(tools, config) {
  ...
  const engine = new NotificationEngine_1.NotificationEngine([
    new TodoListReminderSource_1.TodoListReminderSource({
...
async function getToolsForModel(
  ...
  const wrap = (tool) =>
    (0, wrapWithInitWait_1.wrapWithInitWait)(tool, workspaceId, initStateManager);
  ...
  const { createWebFetchTool } = await Promise.resolve().then(() =>
    __importStar(require("../../../node/services/tools/web_fetch"))
  );
  ...
  const runtimeTools = {
    ...
    file_edit_replace_string: wrap(
      (0, file_edit_replace_string_1.createFileEditReplaceStringTool)(config)
    ),
    ...
    task_apply_git_patch: wrap((0, task_apply_git_patch_1.createTaskApplyGitPatchTool)(config)),
    ...
    bash_background_terminate: wrap(
      (0, bash_background_terminate_1.createBashBackgroundTerminateTool)(config)
    ),
  ...
  const nonRuntimeTools = {
    mux_global_agents_read: (0, mux_global_agents_read_1.createMuxGlobalAgentsReadTool)(config),
    mux_global_agents_write: (0, mux_global_agents_write_1.createMuxGlobalAgentsWriteTool)(config),
    ...
    ...(config.enableAgentReport
      ? { agent_report: (0, agent_report_1.createAgentReportTool)(config) }
      : {}),
    ...
    switch (provider) {
      case "anthropic": {
        const { anthropic } = await Promise.resolve().then(() =>
          __importStar(require("@ai-sdk/anthropic"))
        );
        allTools = {
          ...baseTools,
          ...(mcpTools ?? {}),
      ...
      case "openai": {
        ...
        const sanitizedMcpTools = mcpTools
          ? (0, schemaSanitizer_1.sanitizeMCPToolsForOpenAI)(mcpTools)
          : {};
        ...
        if (modelId.includes("gpt-5") || modelId.includes("gpt-4")) {
          const { openai } = await Promise.resolve().then(() =>
            __importStar(require("@ai-sdk/openai"))
          );
          allTools = {
            ...baseTools,
            ...sanitizedMcpTools,
  ...
  const allowlistedToolNames = new Set(
    (0, toolDefinitions_1.getAvailableTools)(modelString, {
      enableAgentReport: config.enableAgentReport,
      enableMuxGlobalAgentsTools: workspaceId === muxChat_1.MUX_HELP_CHAT_WORKSPACE_ID,
    })
  );
