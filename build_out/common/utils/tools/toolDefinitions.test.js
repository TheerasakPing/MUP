...
describe("TOOL_DEFINITIONS", () => {
    it("accepts custom subagent_type IDs (deprecated alias)", () => {
        const parsed = toolDefinitions_1.TaskToolArgsSchema.safeParse({
    ...
    it("accepts bash tool calls using command (alias for script)", () => {
        const parsed = toolDefinitions_1.TOOL_DEFINITIONS.bash.schema.safeParse({
            command: "ls",
            timeout_secs: 60,
    ...
    it("asks for clarification via ask_user_question (instead of emitting open questions)", () => {
        expect(toolDefinitions_1.TOOL_DEFINITIONS.ask_user_question.description).toContain("MUST be used when you need user clarification");
        expect(toolDefinitions_1.TOOL_DEFINITIONS.ask_user_question.description).toContain("Do not output a list of open questions");
    });
    it("encourages compact task briefs when spawning sub-agents", () => {
        expect(toolDefinitions_1.TOOL_DEFINITIONS.task.description).toContain("compact task brief");
        expect(toolDefinitions_1.TOOL_DEFINITIONS.task.description).toContain("plan file");
    });
    it("accepts ask_user_question headers longer than 12 characters", () => {
        const parsed = toolDefinitions_1.TOOL_DEFINITIONS.ask_user_question.schema.safeParse({
            questions: [
                {
    ...
    it("rejects task(kind=bash) tool calls (bash is a separate tool)", () => {
        const parsed = toolDefinitions_1.TOOL_DEFINITIONS.task.schema.safeParse({
    ...
    it("discourages repeating plan contents or plan file location after propose_plan", () => {
        expect(toolDefinitions_1.TOOL_DEFINITIONS.propose_plan.description).toContain("do not paste the plan contents");
        expect(toolDefinitions_1.TOOL_DEFINITIONS.propose_plan.description).toContain("plan file path");
    });
});
//# sourceMappingURL=toolDefinitions.test.js.map