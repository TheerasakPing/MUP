...
exports.coerceBashToolResult = coerceBashToolResult;
exports.convertTaskBashResult = convertTaskBashResult;
...
function coerceBashToolResult(value) {
    ...
    if (success) {
        ...
        const wallDurationMs = value.wall_duration_ms;
        ...
        // other when possible.
        const taskIdRaw = value.taskId;
        const backgroundProcessIdRaw = value.backgroundProcessId;
...
function maybeApplyLegacySuccessCheck(bashResult, options) {
...
function convertTaskBashResult(taskResult, options) {
    ...
    if (status === "completed") {
        ...
        const nestedRaw = taskResult.result ??
            taskResult.bashResult;
        const nested = coerceBashToolResult(nestedRaw);
        if (nested) {
            return maybeApplyLegacySuccessCheck(nested, options);
        }
        const exitCodeRaw = taskResult.exitCode;
        const exitCode = typeof exitCodeRaw === "number" && Number.isFinite(exitCodeRaw) ? exitCodeRaw : undefined;
        const errorRaw = taskResult.error;
        const error = typeof errorRaw === "string" ? errorRaw : undefined;
        const wallDurationRaw = taskResult.wall_duration_ms;
        const wall_duration_ms = typeof wallDurationRaw === "number" && Number.isFinite(wallDurationRaw) ? wallDurationRaw : 0;
        const outputRaw = taskResult.output;
        const reportMarkdown = taskResult.reportMarkdown;
        const title = taskResult.title;
...
//# sourceMappingURL=taskResultConverters.js.map