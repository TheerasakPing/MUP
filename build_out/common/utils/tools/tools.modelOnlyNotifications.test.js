...
(0, bun_test_1.describe)("getToolsForModel - model-only notifications", () => {
  (0, bun_test_1.test)(
    "injects __mux_notifications into tool results after 5 tool calls",
    async () => {
      ...
      try {
        ...
        const tools = await (0, tools_1.getToolsForModel)(
          "noop:model",
          {
            cwd: process.cwd(),
            runtime,
        ...
        for (let i = 0; i < 4; i += 1) {
          ...
          (0, bun_test_1.expect)(
            internalToolResultFields_1.MODEL_ONLY_TOOL_NOTIFICATIONS_FIELD in result
          ).toBe(false);
        }
        const fifth = asRecord(await todoReadExecute());
        (0, bun_test_1.expect)(
          Array.isArray(fifth[internalToolResultFields_1.MODEL_ONLY_TOOL_NOTIFICATIONS_FIELD])
        ).toBe(true);
        (0, bun_test_1.expect)(
          String(fifth[internalToolResultFields_1.MODEL_ONLY_TOOL_NOTIFICATIONS_FIELD][0])
        ).toContain("Current TODO List");
      } finally {
  ...
  (0, bun_test_1.test)(
    "does not re-wrap cached MCP tools across getToolsForModel() calls",
    async () => {
      const workspaceSessionDir = await fs.mkdtemp(path.join(os.tmpdir(), "mux-ws-"));
      try {
        await (0, todo_1.setTodosForSessionDir)("ws-1", workspaceSessionDir, [
        ...
        const tools1 = await (0, tools_1.getToolsForModel)(
          "noop:model",
          {
            cwd: process.cwd(),
            runtime,
        ...
        const tools2 = await (0, tools_1.getToolsForModel)(
          "noop:model",
          {
            cwd: process.cwd(),
            runtime,
  ...
  (0, bun_test_1.test)(
    "does not enable notification injection when workspaceSessionDir is missing",
    async () => {
      const runtime = new LocalRuntime_1.LocalRuntime(process.cwd());
      const initStateManager = {
        waitForInit: () => Promise.resolve(),
