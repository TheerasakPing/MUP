...
function findLatestCompactionBoundaryIndex(messages) {
    (0, assert_1.default)(Array.isArray(messages), "findLatestCompactionBoundaryIndex requires a message array");
...
function sliceMessagesFromLatestCompactionBoundary(messages) {
    ...
    (0, assert_1.default)(boundaryIndex >= 0 && boundaryIndex < messages.length, "compaction boundary index must be within message history bounds");
    const sliced = messages.slice(boundaryIndex);
    (0, assert_1.default)(sliced.length > 0, "compaction boundary slicing must retain at least one message");
    (0, assert_1.default)(isDurableCompactionBoundaryMarker(sliced[0]), "compaction boundary slicing must start on a durable compaction boundary message");
    return sliced;
}
