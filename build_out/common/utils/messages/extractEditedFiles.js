...
exports.extractEditedFilePaths = extractEditedFilePaths;
exports.extractEditedFileDiffs = extractEditedFileDiffs;
 ...
 */
function extractEditedFilePaths(messages) {
  const editedFiles = [];
  const seen = new Set();
  ...
  for (let i = messages.length - 1; i >= 0; i--) {
    ...
    for (const part of message.parts) {
      ...
      if (!tools_1.FILE_EDIT_TOOL_NAMES.includes(part.toolName)) continue;
  ...
  return editedFiles.slice(0, attachments_1.MAX_EDITED_FILES);
}
/**
 ...
 */
function extractOriginalFromDiffs(diffs) {
  ...
  const firstDiffOriginalIndices = new Set();
  const firstDiffAddedIndices = new Set(); // Indices in the NEW file that were added
  const firstDiff = diffs[0];
  const firstPatches = (0, diff_1.parsePatch)(firstDiff);
 ...
 */
function extractEditedFileDiffs(messages) {
  // Collect all diffs per file path in chronological order
  const diffsByPath = new Map();
  const editOrder = []; // Track order of last edit per file
  for (const message of messages) {
    ...
    for (const part of message.parts) {
      ...
      const output = part.output;
      if (!output?.success) continue;
      const uiOnly = (0, toolOutputUiOnly_1.getToolOutputUiOnly)(output);
  ...
  const results = [];
  for (
    let i = editOrder.length - 1;
    i >= 0 && results.length < attachments_1.MAX_EDITED_FILES;
    i--
  ) {
 ...
 */
function combineDiffs(filePath, diffs) {
  if (diffs.length === 0) return null;
  // Single diff - no combination needed
  if (diffs.length === 1) {
    const diff = diffs[0];
    const truncated = diff.length > attachments_1.MAX_FILE_CONTENT_SIZE;
    return {
      path: filePath,
      diff: truncated ? diff.slice(0, attachments_1.MAX_FILE_CONTENT_SIZE) : diff,
      truncated,
    };
  }
  ...
  for (const diff of diffs) {
    const result = (0, diff_1.applyPatch)(content, diff);
    if (result === false) {
      // Patch failed to apply - fall back to just using the last diff
      const lastDiff = diffs[diffs.length - 1];
      const truncated = lastDiff.length > attachments_1.MAX_FILE_CONTENT_SIZE;
      return {
        path: filePath,
        diff: truncated ? lastDiff.slice(0, attachments_1.MAX_FILE_CONTENT_SIZE) : lastDiff,
        truncated,
      };
    }
    content = result;
  ...
  const combinedDiff = (0, diff_1.createPatch)(filePath, originalContent, content, "", "", {
    context: 3,
  });
  const truncated = combinedDiff.length > attachments_1.MAX_FILE_CONTENT_SIZE;
  return {
    path: filePath,
    diff: truncated ? combinedDiff.slice(0, attachments_1.MAX_FILE_CONTENT_SIZE) : combinedDiff,
    truncated,
  };
}
//# sourceMappingURL=extractEditedFiles.js.map
