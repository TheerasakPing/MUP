    ...
    for (const [key, existingTool] of Object.entries(tools)) {
        if (key === lastToolKey) {
            if (isProviderNativeTool(existingTool)) {
                ...
                const cachedProviderTool = (0, cloneToolPreservingDescriptors_1.cloneToolPreservingDescriptors)(existingTool);
                ...
                const cachedDynamicTool = (0, cloneToolPreservingDescriptors_1.cloneToolPreservingDescriptors)(existingTool);
                cachedDynamicTool.providerOptions = ANTHROPIC_CACHE_CONTROL;
                cachedTools[key] = cachedDynamicTool;
            }
            else {
                (0, assert_1.default)(existingTool.execute != null, `Tool "${key}" must define execute before cache control is applied`);
