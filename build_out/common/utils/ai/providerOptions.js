 ...
 */
function buildProviderOptions(modelString, thinkingLevel, messages, lostResponseIds, muxProviderOptions, workspaceId, // Optional for non-OpenAI providers
openaiTruncationMode) {
    ...
    const [provider, modelName] = (0, models_1.normalizeGatewayModel)(modelString).split(":", 2);
    ...
    if (!provider || !modelName) {
        log_1.log.debug("buildProviderOptions: No provider or model name found, returning empty");
        return {};
    }
    ...
    if (provider === "anthropic") {
        ...
        if (isOpus45 || isOpus46) {
            ...
            const effortLevel = (0, thinking_1.getAnthropicEffort)(effectiveThinking);
            const budgetTokens = thinking_1.ANTHROPIC_THINKING_BUDGETS[effectiveThinking];
            ...
            const thinking = isOpus46
                ...
                : budgetTokens > 0
                    ...
                    : undefined;
            log_1.log.debug("buildProviderOptions: Anthropic effort model config", {
                effort: effortLevel,
                thinking,
        ...
        log_1.log.debug("buildProviderOptions: Returning Anthropic options", options);
        return options;
    }
    // Build OpenAI-specific options
    if (provider === "openai") {
        const reasoningEffort = thinking_1.OPENAI_REASONING_EFFORT[effectiveThinking];
        ...
        if (messages && messages.length > 0 && reasoningEffort) {
            ...
            const currentModelName = (0, models_1.normalizeGatewayModel)(modelString).split(":")[1];
            ...
            for (let i = messages.length - 1; i >= 0; i--) {
                ...
                if (msg.role === "assistant") {
                    ...
                    const msgModelName = msgModel ? (0, models_1.normalizeGatewayModel)(msgModel).split(":")[1] : undefined;
                    if (msgModelName === currentModelName) {
                        ...
                        if (previousResponseId) {
                            ...
                            if (lostResponseIds?.(previousResponseId)) {
                                log_1.log.info("buildProviderOptions: Filtering out lost previousResponseId", {
                                    previousResponseId,
                                ...
                                log_1.log.debug("buildProviderOptions: Found previousResponseId from same model", {
                                    previousResponseId,
                                    model: currentModelName,
                                });
                            }
                        ...
                        log_1.log.debug("buildProviderOptions: Skipping previousResponseId - model changed", {
    ...
    if (provider === "google") {
        ...
        if (effectiveThinking !== "off") {
            ...
            if (isGemini3) {
                ...
                thinkingConfig.thinkingLevel = effectiveThinking;
            }
            else {
                // Gemini 2.5 uses thinkingBudget
                const budget = thinking_1.GEMINI_THINKING_BUDGETS[effectiveThinking];
                if (budget > 0) {
        ...
        log_1.log.debug("buildProviderOptions: Google options", options);
        return options;
    }
    // Build OpenRouter-specific options
    if (provider === "openrouter") {
        const reasoningEffort = thinking_1.OPENROUTER_REASONING_EFFORT[effectiveThinking];
        log_1.log.debug("buildProviderOptions: OpenRouter config", {
            reasoningEffort,
            thinkingLevel: effectiveThinking,
        });
        ...
        if (reasoningEffort) {
            ...
            log_1.log.debug("buildProviderOptions: Returning OpenRouter options", options);
            return options;
        }
        // No reasoning config needed when thinking is off
        log_1.log.debug("buildProviderOptions: OpenRouter (thinking off, no provider options)");
        return {};
    }
    ...
    if (provider === "xai") {
        ...
        log_1.log.debug("buildProviderOptions: Returning xAI options", options);
        return options;
    }
