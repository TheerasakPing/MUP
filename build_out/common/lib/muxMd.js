...
function isMuxMdUrl(url) {
  try {
    ...
    return (
      getMuxMdAllowedHosts().includes(parsed.host) && (0, mux_md_client_1.parseUrl)(url) !== null
    );
  } catch {
    return false;
  }
...
async function updateMuxMdExpiration(id, mutateKey, expiresAt) {
  const result = await (0, mux_md_client_1.setExpiration)(id, mutateKey, expiresAt, {
    baseUrl: getMuxMdBaseUrl(),
  });
