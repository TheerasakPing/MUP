...
import { G as B } from "./graph-ChesIUmq.js";
import { l as I } from "./layout-CsqhZR1q.js";
import { i as b } from "./_baseUniq-BfzZL76d.js";
import { c as L } from "./clone-BvApX1i5.js";
import { m as A } from "./_basePickBy-DcXOyUwY.js";
...
var d = new Map(),
  ...
  se = w((e, t) => {
    ...
    return (
      ...
      e.v === t || e.w === t
        ? !1
        : n
          ? n.includes(e.v) || D(e.v, t) || D(e.w, t) || n.includes(e.w)
          : (i.debug("Tilt, ", t, ",not in descendants"), !1)
    );
  }, "edgeInCluster"),
  R = w((e, t, n, a) => {
    i.warn("Copying children of ", e, "root", a, "data", t.node(e), a);
    const r = t.children(e) || [];
    (e !== a && r.push(e),
      i.warn("Copying (nodes) clusterId", e, "nodes", r),
      r.forEach((o) => {
        if (t.children(o).length > 0) R(o, t, n, a);
        else {
          const l = t.node(o);
          (i.info("cp ", o, " to ", a, " with parent ", e),
            n.setNode(o, l),
            a !== t.parent(o) &&
              (i.warn("Setting parent", o, t.parent(o)), n.setParent(o, t.parent(o))),
            e !== a && o !== e
              ? (i.debug("Setting parent", o, e), n.setParent(o, e))
              : (i.info("In copy ", e, "root", a, "data", t.node(e), a),
                i.debug(
          ...
          (i.debug("Copying Edges", u),
            u.forEach((c) => {
              ...
              try {
                se(c, a)
                  ? (i.info("Copying as ", c.v, c.w, m, c.name),
  ...
  T = w((e, t) => {
    ...
    let a = [...n];
    for (const r of n) (J.set(r, e), (a = [...a, ...T(r, t)]));
    return a;
  }, "extractDescendants"),
  ie = w((e, t, n) => {
    const a = e.edges().filter((c) => c.v === t || c.w === t),
      r = e.edges().filter((c) => c.v === n || c.w === n),
      o = a.map((c) => ({ v: c.v === t ? n : c.v, w: c.w === t ? t : c.w })),
      l = r.map((c) => ({ v: c.v, w: c.w }));
    return o.filter((c) => l.some((m) => c.v === m.v && c.w === m.w));
  }, "findCommonEdges"),
  C = w((e, t, n) => {
    const a = t.children(e);
    if ((i.trace("Searching children of id ", e, a), a.length < 1)) return e;
    let r;
    for (const o of a) {
      const l = C(o, t, n),
        u = ie(t, n, l);
  ...
  k = w(
    (e) => (!d.has(e) || !d.get(e).externalConnections ? e : d.has(e) ? d.get(e).id : e),
    "getAnchorId"
  ),
  re = w((e, t) => {
    if (!e || t > 10) {
    ...
    (e.nodes().forEach(function (n) {
      e.children(n).length > 0 &&
        (i.warn("Cluster identified", n, " Replacement id in edges: ", C(n, e, n)),
        X.set(n, T(n, e)),
      ...
      e.nodes().forEach(function (n) {
        const a = e.children(n),
          r = e.edges();
        a.length > 0
          ? (i.debug("Cluster identified", n, X),
            r.forEach((o) => {
              const l = D(o.v, n),
                u = D(o.w, n);
              l ^ u &&
                (i.warn("Edge: ", o, " leaves cluster ", n),
                i.warn("Descendants of XXX ", n, ": ", X.get(n)),
                (d.get(n).externalConnections = !0));
            }))
    ...
    (e.edges().forEach(function (n) {
      ...
      let r = n.v,
        o = n.w;
      if (
        (i.warn("Fix XXX", d, "ids:", n.v, n.w, "Translating: ", d.get(n.v), " --- ", d.get(n.w)),
        d.get(n.v) || d.get(n.w))
      ) {
        if (
  ...
  G = w((e, t) => {
    ...
    for (const r of n)
      if (
        ...
        d.get(r)?.clusterData?.dir &&
          ((l = d.get(r).clusterData.dir), i.warn("Fixing dir", d.get(r).clusterData.dir, l));
        const u = new B({ multigraph: !0, compound: !0 })
          .setGraph({ rankdir: l, nodesep: 50, ranksep: 50, marginx: 8, marginy: 8 })
          .setDefaultEdgeLabel(function () {
            return {};
          });
        (i.warn("Old graph before copy", h(e)),
          R(r, e, u, r),
          e.setNode(r, {
            clusterNode: !0,
            id: r,
          ...
          i.warn("New graph after copy node: (", r, ")", h(u)),
          i.debug("Old graph after copy", h(e)));
      } else
        (i.warn(
          "Cluster ** ",
          r,
    ...
    for (const r of n) {
      const o = e.node(r);
      (i.warn(" Now next level", r, o), o?.clusterNode && G(o.graph, t + 1));
    }
  }, "extractor"),
  M = w((e, t) => {
  ...
  oe = w((e) => M(e, e.children()), "sortNodesByHierarchy"),
  j = w(async (e, t, n, a, r, o) => {
    ...
    const c = u.insert("g").attr("class", "clusters"),
      m = u.insert("g").attr("class", "edgePaths"),
      v = u.insert("g").attr("class", "edgeLabels"),
      p = u.insert("g").attr("class", "nodes");
    (await Promise.all(
...
Parent cluster`,
            ...
            t.setNode(r.id, g),
            t.parent(f) || (i.trace("Setting parent", f, r.id), t.setParent(f, r.id, g)));
        }
        if ((i.info("(Insert) Node XXX" + f + ": " + JSON.stringify(t.node(f))), s?.clusterNode)) {
          ...
          s.graph.setGraph({ ...s.graph.graph(), ranksep: g + 25, nodesep: E });
          const N = await j(p, s.graph, n, a, t.node(f), o),
            x = N.elem;
          (U(s, x),
            (s.diff = N.diff || 0),
            i.info(
          ...
          t.children(f).length > 0
            ? (i.trace("Cluster - the non recursive path XBX", f, s.id, s, s.width, "Graph:", t),
              i.trace(C(s.id, t)),
              d.set(s.id, { id: C(s.id, t), node: s }))
            : (i.trace("Node - the non recursive path XAX", f, p, t.node(f), l),
              await W(p, t.node(f), { config: o, dir: l }));
      })
    ),
      await w(async () => {
        const f = t.edges().map(async function (s) {
          ...
          (i.info("Edge " + s.v + " -> " + s.w + ": " + JSON.stringify(s)),
            ...
            i.info("Fix", d, "ids:", s.v, s.w, "Translating: ", d.get(s.v), d.get(s.w)),
            await Z(v, g));
        });
        await Promise.all(f);
      }, "processEdges")(),
      i.info("Graph before layout:", JSON.stringify(h(t))),
      i.info("############################################# XXX"),
      i.info("###                Layout                 ### XXX"),
      i.info("############################################# XXX"),
      I(t),
      i.info("Graph after layout:", JSON.stringify(h(t))));
    let P = 0,
      { subGraphTitleTotalMargin: S } = $(o);
    return (
      await Promise.all(
        oe(t).map(async function (f) {
          ...
          if (
            ...
            (i.info("A pure cluster node XBX1", f, s.id, s.x, s.y, s.width, s.height, t.parent(f)),
              (s.height += S),
              t.node(s.parentId));
            const g = s?.padding / 2 || 0,
              E = s?.labelBBox?.height || 0,
              N = E - g || 0;
            (i.debug("OffsetY", N, "labelHeight", E, "halfPadding", g),
              await q(c, s),
              (d.get(s.id).node = s));
          } else {
      ...
      t.edges().forEach(function (f) {
        const s = t.edge(f);
        (i.info("Edge " + f.v + " -> " + f.w + ": " + JSON.stringify(s), s),
          s.points.forEach((x) => (x.y += S / 2)));
        const g = t.node(f.v);
        var E = t.node(f.w);
        const N = z(m, s, d, n, g, E, a);
        K(s, N);
      }),
      t.nodes().forEach(function (f) {
        const s = t.node(f);
        (i.info(f, s.type, s.diff), s.isGroup && (P = s.diff));
      }),
  ...
  ve = w(async (e, t) => {
    const n = new B({ multigraph: !0, compound: !0 })
      ...
      a = t.select("g");
    (F(a, e.markers, e.type, e.diagramId),
      Y(),
      _(),
      H(),
      te(),
      ...
      e.edges.forEach((o) => {
        if (o.start === o.end) {
          ...
          ((v.label = ""),
            ...
            (p.arrowTypeEnd = "none"),
            (p.id = l + "-cyclic-special-mid"),
            (y.label = ""),
            m.isGroup && ((v.fromCluster = l), (y.toCluster = l)),
            (y.id = l + "-cyclic-special-2"),
            (y.arrowTypeStart = "none"),
            n.setEdge(l, u, v, l + "-cyclic-special-0"),
            n.setEdge(u, c, p, l + "-cyclic-special-1"),
            n.setEdge(c, l, y, l + "-cyc<lic-special-2"));
        } else n.setEdge(o.start, o.end, { ...o }, o.id);
      }),
      i.warn("Graph at first:", JSON.stringify(h(n))),
      re(n),
      i.warn("Graph after XAX:", JSON.stringify(h(n))));
    const r = V();
    await j(a, n, e.type, e.diagramId, void 0, r);
  }, "render");
export { ve as render };
