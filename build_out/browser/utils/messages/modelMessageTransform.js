...
exports.filterEmptyAssistantMessages = filterEmptyAssistantMessages;
exports.addInterruptedSentinel = addInterruptedSentinel;
exports.injectAgentTransition = injectAgentTransition;
exports.injectFileChangeNotifications = injectFileChangeNotifications;
exports.injectPostCompactionAttachments = injectPostCompactionAttachments;
exports.stripOrphanedToolCalls = stripOrphanedToolCalls;
exports.transformModelMessages = transformModelMessages;
exports.validateAnthropicCompliance = validateAnthropicCompliance;
exports.getAnthropicThinkingDisableReason = getAnthropicThinkingDisableReason;
...
const attachmentRenderer_1 = require("./attachmentRenderer");
/**
 ...
 */
function filterEmptyAssistantMessages(messages, preserveReasoningOnly = false) {
 ...
 * survives filtering.
 */
function addInterruptedSentinel(messages) {
    const result = [];
 ...
 */
function injectAgentTransition(messages, currentAgentId, toolNames, planContent, planFilePath) {
    ...
    const transitioningToExecOrOrchestrator = currentAgentId === "exec" || currentAgentId === "orchestrator";
    if (planContent && transitioningFromPlan && transitioningToExecOrOrchestrator) {
        ...
        const nextStepText = currentAgentId === "orchestrator"
            ? "orchestrate its implementation (do not re-plan)."
            : "implement it directly (do not re-plan).";
        const followupText = currentAgentId === "orchestrator"
 ...
 */
function injectFileChangeNotifications(messages, changedFileAttachments) {
    if (!changedFileAttachments || changedFileAttachments.length === 0) {
        return messages;
    }
    const notice = changedFileAttachments
        .map((att) => `Note: ${att.filename} was modified, either by the user or by a linter.\n` +
...
function findLatestLegacyCompactionSummaryIndex(messages) {
 ...
 */
function injectPostCompactionAttachments(messages, attachments) {
    if (!attachments || attachments.length === 0) {
        return messages;
    }
    const durableCompactionIndex = (0, compactionBoundary_1.findLatestCompactionBoundaryIndex)(messages);
    ...
    const compactionIndex = durableCompactionIndex !== -1
    ...
    if (compactionIndex === -1) {
        ...
        const syntheticMessage = {
            ...
            parts: [
                {
                    type: "text",
                    text: (0, attachmentRenderer_1.renderAttachmentsToContentWithBudget)(attachments, {
                        maxChars: attachments_1.MAX_POST_COMPACTION_INJECTION_CHARS,
                    }),
                },
            ],
            metadata: {
 ...
 */
function splitMixedContentMessages(messages) {
    const result = [];
    for (let i = 0; i < messages.length; i++) {
        const msg = messages[i];
        if (msg.role !== "assistant") {
        ...
        for (let groupIndex = 0; groupIndex < groups.length; groupIndex++) {
            ...
            if (group.type === "text") {
                ...
                if (nextGroup?.type === "tool-call" && nextGroup.parts.length > 0) {
                    const toolPartsToInclude = nextGroup.parts.filter((p) => p.type === "tool-call" && toolResultsById.has(p.toolCallId));
            ...
            const partsToInclude = group.parts.filter((p) => p.type === "tool-call" && toolResultsById.has(p.toolCallId));
...
function filterReasoningOnlyMessages(messages) {
    return messages.filter((msg) => {
        if (msg.role !== "assistant") {
            return true;
        }
...
function stripOrphanedToolCalls(messages) {
    ...
    const missingResults = new Set([...toolCallIds].filter((toolCallId) => !toolResultIds.has(toolCallId)));
    const orphanResults = new Set([...toolResultIds].filter((toolCallId) => !toolCallIds.has(toolCallId)));
 ...
 */
function coalesceConsecutiveNoProgressTaskAwaitPairs(messages) {
    ...
    function getToolCallInputFingerprint(part) {
        const input = part.input ?? part.args;
        return tryJsonStringify(input);
    }
    function extractToolResultValue(part) {
    ...
    function isNoProgressTaskAwaitResultValue(value) {
    ...
    function getTaskAwaitPairInfo(allMessages, index) {
 ...
 */
function stripUnsignedAnthropicReasoning(messages) {
    const stripped = messages.map((msg) => {
        ...
        const content = assistantMsg.content.filter((part) => {
            ...
            const anthropicMeta = part.providerOptions
                ?.anthropic;
 ...
 */
function coalesceConsecutiveParts(messages) {
...
function ensureAnthropicThinkingBeforeToolCalls(messages) {
    const result = [];
    for (const msg of messages) {
        if (msg.role !== "assistant") {
            result.push(msg);
        ...
        if (reasoningParts.length === 0 && result.length > 0) {
            ...
            if (prev.role === "assistant") {
                ...
                if (typeof prevAssistant.content !== "string") {
                    const prevIsReasoningOnly = prevAssistant.content.length > 0 &&
        ...
        if (reasoningParts.length === 0) {
            reasoningParts = [{ type: "reasoning", text: "..." }];
        }
        result.push({
            ...assistantMsg,
 ...
 */
function transformModelMessages(messages, provider, options) {
 ...
 */
function validateAnthropicCompliance(messages) {
    const pendingToolCalls = new Map(); // toolCallId -> message index
    for (let i = 0; i < messages.length; i++) {
        const msg = messages[i];
        if (msg.role === "assistant") {
            const assistantMsg = msg;
...
function hasAnthropicThinkingSignature(part) {
    const providerOptions = part?.providerOptions;
    return (typeof providerOptions?.anthropic?.signature === "string" &&
 ...
 */
function getAnthropicThinkingDisableReason(messages) {
    for (let i = 0; i < messages.length; i++) {
        const msg = messages[i];
        if (msg.role !== "assistant") {
            continue;
        ...
        const content = msg.content.filter((part) => part.type !== "reasoning" || hasAnthropicThinkingSignature(part));
        const hasToolCall = content.some((part) => part.type === "tool-call");
        if (!hasToolCall) {
            continue;
        }
...
//# sourceMappingURL=modelMessageTransform.js.map