...
exports.renderAttachmentToContent = renderAttachmentToContent;
exports.renderAttachmentsToContent = renderAttachmentsToContent;
exports.renderAttachmentsToContentWithBudget = renderAttachmentsToContentWithBudget;
const todoList_1 = require("../../../common/utils/todoList");
const SYSTEM_UPDATE_OPEN = "<system-update>\n";
const SYSTEM_UPDATE_CLOSE = "\n</system-update>";
function wrapSystemUpdate(content) {
  return `${SYSTEM_UPDATE_OPEN}${content}${SYSTEM_UPDATE_CLOSE}`;
}
/**
 * Render a plan file reference attachment to content string.
 */
function renderPlanFileReference(attachment) {
 ...
 */
function renderTodoListAttachment(attachment) {
  const items = (0, todoList_1.renderTodoItemsAsMarkdownList)(attachment.todos);
  return `TODO list (persisted; \`todo_read\` will return this):\n${items || "- (empty)"}`;
}
/**
 * Render an edited files reference attachment to content string.
 */
function renderEditedFilesReference(attachment) {
 ...
 */
function renderAttachmentToContent(attachment) {
 ...
 */
function renderAttachmentsToContent(attachments) {
  return attachments.map((att) => wrapSystemUpdate(renderAttachmentToContent(att))).join("\n");
}
const PLAN_TRUNCATION_NOTE = "\n\n...(truncated)\n";
function renderPlanFileReferenceWithBudget(attachment, maxChars) {
  if (maxChars <= 0) {
    return null;
  }
...
function renderEditedFilesReferenceWithBudget(attachment, maxChars) {
...
function sortAttachmentsForInjection(attachments) {
  const priority = {
...
function renderAttachmentsToContentWithBudget(attachments, options) {
