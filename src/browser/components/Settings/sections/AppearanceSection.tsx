...
export function AppearanceSection() {
    ...
    const handleActivateTheme = useCallback(async (themeId: string) => {
        if (!api) return;
        setError(null);
        try {
            await api.iconTheme.setActiveTheme({ themeId });
            setSuccessMessage(`Theme activated. Reload to apply.`);
            await fetchThemes();
        } catch {
    ...
    const handleDeleteTheme = useCallback(async (themeId: string, label: string) => {
        if (!api) return;
    ...
    const handleImportVsix = useCallback(async (file: File) => {
        if (!api) return;
        setIsImporting(true);
        setError(null);
        ...
        try {
            ...
            setError("Failed to import .vsix file. Make sure it's a valid VS Code icon theme extension.");
        } finally {
        ...
        <div className="space-y-6">
            ...
            <div>
                ...
                <div className="text-muted mt-1 text-xs">
                    Choose which icon theme to use for file and folder icons.
                    Import .vsix files from VS Code Marketplace extensions.
                </div>
            </div>
...
function ThemeCard({
    ...
    return (
        <div
            className={`border-border-medium bg-background-secondary flex items-center justify-between gap-3 rounded border px-3 py-2 ${isActive ? "ring-accent/40 ring-1" : ""
                }`}
        >
            <div className="flex min-w-0 items-center gap-3">
                ...
                <div className="min-w-0">
                    ...
                    {description && (
                        <div className="text-muted truncate text-xs">{description}</div>
                    )}
                </div>
            </div>
