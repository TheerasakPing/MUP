...
export function DiffViewModeSelector({ value, onChange, className }: DiffViewModeProps) {
  return (
    <ToggleGroup.Root
      ...
      className={`border-border bg-secondary/30 inline-flex rounded-lg border p-0.5 ${className || ""}`}
    >
      <ToggleGroup.Item
        value="inline"
        className="hover:bg-secondary/60 data-[state=on]:bg-background data-[state=on]:text-foreground flex items-center gap-1.5 rounded-md px-2.5 py-1.5 text-xs font-medium transition-colors data-[state=on]:shadow-sm"
